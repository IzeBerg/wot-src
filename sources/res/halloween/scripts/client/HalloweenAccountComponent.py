...
from halloween_common.halloween_account_commands import CMD_UNLOCK_TOKEN, CMD_LOCK_TOKEN, CMD_HW23_ARTEFACT_OPEN, CMD_HW23_BUY_BUNDLE
...
class HalloweenAccountComponent(BaseAccountExtensionComponent):
    ...
    def openArtefact(self, artefactID, isSkipQuest, callback=None):
        if callback is not None:
            ...
            proxy = None
        self.account._doCmdInt2Str(CMD_HW23_ARTEFACT_OPEN, self.account.shop.getCacheRevision(), int(isSkipQuest), artefactID, proxy)
        return

    def buyBundle(self, bundleID, callback=None):
        if callback is not None:
            proxy = lambda requestID, resultID, errorStr, ext={}: callback(requestID, resultID, errorStr)
        else:
            proxy = None
        self.account._doCmdIntStr(CMD_HW23_BUY_BUNDLE, self.account.shop.getCacheRevision(), bundleID, proxy)
        return
