import copy, logging, BigWorld, halloween_account_commands
...
class HalloweenVehicle(object):
    ...
    def fromVehicle(cls, item):
        if isinstance(item, HalloweenVehicle):
            return item
        return HalloweenVehicle(item)
...
class HWAccountEquipmentController(BigWorld.StaticScriptComponent):
    ...
    def makeVehicleHWAdapter(self, vehicle):
        if isinstance(vehicle, HalloweenVehicle):
            return vehicle
        return HalloweenVehicle.fromVehicle(vehicle)
