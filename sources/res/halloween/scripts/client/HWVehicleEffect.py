import enum, Math, BigWorld, AnimationSequence
...
class SequenceEffect(object):
    __slots__ = ('__active', '__animator', '__bindNode', '__spaceID', '__model', '__loadingTaskID',
                 '__owner', '__config', '__sequencePatch', '__unapplyCbkID', '__isPlaying',
                 '__timeToFinish', '__soundGameObject', '__soundStart', '__weakref__')
...
class Effect(object):
    ...
    __slots__ = ('__playerEffect', '__applyAttempts', '__owner', '__config', '__duration',
                 '__isPlaying', '__soundStart')
...
class HWVehicleEffect(BigWorld.DynamicScriptComponent):
    ...
    def __initSequncesData(self, isSpecial=False):
        if self.__sequences is None:
            self.__sequences = [self.__getSequence(self.OwnerType.SELF, isSpecial),
             self.__getSequence(self.OwnerType.TEAM_MATE, isSpecial),
             self.__getSequence(self.OwnerType.ENEMY, isSpecial)]
        return

    def __initEffectsData(self, isSpecial=False):
        if self.__effects is None:
            self.__effects = [self.__getEffect(self.OwnerType.SELF, isSpecial),
             self.__getEffect(self.OwnerType.TEAM_MATE, isSpecial),
             self.__getEffect(self.OwnerType.ENEMY, isSpecial)]
        return
    ...
    def __initExhaustData(self, isSpecial=False):
        if self.__exhaust is None:
            ...
            self.__exhaust = [
             Exhaust(self.entity, selfExhaust) if selfExhaust else None,
             None,
             None]
        return
