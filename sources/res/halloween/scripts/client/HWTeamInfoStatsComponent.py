...
class HWTeamInfoStatsComponent(BigWorld.DynamicScriptComponent):
    ...
    def getSouls(self, vehicleID):
        return self._getValue(self.souls, vehicleID)

    def getTeamSouls(self):
        return sum(info.value for info in self.souls)

    def getSoulsDelivered(self, vehicleID):
        return self._getValue(self.soulsDelivered, vehicleID)
    ...
    def set_souls(self, prev):
        self.onTeamStatsUpdated()
        self.onTeamSoulsUpdated()

    def set_soulsDelivered(self, prev):
        self.onTeamStatsUpdated()
