...
class HWBattleVehicleStatsBlock(RegularVehicleStatsBlock):
    __slots__ = ('hwXP', 'vehicleLvl', 'vehicleType', 'badgeID', 'respawns', 'isPrematureLeave',
                 'fairplayViolations', 'isPremiumIGR')
...
class RegularVehicleStatValuesBlock(base.StatsBlock):
    __slots__ = ('_isPersonal', '_filters', 'shots', 'hits', 'explosionHits', 'damageDealt',
                 ...
                 'damageAssistedStun', 'stunNum', 'stunDuration', 'mileage', '__rawDamageAssistedStun',
                 '__rawStunNum', 'respawns')
...
class ArenaDurationItem(base.StatsItem):
    ...
    def _convert(self, record, reusable):
        if record:
            return backport.text(R.strings.battle_results.hw_details.time.value(), min=self.formatted(int(record / ONE_MINUTE)), sec=self.formatted(int(record % ONE_MINUTE)))
        return ''
        ...
        for qID, progress in sorted(hwQuestProgress.iteritems(), key=lambda item: item[0]):
            ...
            if qID in questsProgress:
                ...
                info.update({'currentProgress': currentProgress, 
                   'prevProgress': pPrev})
            if needShow:
                self.addComponent(self.getNextComponentIndex(), base.DirectStatsItem('', {'currentPhaseIndex': phaseIndex, 'info': info}))