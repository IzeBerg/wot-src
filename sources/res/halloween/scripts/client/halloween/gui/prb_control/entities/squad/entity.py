...
class HalloweenBattleSquadEntity(SquadEntity):
    ...
    def getConfirmDialogMeta(self, ctx):
        if not self.__controller.isEnabled() or self.__controller.isPostPhase():
            ...
            if self.isCommander() and len(self.getPlayers()) == 1:
                return
            return self.createUnitLeaveMeta(ctx, self.canSwitch(ctx))
    ...
    def createUnitLeaveMeta(self, unlockCtx, isSwitching=False):
        ...
        if (unlockCtx.hasFlags(FUNCTIONAL_FLAG.SWITCH) or isSwitching) and not isEventEntityCtx or unlockCtx.hasFlags(FUNCTIONAL_FLAG.EXIT) and isEventEntityCtx:
            return self.__getLeaveSquadBuilder(R.strings.hw_dialogs.squad.leave)
        return self.__getLeaveSquadBuilder(R.strings.hw_dialogs.squad.goToAnother)
    ...
    def _createActionsValidator(self):
        if self.isBalancedSquadEnabled():
            return HalloweenBattleBalanceSquadActionsValidator(self)
        return HalloweenBattleSquadActionsValidator(self)

    def _createActionsHandler(self):
        if self.isBalancedSquadEnabled():
            return HalloweenBattleBalancedSquadActionsHandler(self)
        return HalloweenBattleSquadActionsHandler(self)
