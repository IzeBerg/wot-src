...
class HWBattleQuestsRewardHandler(ServiceChannelHandler):
    ...
    def _showAward(self, ctx):
        ...
        if self._completedPhaseQuestIds:
            ...
            for quest in sorted(completedPhaseQuests, key=lambda q: q.getID()):
                ...
                elif int(result.group(2)) == self.SECOND_QUEST_INDEX:
    ...
    def _showRewardWindow(self, screenType, bonuses=None, phase=None):
        ...
        data = {'screenType': screenType, 'phase': phase}
            ...
            for bonusType in bonusesType:
                if isinstance(bonusType, DossierBonus):
                    for item in bonusType.getValue().itervalues():
                        for key in item:
                            bonuses.append((None, key[0], key[1], 1))

                elif isinstance(bonusType, CustomizationsBonus):
                    for item in bonusType.getValue():
                        bonuses.append((None, item['custType'], item['id'], item['value']))
