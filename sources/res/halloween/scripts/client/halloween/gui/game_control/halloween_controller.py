...
class _HalloweenConfig(namedtuple('_HalloweenConfig', (
 'isEnabled',
 'peripheryIDs',
 'primeTimes',
 'seasons',
...
PhaseBoosterInfo = namedtuple('PhaseBoosterInfo', (
 'bonusName', 'bonusValue', 'isActive', 'startTime', 'finishTime', 'timeLeft'))
...
Parse error at or near `RETURN_END_IF' instruction at offset 51
    ...
    def getAllAbilitiesInfo(self, dailyQuest=False):
        ...
        if not abilityBonus:
            ...
            return (
             abilityBonus.getItems(), missionData)
...
def sortedFunction(item):
    ...
    if type in __type.ORDERED_LIST:
        return __type.ORDERED_LIST.index(type)
    return 0
...
class ProgressQuest(object):
    ...
    def getBonusesInfo(self):
        ...
        for bonuses in getMissionInfoData(self._quest).getSubstituteBonuses():
            ...
            elif bonusesName == 'credits':
                info.append((bonusesName, HWBonusesType.CREDITS, None, bonuses.getValue()))
            ...
            elif bonusesName == 'battleToken':
                for key, data in bonuses.getTokens().iteritems():
                    if 'lootBox' in key:
                        _, id = data.id.split(':')
                        info.append((id, HWBonusesType.LOOT_BOXES, None, data.count))
