...
class BattleResultView(BaseEventView, CallbackDelayer):
    ...
    def createContextMenu(self, event):
        if event.contentID == R.views.common.BackportContextMenu():
            ...
            if dbID == self.currentPlayerID:
                return
            contextMenuArgs = {'dbID': event.getArgument('playerId'), 'userName': event.getArgument('userName'), 
    ...
    def __fillTeams(self, model, vo):
        for team, playerVO, isAlly in ((model.getPlayerTeam(), vo['team1'], True),
         (
          model.getEnemyTeam(), vo['team2'], False)):
    ...
    def __isPlayerInSquad(self, dbID):
        if not self.platoonCtrl.isInPlatoon() or self.platoonCtrl.getPrbEntityType() != PREBATTLE_TYPE.EVENT:
            return False
