...
class GlobalProgressionView(BaseEventView):
    ...
    def __getIcon(self, bonusType, bonusID):
        ...
        if bonusType == HWBonusesType.MEDAL:
            return backport.image(R.images.gui.maps.icons.achievement.c_80x80.dyn(bonusID)())
        return ''

    def __getInfoCardsData(self):
        return [
         self.__getTankmenProgressInfo(), self.__getMainHWXPProgressInfo()]
    ...
    def __getTankmenProgressInfo(self):
        ...
        return {'type': QuestTypeEnum.TANKMAN, 'currentProgress': len(tankmanPassedTokens), 
    ...
    def __getMainHWXPProgressInfo(self):
        ...
        for bonusType in bonusesType:
            ...
            elif isinstance(bonusType, CustomizationsBonus):
    ...
    def __badgeTooltip(self, badgeID):
        window = backport.BackportTooltipWindow(backport.createTooltipData(isSpecial=True, specialAlias=HWTooltips.HW_BADGE, specialArgs=[
         int(badgeID)]), self.getParentWindow())
        window.load()
        return window
