...
from halloween_common.halloween_constants import UNIT_HALLOWEEN_EXTRA_DATA_KEY, UNIT_DIFFICULTY_LEVELS_KEY, QUEUE_TYPE
        ...
        for slotInfo in unitFullData.slotsIterator:
            ...
            for slot in slots:
                ...
                if queueType in availableQueueTypes:
                    continue
                maxAvailableQueue = max([ q for q in availableQueueTypes if q != QUEUE_TYPE.HALLOWEEN_DEFENCE ])
                additionalMsg = backport.text(R.strings.halloween_platoon.platoon.difficulty.maxAvailable(), icon=('level_{0}').format(QUEUE_TYPE_TO_DIFFICULTY_LEVEL[maxAvailableQueue].value))
