import functools, copy, BigWorld
...
class HWDataProvider(_VehParamsDataProvider, IGlobalListener):
    ...
    def _getComparator(self):
        if self.prbDispatcher is not None and (self.prbDispatcher.getFunctionalState().isInPreQueue(QUEUE_TYPE.HALLOWEEN_BATTLES) or self.prbDispatcher.getFunctionalState().isInUnit(PREBATTLE_TYPE.HALLOWEEN_BATTLES) or self.prbDispatcher.getFunctionalState().isInPreQueue(QUEUE_TYPE.HALLOWEEN_BATTLES_WHEEL)):
            return self.hwVehicleComparator(self._cache.item)
        else:
            return super(HWDataProvider, self)._getComparator()
