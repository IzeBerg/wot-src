...
_FEEDBACK_EVENT_TYPE_TO_PERSONAL_EFFICIENCY_TYPE.update({_FET.PLAYER_DAMAGED_DEVICE_ENEMY: (
                                    HW_EFFICIENCY_SUPER_SHELL_CRITS, _CriticalHitsEfficiencyInfo)})

class _HWSuperShellCritsRecordVOBuilder(_LogRecordVOBuilder):

    def buildVO(self, info, arenaDP):
        if not info.isSuperShell(primary=False):
            return None
        else:
            return super(_HWSuperShellCritsRecordVOBuilder, self).buildVO(info, arenaDP)
...
class _HalloweenLogViewComponent(_LogViewComponent):
    ...
    def _buildLogMessageVO(self, info):
        ...
        if builder is not None:
            return builder.buildVO(info, self._LogViewComponent__arenaDP)
        else:
            return super(_HalloweenLogViewComponent, self)._buildLogMessageVO(info)
