...
class BattleLoading(HWBattleLoadingMeta, IArenaVehiclesController):
    ...
    def _getPages(self):
        return [ {'title': backport.text(_R_STRINGS.title.num(i, default=R.invalid)()), 'titlePart': backport.text(_R_STRINGS.title_part.num(i, default=R.invalid)()), 'description': backport.text(_R_STRINGS.description.num(i, default=R.invalid)()), 'background': backport.image(_R_IMAGES.dyn(('halloween_{id}').format(id=i), default=R.invalid)())} for i in xrange(1, R.strings.hw_tips.loading.title.length() + 1)
               ]
