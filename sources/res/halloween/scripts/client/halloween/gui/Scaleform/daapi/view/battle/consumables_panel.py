...
class HWConsumablesPanel(HWConsumablesPanelMeta):
    ...
    def _updateEquipmentSlot(self, idx, item):
        ...
        if not isGlowAllowed or currStage in (EQUIPMENT_STAGES.NOT_RUNNING, EQUIPMENT_STAGES.DEPLOYING, EQUIPMENT_STAGES.COOLDOWN):
         ...
         prevStage, currStage) == (EQUIPMENT_STAGES.COOLDOWN, EQUIPMENT_STAGES.ACTIVE):
            self._showHWEqupmentGlow(idx, glowID=CONSUMABLES_PANEL_SETTINGS.GLOW_ID_GREEN)
    ...
    def _buildEquipmentSlotTooltipText(self, item):
        ...
        if isinstance(descriptor, EventEquipment):
            if int(descriptor.durationSeconds) > 0:
                body = ('\n').join((
                 body,
                 backport.text(R.strings.hw_ingame_gui.consumablesPanel.tooltip.durationTime(), time=str(int(descriptor.durationSeconds)))))
