...
class _HpRepairAndCrewHeal(_EventItem):
    ...
    def _checkCrew(self, avatar):
        ...
        if not result and not isinstance(error, (NeedEntitySelection, NotApplyingError)) and avatar_getter.isVehicleStunned() and self.isReusable:
...
class _DamageShield(_EventBuffItem):
    ...
    def canActivate(self, entityName=None, avatar=None):
        ...
        if not vehicle:
            ...
            if damageShieldComponent is not None:
                return (False, _ActivationError('equipmentAlreadyActivated', {'name': self._descriptor.userString}))
            return super(_DamageShield, self).canActivate(entityName, avatar)
