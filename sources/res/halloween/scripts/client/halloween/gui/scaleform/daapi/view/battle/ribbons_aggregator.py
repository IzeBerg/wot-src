...
from gui.Scaleform.daapi.view.battle.shared.ribbons_aggregator import RibbonsAggregator, _RibbonClassFactory, _ACCUMULATED_RIBBON_TYPES, _FEEDBACK_EVENT_TO_RIBBON_CLS_FACTORY as _DEFAULT_RIBBON_FACTORIES, _CausedDamageRibbon, _RIBBON_TYPES_AGGREGATED_WITH_KILL_RIBBON as _DEFAULT_RIBBONS_AGGREGATED_WITH_KILL, DAMAGE_SOURCE, _Ribbon, _SingleVehicleReceivedHitRibbon
...
class HWRibbonsAggregator(RibbonsAggregator):
    ...
    def _aggregateRibbons(self, ribbons):
        ...
        for destEffType, sourceClasses in self.RIBBON_TYPES_COPY_TO.iteritems():
            sourceRibbons = [ r for r in ribbons if r.__class__ in sourceClasses ]
            destRibbons = [ r for r in ribbons if r.getType() == destEffType ]
