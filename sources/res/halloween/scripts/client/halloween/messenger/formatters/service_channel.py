...
class EventLifetimeFormatter(ServiceChannelFormatter):
    ...
    _PHASE_TYPE_FORMATTERS = {PhaseType.REGULAR: {(FIRST_PHASE_INDEX, PhaseTransition.STARTED): (
                                                                        'HW22EventStartedSystemMessage', [])}, 
       PhaseType.POST: {PhaseTransition.STARTED: (
                                                'HW22PostSaleStartedSystemMessage', ['date']), 
                        PhaseTransition.ENDED: (
                                              'HW22EventFinishedSystemMessage', [])}}
    ...
    def format(self, data, *args):
        ...
        if phase is None:
            ...
            return [
             MessageData(formatted, self._getGuiSettings(data, formatter))]
