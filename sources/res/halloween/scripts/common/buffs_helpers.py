...
def makeBuffName(buffName, typeDescriptor):
    return buffName.format(**{'lvl': typeDescriptor.level, 'class': typeDescriptor.type.getVehicleClass()})
...
class ModifiersDict(object):
    ...
    def __combineModifiers(modifiersA, modifiersB):
        ...
        if isTupleA or isTupleB:
            ...
            modifiersB = (isTupleB or modifiersB,) if 1 else modifiersB
...
class ValueSimpleModifier(object):
    ...
    OPERATIONS_ORDERED = (
     (
      ModifierOpType.ADD_FACTOR,
      lambda a, b: a * (ValueSimpleModifier.DEFAULT_FACTOR + b)),
     (
      ModifierOpType.ADD_VALUE,
      lambda a, b: a + b),
     (
      ModifierOpType.SET_VALUE,
      lambda a, b: b))
    VALUE_FAIL_CHECKS = (
     lambda a, b: isinstance(a, tuple) and isinstance(b, tuple) and len(a) != len(b),
     lambda a, b: isinstance(a, list) and isinstance(b, tuple) and len(a) != len(b),
     lambda a, b: isinstance(a, (int, float)) and isinstance(b, tuple))
