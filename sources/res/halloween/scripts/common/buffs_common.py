    ...
    def getSpecialBuffs(self, buffName=None):
        if buffName is not None:
            ...
            if buff is not None:
                return [buff]
            return
        if self._specialBuffs:
            return list(self._specialBuffs.values())
        else:
            return
...
class HWGettingFatApple(object):
    __slots__ = ('id', 'name', 'componentName', 'increaseMaxHealth', 'maxHealth', 'factors',
                 'minValueFactors', 'effects')
