import battle_params, battle_modifier_readers, battle_modifier_appliers
...
class BattleModifier(object):
    __slots__ = ('param', 'useType', 'value', 'gameplayImpact', '__id', '__descr',
                 '__shouldLimit')
    ...
    def __call__(self, value):
        ...
        if self.__shouldLimit:
            return self.param.valueLimiter(modifiedValue)
...
class BattleModifiers(battle_modifiers.BattleModifiers):
    ...
    def __call__(self, paramId, value):
        if paramId in self.__modifiers:
            return self.__modifiers[paramId](value)
        return value
