...
Token = namedtuple('Token', [
 'prefix', 'launchID', 'tradedBranch', 'receivedBranch', 'tradeSummary'])
...
class TradeInToken(object):
    ...
    def generateToken(launchID, tradedBranch, receivedBranch, tradeSummary=__PENDING):
        return (':').join([TradeInToken.PREFIX, launchID, tradedBranch, receivedBranch, tradeSummary])
    ...
    def isBlockerToken(token):
        if token.startswith(TradeInToken.PREFIX) and TradeInToken.parseToken(token).tradeSummary == TradeInToken.__PENDING:
            return True
        return False
...
def generateBranchSign(branch):
    ...
    if branch[(-1)] in config['branches']['multy10']:
        return ('{}!{}').format(vehicleName(branch[0]), vehicleName(branch[(-1)]))
    return vehicleName(branch[(-1)])
