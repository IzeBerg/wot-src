...
class TechTreeTradeInCompletedFormatter(ServiceChannelFormatter):
    ...
    def __formatBlueprints(self, blueprintResources):
        ...
        return self.__SPACE.join([
         backport.text(self.__TECH_TREE_TRADE_IN_MESSAGES.blueprints.title()),
         (', ').join(blueprintStrings)])
...
class TechTreeTradeInDetailsFormatter(ServiceChannelFormatter):
    ...
    def __formatDetails(self, details):
        detailsStrings = [
         backport.text(self.__TECH_TREE_TRADE_IN_MESSAGES.details.crew(), sentToBarracksAmount=self.__formatAmount(details['crew']['sentToBarracks']), nationalRescruitsAmount=self.__formatAmount(details['crew']['recruits'])),
         backport.text(self.__TECH_TREE_TRADE_IN_MESSAGES.details.postProgressionCompensation(), amount=self.__formatAmount(details['postProgressionCompensation']['currency'][Currency.CREDITS])),
         backport.text(self.__TECH_TREE_TRADE_IN_MESSAGES.details.xpTransfer(), vehicleName=text_styles.credits(self.__getVehName(details['xpTransferredVehCD'])))]
