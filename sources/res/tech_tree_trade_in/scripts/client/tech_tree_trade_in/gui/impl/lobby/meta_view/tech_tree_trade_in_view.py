...
class TechTreeTradeInView(ViewImpl):
    __slots__ = ('__moneyBalanceWidget', '__subViews', '__currentSubView', '__viewId',
                 '__summaryRequestCache', '__errorNotificationShown')
    ...
    def switchToSummaryView(self, price):
        ...
        responseType = self.__summaryRequestCache.request((
         branchToTradeId, branchToReceiveId), partial(self.__onTradeSummaryReceived, price), self.__onTradeSummaryRequestError)
    ...
    def _getEvents(self):
        return (
         (
          self.viewModel.mainOverlayModel.onSwitchContent, self.switchContent),
         (
    ...
    def __initSubViews(self):
        subViews = (
         IntroSubView(self.viewModel.introModel, self),
    ...
    def __onTradeSummaryReceived(self, price, _, data):
        ...
        self.switchContentWithContext(MainViews.SUMMARY, {'price': price, 'summary': data})
