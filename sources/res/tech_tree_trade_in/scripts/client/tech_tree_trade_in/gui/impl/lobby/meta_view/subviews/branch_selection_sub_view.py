...
class BranchSelectionSubView(SubViewBase):
    ...
    __slots__ = ('__branchToTradeId', '__branchToReceiveId', '__priceRequestCache',
                 '__summaryRequestCache', '__currentPrice')
    ...
    def getTooltipData(self, event):
        ...
        if tooltipId == TOOLTIPS_CONSTANTS.TECHTREE_VEHICLE:
            ...
            return createTooltipData(tooltip=tooltipId, isSpecial=True, specialAlias=tooltipId, specialArgs=[
             _Node(id=node['id'], state=node['state'], unlockProps=UnlockProps(*node['unlockProps'])),
             0])
        else:
            return
    ...
    def _getEvents(self):
        return (
         (
          self.viewModel.onConfirm, self.onConfirm),
         (
          self.viewModel.onSelectBranchToReceiveId, self.__onSelectBranchToReceiveId),
         (
          self.viewModel.onSelectBranchToTradeId, self.__onSelectBranchToTradeId))
    ...
    def __requestPrice(self):
        if self.__branchToTradeId == -1 or self.__branchToReceiveId == -1:
            return
        responseType = self.__priceRequestCache.request((
         self.__branchToTradeId, self.__branchToReceiveId), self.__onPriceReceived)
