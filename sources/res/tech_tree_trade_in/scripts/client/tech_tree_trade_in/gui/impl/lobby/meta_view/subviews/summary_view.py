...
_VEH_TRADED_STATUSES_GROUP = (
 SummaryViewModel.VEHICLE_TRADED_RESEARCH,
 SummaryViewModel.VEHICLE_TRADED_RESEARCH_INVENTORY)
...
def _getMovedItemsSubcategoryText(items):
    if items:
        return backport.text(rCategory.movedItemsSubcategory())
    else:
        return
...
class SummaryView(SubModelPresenter):
    ...
    def __fillSummary(self, vehPropsArray, tradeData):
        ...
        for vehCD in tradeData[BranchType.BRANCHES_TO_TRADE]:
            ...
            if vehState in (
             SummaryViewModel.VEHICLE_TRADED_RESEARCH_INVENTORY, SummaryViewModel.VEHICLE_TRADED_RESEARCH):
    ...
    def __mapVehicleStatus(isTraded=False, isInInventory=False, isRented=False):
        ...
        if isInInventory and not isRented:
            return SummaryViewModel.VEHICLE_TRADED_RESEARCH_INVENTORY
        return SummaryViewModel.VEHICLE_TRADED_RESEARCH
