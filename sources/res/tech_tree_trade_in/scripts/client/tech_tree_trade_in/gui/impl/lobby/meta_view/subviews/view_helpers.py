...
class RequestDataCache(CallbackDelayer):
    ...
    def _response(self, key, callback, onErrorCallback, response):
        ...
        if not isCodeValid(resultId):
            LOG_DEBUG_DEV('TechTreeTradeIn: %s: error response from server for key %r: %r' % (
             self.__class__.__name__, key, errorCode))
            if onErrorCallback:
    ...
    def __getRequestDelay(self):
        if not self.__previousRequestTime:
            return 0
        return max(self._COOLDOWN - (BigWorld.time() - self.__previousRequestTime), 0)
