...
class FunProgressionQuestPostBattleInfo(QuestPostBattleInfo, FunProgressionWatcher):
    ...
    def _getProgresses(self, pCur, pPrev):
        ...
        if stage is not None and prevCounter < currCounter:
            return [{'progrTooltip': None, 'progrBarType': formatters.PROGRESS_BAR_TYPE.SIMPLE, 'currentProgrVal': currCounter - stage.prevRequiredCounter, 'maxProgrVal': stage.requiredCounter - stage.prevRequiredCounter, 'description': backport.text(descriptionRes(), triggerDescription=self.event.getDescription()), 'progressDiff': '+ %s' % backport.getIntegralFormat(currCounter - prevCounter), 'progressDiffTooltip': TOOLTIPS.QUESTS_PROGRESS_EARNEDINBATTLE}]
        else:
            return []
