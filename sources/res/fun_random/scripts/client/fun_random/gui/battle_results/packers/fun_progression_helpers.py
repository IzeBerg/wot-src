...
class _PostbattleProgressionHelper(object):
    ...
    def _getPoints(self):
        return (
         sum([ data.get('diff', 0) for data in self._countersProgress.values() ]),
         sum([ data.get('total', 0) for data in self._countersProgress.values() ]))
