...
class FunRandomSelectorItem(ModeSelectorLegacyItem):
    ...
    def _getIsDisabled(self):
        ...
        if isNotStarted:
            return super(FunRandomSelectorItem, self)._getIsDisabled()
        else:
            return super(FunRandomSelectorItem, self)._getIsDisabled() or not self.__funRandomCtrl.canGoToMode()
    ...
    def _getDisabledTooltipText(self):
        if not self.__funRandomCtrl.hasSuitableVehicles() and not self.__funRandomCtrl.isFrozen():
            return self.__getNotAvailableVehiclesTooltip()
        return super(FunRandomSelectorItem, self)._getDisabledTooltipText()
