...
class ShopBundle(object):
    ...
    @property
    def prices(self):
        if self._price.type == EventShop.PriceType.MULTI:
            return self._price.subPrices
        return (self._price,)
...
class EventShopBundlePurchaseProcessor(Processor):
    ...
    def _getPlugins(self):
        ...
        def makePlugin(price):
            if price.currencyType == EventShop.CurrencyType.REAL:
                return plugins.MoneyValidator(Money(**{price.currency: price.amount}))
            return plugins.TokenValidator(price.currency, price.amount)
...
class EventShopAccountComponentBase(BigWorld.StaticScriptComponent):
    ...
    def purchaseBundle(self, bundleID, count=1, callback=None):
        self.entity._doCmdIntArrStrArr(self._purchaseCmdID, [
         count], [
         bundleID], callback)
    ...
    def getBundlePurchasesLeft(self, bundle):
        if bundle.purchasesLimit is not None:
            return min(max(0, limit - self._getCounterTokenCount(limitID)) for limitID, limit in bundle.purchasesLimit)
        else:
            return

    def getGroupPurchasesLeft(self, groupID, default=None):
        group = self._shopData.get('groups', {}).get(groupID)
        if group is None or 'limit' not in group:
            return default
        return max(0, group['limit'] - self._getCounterTokenCount(groupID))
