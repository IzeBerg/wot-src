...
class OptionalDevicesView(BaseShopGroupView):
    ...
    def __onItemClicked(self, args):
        ...
        if bundleID is None:
            ...
            data = {'titleText': backport.text(R.strings.hb_shop.opt_device_buy_dialog.title(), name=device.userName), 
               'overlays': [
                          iconsRes.dyn(('{}_{}').format(device.getOverlayType(), 'overlay'))()], 
               'backgrounds': [
                             R.images.historical_battles.gui.maps.icons.common.upgrade_bg_enhanced()], 
    ...
    def __getDevice(bundle):
        return next((device for bonus in bundle.bonuses for device, count in bonus.getItems().iteritems() if GUI_ITEM_TYPE.OPTIONALDEVICE == device.itemTypeID), (None,
                                                                                                                                                                  None))
