...
ShopBonusItemInfo = namedtuple('ShopBonusItemInfo', (
 'bonusItem', 'iconName', 'nationName', 'inventoryCount', 'description',
 'dialogDescription'))
ShopBonusItemInfo.__new__.__defaults__ = (None, None, '', None, '', '')
COLOR_TAG_OPEN = '{green_Open}'
COLOR_TAG_CLOSE = '{green_Close}'
GOODIES_ORDER = [
 GOODIE_RESOURCE_TYPE.XP, GOODIE_RESOURCE_TYPE.FL_XP, GOODIE_RESOURCE_TYPE.CREW_XP,
 GOODIE_RESOURCE_TYPE.FREE_XP, GOODIE_RESOURCE_TYPE.GOLD, GOODIE_RESOURCE_TYPE.CREDITS]
BATTLE_BOOSTERS_ORDER = [
 'equipmentBattleBooster', 'crewSkillBattleBooster']
...
def crewBookTabComparator(preformattedBonusA, preformattedBonusB):
    ...
    def nationComparator(item1, item2):
        ...
        if isNoNation(item1) and isNoNation(item2):
            return 0
        return -cmp(isNoNation(item1), isNoNation(item2)) or cmp(GUI_NATIONS_ORDER_INDICES[item1.getNationID()], GUI_NATIONS_ORDER_INDICES[item2.getNationID()])
...
def equipmentsTabComparator(preformattedBonusA, preformattedBonusB):
    ...
    def getNation(item):
        ...
        if not compatibleNations or len(compatibleNations) == len(nations.INDICES):
            return nations.ALL_NATIONS_INDEX
        return GUI_NATIONS_ORDER_INDICES[min(compatibleNations)]
...
def getNationName(nationIndex):
    if nationIndex is None:
        ...
        if nationIndex != nations.NONE_INDEX:
            return nations.NAMES[nationIndex]
        return ''
    ...
    def _getItemDescription(item):
        if item.itemTypeName == FITTING_TYPES.BOOSTER:
            if item.isCrewBooster():
                description = item.getCrewBoosterDescription(True, {'colorTagOpen': COLOR_TAG_OPEN, 'colorTagClose': COLOR_TAG_CLOSE})
            else:
    ...
    def __fillShowcaseBundles(self, model):
        ...
        for bundle in showcaseBundles:
            ...
            self.__confirmationDialogDataCache[bundle.id] = {'icon': R.images.historical_battles.gui.maps.icons.shop.bundles.dyn(('{}_260x135').format(bundle.id))(), 
               ...
               'backgrounds': [
                             R.images.historical_battles.gui.maps.icons.common.upgrade_bg_regular()], 
               'bundle': bundle}
