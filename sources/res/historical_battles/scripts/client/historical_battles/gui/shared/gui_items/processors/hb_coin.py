...
class HBCoinsExchangeValidator(SyncValidator):
    ...
    def _validate(self):
        if self._areEqual() or self._notEnoughCoins() or self._notEnoughCredits() or self._exchangeDisabled() or self._isAnyEmpty():
            return plugins.makeError(text(R.strings.hb_shop.errors.serverError()))
...
class HBCoinsExchangeProcessor(Processor):
    ...
    def __init__(self, coinsToGive, coinsToGet, count):
        pluginList = (
         HBCoinsExchangeValidator(coinsToGive, coinsToGet, count),)
