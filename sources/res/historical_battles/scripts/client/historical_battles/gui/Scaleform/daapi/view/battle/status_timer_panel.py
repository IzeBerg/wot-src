...
class HBStaticDeathZoneSN(_DeathZoneSN):
    ...
    def _canBeShown(self, value):
        if super(HBStaticDeathZoneSN, self)._canBeShown(value):
...
class HBPersonalDeathZoneSN(_DeathZoneSN):
    ...
    def _canBeShown(self, value):
        if super(HBPersonalDeathZoneSN, self)._canBeShown(value):
            return value.needToShow()
        return False
...
class _HBHighPriorityGroup(components.StatusNotificationsGroup):
    ...
    def __init__(self, updateCallback):
        super(_HBHighPriorityGroup, self).__init__((
         HBDamagingZoneSN,
         HBStaticDeathZoneSN,
         sn_items.FireSN,
