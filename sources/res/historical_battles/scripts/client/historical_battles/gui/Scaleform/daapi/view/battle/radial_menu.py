...
HB_OVERRIDE_SHORTCUTS_BY_ACTION = {BATTLE_CHAT_COMMAND_NAMES.OBJECTIVES_POINT: (
                                              radial_menu.Shortcut(title=INGAME_HELP.RADIALMENU_GOING_THERE, action=BATTLE_CHAT_COMMAND_NAMES.OBJECTIVES_POINT, icon=ATTACK_BOSS, groups=[
...
class HistoricalRadialMenu(radial_menu.RadialMenu):
    ...
    def _getSubtypeMapByMarkersType(self, targetMarkerType):
        if targetMarkerType != MarkerType.BASE_MARKER_TYPE:
    ...
    def _getCanReplyShortcut(self, shortcut, canReplyAction):
        ...
        if override:
            return override
        return super(HistoricalRadialMenu, self)._getCanReplyShortcut(shortcut, canReplyAction)

    def _getAlterCanReplyShortcut(self, shortcut, canReplyAction):
        override = self.__overrideShortcut(shortcut, canReplyAction)
        if override:
            return override
        return super(HistoricalRadialMenu, self)._getAlterCanReplyShortcut(shortcut, canReplyAction)
