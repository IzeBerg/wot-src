...
class SimpleGoalFormatter(object):
    ...
    def formatTitle(cls, goalsInfo):
        ...
        if not message:
            return ''
        return makeHtmlString('html_templates:hbEventGoals', 'title', {'message': backport.text(message())})
    ...
    def formatDescription(cls, goalsInfo):
        ...
        state = goalsInfo['state']
        if state == GoalState.ACTIVE:
            return cls._formatDescription(goalsInfo)
        return ''
    ...
    def formatProgress(cls, goalsInfo):
        ...
        state = goalsInfo['state']
        if state == GoalState.ACTIVE:
            return cls._formatProgress(goalsInfo)
        return ''
    ...
    def _formatFinishReason(cls, goalsInfo):
        ...
        if not message:
            return ''
        return makeHtmlString('html_templates:hbEventGoals', key, {'message': backport.text(message())})
    ...
    def _formatProgress(cls, goalsInfo):
        ...
        kwargs = {'current': goalsInfo['progress'], 'needed': goalsInfo['progressTotal'], 
           'left': goalsInfo['progressTotal'] - goalsInfo['progress']}
