...
class HBObjectivesMarker(markers.LocationMarker, _ObjectivesMarkerMixin):
    ...
    def getPosition(self):
        if self._matrix is not None:
            return Math.Matrix(self._matrix).translation
        else:
...
class HBPlayerPanelMarker(markers.Marker):
    ALLOWED_MARKER_TYPES = {MarkerType.LOCATION_MARKER_TYPE: (
                                       _CMD_NAMES.GOING_THERE,
                                       _CMD_NAMES.OBJECTIVES_POINT), 
       MarkerType.VEHICLE_MARKER_TYPE: (
                                      _CMD_NAMES.ATTACK_ENEMY,
...
class HBVehiclePanelMarker(HBPlayerPanelMarker):
    ALLOWED_MARKER_TYPES = {MarkerType.VEHICLE_MARKER_TYPE: (
                                      _CMD_NAMES.ATTACK_ENEMY,
                                      _CMD_NAMES.ATTACKING_ENEMY,
                                      _CMD_NAMES.ATTACKING_ENEMY_WITH_SPG)}