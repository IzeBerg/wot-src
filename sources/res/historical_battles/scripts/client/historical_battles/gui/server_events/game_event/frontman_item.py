import typing, BigWorld, HBAccountSettings
...
def getFrontmanProgressItem(frontmanID, eventsCache=None):
    ...
    if quest:
        return GameEventProgressQuest(quest, FRONTMAN_PROGRESS_POINTS_TOKEN_TPL.format(frontmanID))
    else:
        return GameEventProgressItemEmpty(None)
...
class FrontmanItem(GameEventCollection):
    ...
    def getNextVehicleCD(self):
        ...
        if vehicles:
            return vehicles[self._getNextVehicleId()]['vehTypeCD']
        else:
            return
    ...
    def getHeroVehicle(self):
        ...
        if heroVehicle:
            return self.itemsCache.items.getItemByCD(heroVehicle['vehTypeCD'])
        else:
            return
