...
def preInit():
    ...
    initPrbSelector(prb_config, prb_config.PREBATTLE_ACTION_NAME.HISTORICAL_BATTLES, (
     ModeSelectorColumns.COLUMN_1, 10), HistoricalBattlesModeSelectorItem, HistoricalBattlesEntryPoint, addHistoricalBattlesType, __name__)
    for queueType in hb_constants_extension.QUEUE_TYPE.HB_RANGE:
    ...
    initPlatoonControllerConfig(hb_constants_extension.PREBATTLE_TYPE.HISTORICAL_BATTLES, HistoricalBattlesMembersView, [
     (
      EPlatoonLayout.MEMBER, PlatoonLayout(R.views.historical_battles.lobby.MembersWindow(), MembersWindow))], __name__)
    for queueType in hb_constants_extension.QUEUE_TYPE.HB_RANGE:
