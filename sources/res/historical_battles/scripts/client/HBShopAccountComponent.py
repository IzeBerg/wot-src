...
class HBShopAccountComponent(EventShopAccountComponentBase):
    ...
    def purchaseMainPrize(self, bundleID, crewType, withAmmo, callback=None):
        self.entity._doCmdIntArrStrArr(account_commands.CMD_PURCHASE_HB_SHOP_MAIN_PRIZE, [
         int(crewType), int(withAmmo)], [
         bundleID], callback)
    ...
    def _isShopDataUpdated(self, diff):
        ...
        if 'serverSettings' in diff:
            return HB_SHOP_GAME_PARAMS_KEY in diff['serverSettings']
        return False
