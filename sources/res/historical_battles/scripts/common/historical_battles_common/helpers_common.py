...
class EventShopBundlePrice(object):
    ...
    class SinglePrice(object):
        ...
        def __add__(self, other):
            if isinstance(other, EventShopBundlePrice.SinglePrice) and self.isSamePriceCurrency(other):
                return self.__class__(self.currencyType, self.currency, self.amount + other.amount)
            return EventShopBundlePrice.MultiPrice([self]) + other
        ...
        def getDiscountedPrice(self, discount):
            if not (isinstance(discount, Discount) and self.isSamePriceCurrency(discount.discountAmountPrice)):
                return None
            else:
                return self - discount.discountAmountPrice

        def isSamePriceCurrency(self, other):
            if isinstance(other, self.__class__) and self.currencyType == other.currencyType and self.currency == other.currency:
                return True
            return False
    ...
    class MultiPrice(object):
        ...
        def __add__(self, other):
            ...
            for subPrice in other.subPrices:
                key = (
                 subPrice.currencyType, subPrice.currency)
    ...
    def makePrice(cls, priceTuple):
        if priceTuple[0] == EventShop.PriceType.SINGLE.value:
            ...
            if priceTuple[0] == EventShop.PriceType.MULTI.value:
                return cls.MultiPrice(priceTuple[1])
            return
    ...
    def getDiscountPercent(cls, discountedPrice, originalPrice):
        if originalPrice is None:
            ...
            def priceToDict(price):
                if price.type == EventShop.PriceType.MULTI:
                    return {subPrice.currency:subPrice.amount for subPrice in price.subPrices}
