...
FORCED_ORDERS = (
 RTSOrder.FORCE_ATTACK_ENEMY, RTSOrder.FORCE_GO_TO_POSITION, RTSOrder.STOP)
...
class RTSSupply(object):
    ...
    FLAMER = 7
    ALL = (
     BARRICADES, BUNKER, AT_GUN, WATCH_TOWER, PILLBOX, MORTAR, FLAMER)
...
_OrderDataTuple = namedtuple('OrderData', ['order', 'manner', 'position',
 'isPositionModified', 'target', 'heading',
 'baseID', 'baseTeam', 'companions'])
...
class OrderData(_OrderDataTuple):
    ...
    class InvalidOrderData(SoftException):
        ...
        def __init__(self, fieldName, valueGiven, valueExpected):
            log = 'OrderData type check failure: %s = %s (expected %s)' % (
             fieldName, valueGiven, valueExpected)
...
class OwnVehicleCommanderData(AnyVehicleCommanderData):
    __slots__ = ('health', 'maxHealth', 'orderData', 'currentPath', 'isSpeedLinked',
                 'wasSpotted', 'groupID')
