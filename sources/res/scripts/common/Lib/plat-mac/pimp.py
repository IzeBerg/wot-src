...
import sys, os, subprocess, urllib, urllib2, urlparse, plistlib, distutils.util, distutils.sysconfig, hashlib, tarfile, tempfile, shutil, time
__all__ = [
...
class PimpPreferences():
    ...
    def check(self):
        ...
        if not os.path.exists(self.buildDir):
            ...
            if not os.access(self.buildDir, RWX_OK):
...
class PimpDatabase():
    ...
    def dump(self, pathOrFile):
        ...
        plistdata = {'Version': self._version, 'Maintainer': self._maintainer, 
...
class PimpPackage():
    ...
    def prerequisites(self):
        ...
        if not self._dict.get('Download-URL'):
            ...
            return [
             (None,
              'Package %s cannot be installed automatically, see the description' % self.fullname())]
        else:
            if self.systemwideOnly() and self._db.preferences.isUserInstall():
                return [
                 (None,
                  'Package %s can only be installed system-wide' % self.fullname())]
