...
import MacOS, EasyDialogs, os, string, sys, types, StringIO, keyword, macresource, aetools, distutils.sysconfig, OSATerminology
from Carbon.Res import *
import Carbon.Folder, MacOS, getopt, plistlib
...
def simplify(item):
    if type(item) is types.ListType:
        ...
        if type(item) == types.TupleType and len(item) == 2:
...
class SuiteCompiler():
    ...
    def compilesuite(self, major, minor, language, script, fname, precompinfo):
        ...
        if self.basepackage and code in self.basepackage._code_to_module:
            ...
            if self.basepackage and code.lower() in self.basepackage._code_to_module:
...
class CodeNameMapper():
    ...
    def __init__(self, interact=1, verbose=None):
        self.code2name = {'property': {}, 'class': {}, 'enum': {}, 'comparison': {}}
        self.name2code = {'property': {}, 'class': {}, 'enum': {}, 'comparison': {}}
