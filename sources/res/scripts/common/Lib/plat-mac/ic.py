...
import icglue, string, sys, os
from Carbon import Res
import Carbon.File, macostools
error = icglue.error
...
_decoder_table = {'ArchieAll': (
               _decode_multistr, _code_multistr), 
   'UMichAll': (
              _decode_multistr, _code_multistr), 
   'InfoMacAll': (
                _decode_multistr, _code_multistr), 
   'ListFont': (
              _decode_fontrecord, _code_fontrecord), 
   'ScreenFont': (
                _decode_fontrecord, _code_fontrecord), 
   'PrinterFont': (
                 _decode_fontrecord, _code_fontrecord), 
   'Signature': (
               _decode_text, _code_text), 
   'Plan': (
          _decode_text, _code_text), 
   'MailHeaders': (
                 _decode_text, _code_text), 
   'NewsHeaders': (
                 _decode_text, _code_text), 
   'CharacterSet': (
                  _decode_charset, _code_charset), 
   b'Helper\xa5': (
                 _decode_appspec, _code_appspec), 
   'NewMailFlashIcon': (
                      _decode_boolean, _code_boolean), 
   'NewMailDialog': (
                   _decode_boolean, _code_boolean), 
   'NewMailPlaySound': (
                      _decode_boolean, _code_boolean), 
...
def _decode(data, key):
    if b'\xa5' in key:
        key2 = key[:string.index(key, b'\xa5') + 1]
    else:
        key2 = key
