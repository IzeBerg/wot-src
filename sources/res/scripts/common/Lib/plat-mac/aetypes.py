...
class Comparison:
    ...
    def __aepack__(self):
        return pack({'obj1': self.obj1, 'relo': mkenum(self.relo), 
           'obj2': self.obj2}, 'cmpd')
...
class AEText:
    ...
    def __aepack__(self):
        return pack({keyAEScriptTag: self.script, keyAEStyles: self.style, keyAEText: self.text}, typeAEText)
...
class ObjectSpecifier:
    ...
    def __aepack__(self):
        return pack({'want': mktype(self.want), 'form': mkenum(self.form), 
           'seld': self.seld, 
           'from': self.fr}, 'obj ')
