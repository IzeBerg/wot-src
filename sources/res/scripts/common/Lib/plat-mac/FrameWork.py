...
class Window():
    ...
    growlimit = (50, 50, screenbounds[2] - screenbounds[0], screenbounds[3] - screenbounds[1])

    def do_resize(self, width, height, window):
        l, t, r, b = self.wid.GetWindowPort().GetPortBounds()
        self.SetPort()
        self.wid.InvalWindowRect((r - SCROLLBARWIDTH + 1, b - SCROLLBARWIDTH + 1, r, b))
...
class ScrolledWindow(ControlsWindow):
    ...
    def do_postresize(self, width, height, window):
        ...
        if self.barx:
            self.barx.ShowControl()
            self.wid.ValidWindowRect((l, b - SCROLLBARWIDTH + 1, r - SCROLLBARWIDTH + 2, b))
        if self.bary:
            self.bary.ShowControl()
            self.wid.ValidWindowRect((r - SCROLLBARWIDTH + 1, t, r, b - SCROLLBARWIDTH + 2))
        self.wid.InvalWindowRect((r - SCROLLBARWIDTH + 1, b - SCROLLBARWIDTH + 1, r, b))
