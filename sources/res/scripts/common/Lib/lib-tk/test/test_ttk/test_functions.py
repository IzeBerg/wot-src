...
class InternalFunctionsTest(unittest.TestCase):
    ...
    def test_format_optdict(self):
        ...
        check_against(ttk._format_optdict({'test': [1, -1, '', '2m', 0], 'test2': 3, 'test3': '', 
           'test4': 'abc def', 'test5': '"abc"', 
           'test6': '{}', 'test7': '} -spam {'}, script=True), {'-test': '{1 -1 {} 2m 0}', '-test2': '3', '-test3': '{}', 
           '-test4': '{abc def}', '-test5': '{"abc"}', 
           '-test6': '\\{\\}', '-test7': '\\}\\ -spam\\ \\{'})
    ...
    def test_format_elemcreate(self):
        ...
        self.assertEqual(ttk._format_elemcreate('image', False, 'test', ('a', 'b',
                                                                         'c')), ('test {a b} c', ()))
        res = ttk._format_elemcreate('image', False, 'test', ('a', 'b'), a='x', b='y')
        self.assertEqual(res[0], 'test a b')
        self.assertEqual(set(res[1]), {'-a', 'x', '-b', 'y'})
        self.assertEqual(ttk._format_elemcreate('image', True, 'test', ('a', 'b', 'c',
                                                                        'd'), x=[2, 3]), ('{test {a b c} d}',
                                                                                          '-x {2 3}'))
