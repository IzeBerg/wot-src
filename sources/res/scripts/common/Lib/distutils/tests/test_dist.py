...
class DistributionTestCase(support.TempdirManager, support.LoggingSilencer, support.EnvironGuard, unittest.TestCase):
    ...
    def test_write_pkg_file(self):
        ...
        klass = Distribution
        dist = klass(attrs={'author': 'Mister Café', 'name': 'my.package', 
           'maintainer': 'Café Junior', 
           'description': 'Café torréfié', 
           'long_description': 'Héhéhé'})
        dist.metadata.write_pkg_file(open(my_file, 'w'))
        dist = klass(attrs={'author': 'Mister Cafe', 'name': 'my.package', 
    ...
    def test_empty_options(self):
        ...
        warnings.warn = _warn
        dist = Distribution(attrs={'author': 'xxx', 'name': 'xxx', 'version': 'xxx', 
           'url': 'xxxx', 'options': {}})
...
class MetadataTestCase(support.TempdirManager, support.EnvironGuard, unittest.TestCase):
    ...
    def test_provides_illegal(self):
        self.assertRaises(ValueError, Distribution, {'name': 'package', 'version': '1.0', 
           'provides': [
                      'my.pkg (splat)']})
