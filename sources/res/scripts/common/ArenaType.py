...
def _readVisualScript(section):
    if section.has_key(VisualScriptTag):
        ...
        return {ASPECT.CLIENT: _readVisualScriptAspect(vseSection, ASPECT.CLIENT.lower(), []), ASPECT.SERVER: _readVisualScriptAspect(vseSection, ASPECT.SERVER.lower(), [])}
...
class EnvSwitcherSettings(object):
    ...
    class EnvSetting(object):
        ...
        def readFader(section):
            ...
            for name, value in section.items():
                if name == 'fader':
                    fader = {'duration': value.readFloat('duration'), 'color': value.readVector3('color')}
    ...
    def getSwitcherByID(self, id):
        if id in self.switchers:
            return self.switchers[id]

    def getEnvSettingByID(self, name):
        if name in self.envSwitcherSettings:
            return self.envSwitcherSettings[name]