import random, items, items.customizations as vc
...
def _selectItemByQuality(items, quality):
    ...
    if itemsCount == 0:
        ...
        if itemsCount == 1 or quality == VEHICLE_PRESET_QUALITY.MIN_QUALITY:
            return items[0]
        if itemsCount == 2:
            return items[1]
...
def _prepareGunLists(vehDescr):
    ...
    return (
     allGunsList, thisAndLowerTurretsGunsList)
...
def _fillGunWithAmmo(defaultAmmoForGun, defaultMaxAmmo, vehiclePresetData, vehicles):
    ...
    currentAmmoForGun.extend([0, 0, 0, 0, 0, 0])
    ...
    return (currentAmmoForGun,
     {TankSetups.EQUIPMENT: [currentAmmoForGun[len(shells):]], TankSetups.SHELLS: [
                          shells], 
        TankSetups.BATTLE_BOOSTERS: [[]], TankSetups.OPTIONAL_DEVICES: [[]]}, {TankSetupGroupsId.EQUIPMENT_AND_SHELLS: 0, TankSetupGroupsId.OPTIONAL_DEVICES_AND_BOOSTERS: 0})
...
def _fillOptionalDevices(vehDescr, vehicles, vehiclePresetData):
    ...
    return (
     vehDescr, [ device.compactDescr for device in installedDevices ])
...
def _isCamoFitsArena(camo, season):
    if not camo.season == season:
        return False
    if camo.customizationDisplayType != CustomizationDisplayType.HISTORICAL:
        return False
    return True
