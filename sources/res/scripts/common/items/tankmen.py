...
class TankmanDescr(object):
    ...
    def validateLearningFreeSkill(self, skillName):
        ...
        if skillName not in COMMON_SKILLS and skillName not in SKILLS_BY_ROLES[self.role]:
            return (False, 'Cannot learn free skill (%s) for this tankman' % skillName, False)
        return (True, '', skillName in self.earnedSkills)
    ...
    def gid(self):
        if self.__gid is None:
            ...
            elif g and self.iconID not in g.icons and self.nationID == 5 and self.iconID in (3001,
                                                                                             3002,
                                                                                             3003,
                                                                                             3004):
                self.__gid = g.groupID
...
class TankmanGroupSelection(NoneGroupSelection):
    ANY = ('', '*')
    PTYPE = {'premium': True, 'normal': False}
    ...
    def fromString(tstr):
        ...
        try:
            ...
            ns = {n for n in ns.split('|') if n in nations.NAMES if n in nations.NAMES} if ns not in TankmanGroupSelection.ANY else ()
