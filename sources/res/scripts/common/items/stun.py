...
def readStunType(config, xmlCtx, section, default=None):
    ...
    factors = ('stunFactorEnginePower', 'stunFactorVehicleRotationSpeed', 'stunFactorTurretTraverse',
               'stunFactorViewDistance', 'stunFactorMaxSpeed')
    for factor in factors:
        stunType[factor] = _xml.readFraction(xmlCtx, section, factor, 0.0 if default is None else default[factor])

    factors = ('stunFactorReloadTime', 'stunFactorAimingTime', 'stunFactorVehicleMovementShotDispersion',
               'stunFactorVehicleRotationShotDispersion', 'stunFactorTurretRotationShotDispersion',
               'stunFactorMinShotDispersion')
    for factor in factors:
