import math, random, BigWorld, Math, ResMgr, WWISE, AnimationSequence
...
def loadLootById(radius, callback, typeID):
    ...
    if itemDesc is not None:
        return _loadLoot(typeID, radius, callback, itemDesc)
    else:
        return
...
def loadLootByName(radius, callback, typeName):
    ...
    if itemDesc is not None:
        return _loadLoot(0, radius, callback, itemDesc)
    else:
        return
