...
class CGFGameObjectContext(VScriptContext):
    ...
    @vse_get_property(SLOT_TYPE.GAME_OBJECT, display_name='Self', description='Returns current context GameObject', aspects=[
     ASPECT.CLIENT, ASPECT.HANGAR])
    def getSelf(self):
        return weakref.proxy(self.gameObject)

    @vse_event_out((), display_name='OnClick', description='Reacts on click on game object                      (only if go have CollisionComponent, OnClickComponent, VSEComponent)', aspects=[
     ASPECT.CLIENT, ASPECT.HANGAR])
    def onGameObjectClick(self):
        pass

    @vse_event_out((), display_name='OnHoverIn', description='Reacts on hover over game object                      (only if go have CollisionComponent, VSEComponent)', aspects=[
     ASPECT.CLIENT, ASPECT.HANGAR])
    def onGameObjectHoverIn(self):
        pass

    @vse_event_out((), display_name='OnHoverOut', description='Reacts on exit from hover over game object                      (only if go have CollisionComponent, VSEComponent)', aspects=[
     ASPECT.CLIENT, ASPECT.HANGAR])
    def onGameObjectHoverOut(self):
        pass

    @vse_event_out(SLOT_TYPE.STR, display_name='OnTrigger', description='Custom triggered event', aspects=[
     ASPECT.CLIENT, ASPECT.HANGAR])
