...
class RTSBootcampPlayerInputControl(Block, PlayerMeta):
    ...
    def validate(self):
        ...
        if not self.__getCorrectValue(rightMouse) and self.__getCorrectValue(rightDouble):
            return "Right Mouse Doubleclick won't work if Right Mouse is disabled!"
        return super(RTSBootcampPlayerInputControl, self).validate()
    ...
    def __addInputSlot(self, name, process):
        self.inputs[name] = (
         self._makeDataInputSlot(name, SLOT_TYPE.BOOL), process)
