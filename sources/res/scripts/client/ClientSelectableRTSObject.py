...
_TANKER_FORCED_ALIASES = [
 VIEW_ALIAS.TRADE_IN_VEHICLE_PREVIEW, VIEW_ALIAS.VEHICLE_PREVIEW,
 VIEW_ALIAS.WOT_PLUS_VEHICLE_PREVIEW, VIEW_ALIAS.MARATHON_VEHICLE_PREVIEW,
 VIEW_ALIAS.CONFIGURABLE_VEHICLE_PREVIEW, VIEW_ALIAS.OFFER_GIFT_VEHICLE_PREVIEW,
 VIEW_ALIAS.STYLE_PROGRESSION_PREVIEW, VIEW_ALIAS.STYLE_PREVIEW,
...
class ClientSelectableRTSObject(ClientSelectableCameraObject):
    ...
    def __getRequiredCameraLocation(self):
        ...
        if battleMode == ARENA_BONUS_TYPE.RTS_BOOTCAMP:
            ...
            if battleMode == ARENA_BONUS_TYPE.RTS:
                if self.__rtsController.isCommander():
                    return RTS_CAMERA_LOCATION.STRATEGIST_1V7
                return RTS_CAMERA_LOCATION.TANKER
            if battleMode == ARENA_BONUS_TYPE.RTS_1x1:
                return RTS_CAMERA_LOCATION.STRATEGIST_1V1
            return
    ...
    def __setIdleAndParalaxMovement(self, isEnabled):
        g_eventBus.handleEvent(CameraRelatedEvents(CameraRelatedEvents.FORCE_DISABLE_IDLE_PARALAX_MOVEMENT, ctx={'isDisable': not isEnabled, 'setIdle': True, 'setParallax': True}), EVENT_BUS_SCOPE.LOBBY)
