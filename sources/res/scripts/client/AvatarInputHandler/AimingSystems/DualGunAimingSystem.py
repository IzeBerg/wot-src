...
import BigWorld, GUI, math_utils
...
class GunTransitionInterpolator(object):
    __slots__ = ('__enabled', '__initialState', '__finalState', '__totalTime', '__elapsedTime',
                 '__prevTime', '__shadowEffect')
...
class SingleGunAimingSystem(SniperAimingSystem):
    ...
    def _getTurretYawGunPitch(self, targetPos, compensateGravity=False):
        ...
        gravity = (compensateGravity or self._vehicleTypeDescriptor.shot).gravity if 1 else 0.0
...
class DualGunAimingSystem(IAimingSystem):
    ...
    def __init__(self):
        ...
        self._aim = (
         SingleGunAimingSystem(DUAL_GUN.ACTIVE_GUN.LEFT),
         SingleGunAimingSystem(DUAL_GUN.ACTIVE_GUN.RIGHT))
