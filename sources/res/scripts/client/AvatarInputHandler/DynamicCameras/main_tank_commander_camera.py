import time, typing, weakref
from functools import wraps
import math, Keys, BigWorld, Math, GUI, math_utils
...
class CommanderCameraStateNormal(CommanderCameraState):
    ...
    def __init__(self, commanderCamera):
        ...
        self.activeControllers = [commanderCamera.horizontalMoveController, commanderCamera.rotationController,
         commanderCamera.zoomController]
...
class CommanderCamera(ICamera, CallbackDelayer, TimeDeltaMeter):
    ...
    def _setupCamera(self, position, rotation):
        ...
        extendedBoundingBox = (self._levelBoundingBox[0] - Math.Vector2(boundingBoxExtension, boundingBoxExtension),
         self._levelBoundingBox[1] + Math.Vector2(boundingBoxExtension, boundingBoxExtension))
...
class HorizontalMoveController(BaseMoveController):
    ...
    def __divideWrapper(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if isAnyKeysDown(Keys.KEY_LSHIFT, Keys.KEY_RSHIFT, Keys.MODIFIER_SHIFT):
                return res / 2
            return res
...
class RotationController(BaseMoveController):
    ...
    def init(self, initialRotation):
        ...
        restrictions = (
         angleToRadian(self.config['max_mouse_camera_pitch']),
         angleToRadian(self.config['min_mouse_camera_pitch']))
...
def getNormilizedAngle(angle):
    ...
    if angle < 0:
        return _TWO_PI - abs(angle) % _TWO_PI
    return angle
