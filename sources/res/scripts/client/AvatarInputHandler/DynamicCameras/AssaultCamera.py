...
class AssaultCamera(CameraWithSettings, CallbackDelayer):
    ...
    def __getCollsionDist(self, startPos):
        ...
        collision = collideDynamicAndStatic(startPos, endPoint, (
    ...
    def __getDistInterpolationParam(self, collisionDist, deltaTime):
        ...
        if collisionDist < self.__curDistRange[1]:
            return math_utils.clamp(0.0, 1.0, self._cfg['rDefaultInterpolationSpeed'] * deltaTime)
        return math_utils.clamp(0.0, 1.0, self._cfg['rMaxInterpolationSpeed'] * deltaTime)
    ...
    def _readStatesCfg(self, dataSec):
        ...
        for index in range(0, bcfg['statesCount']):
            ...
            bcfg['states'][index] = {}
            bcfg['states'][index]['distRange'] = readVec2(dataSec, stateSectionName + subsectionStateName + 'distRange', (0.0,
                                                                                                                          0.0), (1000.0,
                                                                                                                                 1000.0), (0.0,
                                                                                                                                           1000.0))
