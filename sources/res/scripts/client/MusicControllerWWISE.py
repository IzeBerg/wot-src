...
class HW21BattleResultsStories(object):
    ...
    _NarrativeSoundInfo = namedtuple('_NarrativeSoundInfo', ('wasPlayed', 'eventId',
                                                             'storeKey', 'index'))
    ...
    def getSoundEvents(self, event, wwSetup, arenaUniqueID):
        ...
        if narrativeInfo is not None and narrativeInfo.difficulty != EVENT.INVALID_DIFFICULTY_LEVEL and narrativeInfo.environmentID != EVENT.INVALID_ENVIRONMENT_ID:
            ...
            if event == MUSIC_EVENT_COMBAT_VICTORY:
                ...
                soundEvents.append((isBossFight or HW21SoundConsts).HANGAR_TEAMFIGHT_LOSE if 1 else HW21SoundConsts.HANGAR_BOSSFIGHT_LOSE)
        return soundEvents
...
class MusicController(object):
    ...
    def __getArenaSoundEvents(self, eventId):
        if not self.__lastBattleArenaWwSetup and eventId in (
         MUSIC_EVENT_COMBAT_VICTORY, MUSIC_EVENT_COMBAT_LOSE, MUSIC_EVENT_COMBAT_DRAW):
            return []
        else:
            soundEvents = []
            if eventId == MUSIC_EVENT_COMBAT_VICTORY:
                soundEvents = [self.__lastBattleArenaWwSetup.get('wwmusicResultWin'),
                 self.__lastBattleArenaWwSetup.get('wwmusicVoResultWin')]
            else:
                if eventId == MUSIC_EVENT_COMBAT_LOSE:
                    soundEvents = [self.__lastBattleArenaWwSetup.get('wwmusicResultDefeat'),
                     self.__lastBattleArenaWwSetup.get('wwmusicVoResultDefeat')]
                elif eventId == MUSIC_EVENT_COMBAT_DRAW:
                    soundEvents = [self.__lastBattleArenaWwSetup.get('wwmusicResultDrawn'),
                     self.__lastBattleArenaWwSetup.get('wwmusicVoiceoverResultDrawn')]
                else:
                    ...
                    if eventId == MUSIC_EVENT_COMBAT_LOADING:
                        ...
                        if wwmusicSetup is not None:
                            soundEvents = [
                             wwmusicSetup.get('wwmusicLoading'), wwmusicSetup.get('wwmusicLoadingVoiceover')]
