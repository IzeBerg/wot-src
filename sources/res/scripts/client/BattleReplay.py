import base64, os, datetime, json, copy, cPickle as pickle, logging, Math, BigWorld, ArenaType, Settings, CommandMapping, constants, Keys, Event, AreaDestructibles, TriggersManager
...
class BattleReplay(object):
    ...
    def getArenaPeriod(self):
        ...
        if ret not in (constants.ARENA_PERIOD.IDLE, constants.ARENA_PERIOD.WAITING, constants.ARENA_PERIOD.PREBATTLE,
    ...
    def onClientReady(self):
        if not (self.isPlaying or self.isRecording):
            ...
            if self.isRecording:
                ...
                now = '%02d.%02d.%04d %02d:%02d:%02d' % (now.day, now.month, now.year, now.hour, now.minute, now.second)
