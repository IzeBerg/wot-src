...
class RandomCrewBooksWrapper(BonusAggregateWrapper):
    ...
    def getWrappedBonus(cls, bonus):
        return {'id': bonus.get('id', 0), 
           ...
           'icon': {AWARDS_SIZES.SMALL: sanitizeResPath(backport.image(cls._R_IMAGE.small.brochure_universal())), AWARDS_SIZES.BIG: sanitizeResPath(backport.image(cls._R_IMAGE.big.brochure_universal()))}, 
           'name': '', 
           'description': ''}
...
class RandomGuideWrapper(BonusAggregateWrapper):
    ...
    def getWrappedBonus(cls, bonus):
        return {'id': bonus.get('id', 0), 
           'type': ItemPackType.CREW_BOOK_RANDOM, 
           'value': bonus.get('value', 0), 
           'icon': {AWARDS_SIZES.SMALL: sanitizeResPath(backport.image(cls._R_IMAGE.small.guide_universal())), AWARDS_SIZES.BIG: sanitizeResPath(backport.image(cls._R_IMAGE.big.guide_universal()))}, 
           'name': '', 
           'description': ''}
...
class RandomNationalBlueprintWrapper(BonusAggregateWrapper):
    ...
    def getWrappedBonus(cls, bonus):
        return {'id': 0, 
           ...
           'value': bonus.get('value', 0), 
           'icon': {AWARDS_SIZES.SMALL: sanitizeResPath(backport.image(cls._R_IMAGE.small.randomNational())), AWARDS_SIZES.BIG: sanitizeResPath(backport.image(cls._R_IMAGE.big.randomNational()))}, 
           'name': '', 
           'description': ''}
...
_BONUS_FOR_AGGREGATE = (
 ItemPackType.CREW_BOOK_BROCHURE, ItemPackType.BLUEPRINT_NATIONAL,
 ItemPackType.CUSTOM_COLLECTION_ENTITLEMENT, ItemPackType.CREW_BOOK_GUIDE)
    ...
    def __parseBoxInfo(self, boxInfo):
        ...
        if 'slots' in boxInfo:
            for idx, slotData in boxInfo['slots'].iteritems():
                result.update({idx: {'probability': round(slotData.get('probability', [[0]])[0][0] * 100, 2), 'bonuses': []}})
                ...
                for bonus in slotData.get('bonuses', []):
                    ...
                    for bonusEntry in bonusList:
                        ...
                        elif not self.__isExistingVehicle(bonusEntry, result[idx]['bonuses']):
