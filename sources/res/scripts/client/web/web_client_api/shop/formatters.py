...
def _formatVehicleInfo(crewItem):
    return {vType:{'id': descr.type.compactDescr, 'type': first(t for t in VEHICLE_CLASS_TAGS if t in descr.type.tags), 'name': descr.type.userString, 'nation': nations.MAP[descr.type.id[0]]} for vType, descr in (
     (
