    ...
    def __tryNotify(self):
        if not self.__adventCalendarV2Ctrl.isAvailable():
            ...
            if availableDoorsAmount:
                ...
                data.update({'state': state.value, 
                   'template': template})
                self._notifyOrRemove(True, ctx={'template': template, 'linkageData': data})
            else:
                self._notifyOrRemove(False)
            return
...
class EventLootBoxesListener(_NotificationListener, EventsHandler):
    ...
    def _getEvents(self):
        return (
         (
          self.__eventLootBoxes.onIntroShownChanged, self.__onIntroShownChanged),
         (
...
registerNotificationsListeners((
 ...
 BattlePassSwitchChapterReminder, ResourceWellListener, IntegratedAuctionListener, SeniorityAwardsStateListener,
 SeniorityAwardsQuestListener, SeniorityAwardsTokenListener, EventLootBoxesListener, CollectionsListener,
 WinbackSelectableRewardReminder, WotPlusIntroViewListener, BattleMattersTaskReminderListener,
 PrestigeListener, SeniorityAwardsVehicleSelectionListener,
 PersonalReservesConversionListener, NyDogReminderListener, NyMarketplaceAvailableListener,
 LootBoxConfigListener, NyResourceReminderListener, AdventCalendarDoorsAvailableListener,
 NyFriendResourceReminderListener, NyNoFriendResourceReminderListener))
