...
class NewYearTutorialController(INewYearTutorialController):
    ...
    def __updateState(self):
        ...
        if not self._isNewYearSpace():
            ...
            if self._nyController.getHangarNameMask() is None:
                self.startTutorial()
                return
    ...
    def __getCameraEvents(self):
        ...
        cameraManager = self._cameraManager
        if cameraManager:
            events += ((cameraManager.onCameraSwitched, self.__onCameraSwitched),
             (
              self._cameraManager.onCameraSwitchCancel, self.__onCameraSwitchCancel))
        return events
