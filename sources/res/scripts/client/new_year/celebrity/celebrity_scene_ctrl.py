...
class CelebritySceneController(ICelebritySceneController):
    __slots__ = ('__eventsManager', '__celebrityEntity', '__questGroups', '__quests',
                 '__isInChallengeView', '__tokens', '__marathonQuests', '__completedQuestsMask',
                 '__questsCount', '__completedQuestsCount')
    ...
    def __setAnimationTrigger(self, triggerName):
        if not self.__hangarSpace.spaceInited:
            ...
            if self.__celebrityEntity is None:
