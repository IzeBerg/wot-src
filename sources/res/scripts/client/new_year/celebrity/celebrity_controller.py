    ...
    def doActionByCelebActionToken(self, tokenID):
        if tokenID is None:
            ...
            if guestName is None:
                return
            if level is not None and not self.__nyController.isTokenReceived(tokenID):
                return
            if actionType in (GuestQuestTokenActionType.ANIM, GuestQuestTokenActionType.DECORATION):
                self.onDoActionByCelebActionToken(guestName, actionType, level)
                return
            if actionType == GuestQuestTokenActionType.STORY:
                self.onDoActionByCelebActionToken(guestName, actionType, level)
                return
            return
