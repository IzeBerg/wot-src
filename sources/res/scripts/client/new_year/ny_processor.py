...
class NewYearConvertResourcesProcessor(Processor):
    ...
    def __init__(self, fromResourceType, fromValue, toResourceType, toValue):
        confirmators = [
         plugins.AsyncDialogConfirmator(showResourcesConvertDialog, fromResourceType, fromValue, toResourceType, toValue)]
...
class RerollCelebrityQuestProcessor(Processor):
    ...
    def __init__(self, questID):
        ...
        self.addPlugins([
         _CelebrityQuestLockedByVehicleValidator()])
...
class BuyMarketplaceItemProcessor(_NyBuyProcessor):
    ...
    def __init__(self, item, categoryID, itemID, resourceID, dialog, parent=None):
        confirmators = [
         ...
         CollectionBoughtValidator(item)]
...
class BuyToyProcessor(Processor):
    ...
    def __init__(self, toyID, slotID, parent=None):
        confirmators = [
         plugins.AsyncDialogConfirmator(showCustomizationBuyDialog, parent=parent, toyID=toyID)]
...
class _CelebrityQuestLockedByVehicleValidator(SyncValidator):
    ...
    def _validate(self):
        if isCelebrityQuestsRerollLockedByVehicle():
...
class CollectionBoughtValidator(SyncValidator):
    ...
    def _validate(self):
        if isCollectionItemReceived(self._item):
            return plugins.makeError('Collection already bought')
        return plugins.makeSuccess()
...
class BuyItemValidator(SyncValidator):
    ...
    def _validate(self, nyController=None, itemsCache=None):
        ...
        if priceWithDiscount > balance:
            return plugins.makeError('not_enough_resource')
        return plugins.makeSuccess()
...
class ResourceTypeValidator(SyncValidator):
    ...
    def _validate(self):
        if self.__resourceID in NyCurrency.ALL:
            return plugins.makeSuccess()
        return plugins.makeError('invalid resource type')
...
class BuyCoinsCountValidator(SyncValidator):
    ...
    def _validate(self):
        if self.__amount > 0:
            return plugins.makeSuccess()
        return plugins.makeError('invalid coins count')
...
class ResourceEnoughValidator(SyncValidator):
    ...
    def _validate(self, nyController=None):
        ...
        if balance >= self.__price:
            return plugins.makeSuccess()
        return plugins.makeError('Insufficient amount')