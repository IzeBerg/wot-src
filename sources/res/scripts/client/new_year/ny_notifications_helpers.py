...
class LootBoxNotificationHelper(Notifiable):
    ...
    def __notify(self):
        if not self.__nyController.isEnabled():
            return
        tokenID, value = findFirst(lambda data: data[1] > 0, sorted(self.__delayedNotifications.iteritems(), key=itemgetter(0)), ('',
                                                                                                                                  0))
    ...
    def __pushNewLootBoxesNotification(self, tokenID, count):
        ...
        if lootBox is None:
            ...
            self.__systemMessages.proto.serviceChannel.pushClientMessage({'data': {'category': category, 'count': count}, 'template': NyGFNotificationTemplates.NY_NEW_REWARD_KIT}, SCH_CLIENT_MSG_TYPE.NY_GF_SM_TYPE)
            return
...
def sendNYResourceCollectingAvailableMessage(resourcesCount, isExtra=False, withCoolDown=None, systemMessages=None):
    if __coolDowner(withCoolDown, LAST_SEEN_COLLECTING_NOTIFY_TIME, getNYResourceCollectingConfig().getCollectingNotifyTimeout()):
        return
    systemMessages.proto.serviceChannel.pushClientMessage({'data': {'resourceCount': resourcesCount, 'isExtra': isExtra, 'viewType': 'Personal'}, 'template': NyGFNotificationTemplates.NY_RESOURCES_REMINDER}, msgType=SCH_CLIENT_MSG_TYPE.NY_GF_SM_TYPE)
...
def sendNYFriendResourceCollectingAvailableMessage(resourcesCount, friendName, friendSpaID, withCoolDown=None, systemMessages=None):
    if __coolDowner(withCoolDown, LAST_SEEN_FRIENDS_NOTIFY_TIME, getNYResourceCollectingConfig().getCollectingNotifyTimeout()):
        return
    systemMessages.proto.serviceChannel.pushClientMessage({'data': {'resourceCount': resourcesCount, 'friendName': friendName, 'friendID': friendSpaID, 'viewType': 'Friends'}, 
...
def sendNYNoFriendsAvailableMessage(resourcesCount, withCoolDown=None, systemMessages=None):
    if __coolDowner(withCoolDown, LAST_SEEN_NO_FRIENDS_NOTIFY_TIME, getNYResourceCollectingConfig().getNoFriendsNotifyTimeout()):
        return
    systemMessages.proto.serviceChannel.pushClientMessage({'data': {'resourceCount': resourcesCount, 'viewType': 'FindFriends'}, 'template': NyGFNotificationTemplates.NY_RESOURCES_REMINDER}, msgType=SCH_CLIENT_MSG_TYPE.NY_GF_SM_TYPE)