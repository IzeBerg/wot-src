...
class NewYearController(INewYearController, IGlobalListener):
    ...
    def getActiveMultiplier(self):
        if self.isPostEvent():
            return BonusHelper.getCommonPostEventAtmosphereMultiplier()
        return BonusHelper.getAtmosphereMultiplier()

    def getActiveCollectionsBonus(self):
        if self.isPostEvent():
            return BonusHelper.getCommonPostEventCollectionsBonus()
        return BonusHelper.getCommonCollectionsFactor()
    ...
    def __showSystemMessage(self, newState):
        ...
        if msg is not None:
            if msg.type == SystemMessages.SM_TYPE.NewYearEventStarted:
                auxData = [
                 msg.type, msg.priority, None, None]
