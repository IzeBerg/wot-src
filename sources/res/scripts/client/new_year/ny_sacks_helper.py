        ...
        if result:
            ...
            if result.success:
                ...
                if self.__hasRareReward(rewardList[0]):
                    self.__systemMessages.proto.serviceChannel.pushClientMessage({'data': rewardList[0].get('customizations')[0], 'template': NyGFNotificationTemplates.NY_SACKS_RARE_LOOT}, SCH_CLIENT_MSG_TYPE.NY_GF_SM_TYPE)
                self.onSackOpened(rewardList[0])
        self.__onOpenProcess = False

    def __hasRareReward(self, data):
        if data.get('customizations'):
            return data.get('customizations')[0].get('custType') == 'style'
