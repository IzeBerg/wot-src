...
import math_utils, BigWorld, Math, CGF
...
class _CameraParams(object):
    __slots__ = ('name', 'initMatrix', 'targetPos', 'sensitivity', 'yawConstraints',
                 'pitchConstraints', 'distConstraints', 'fluency', 'fov', 'dofParams',
                 'flightPoints')
...
class CustomizationCamera(CallbackDelayer):
    ...
    def __readCameraParamsFromGameObject(self, gameObject):
        ...
        if dofComponent:
            self.__currentParams.dofParams = (dofComponent.nearStart, dofComponent.nearDist, dofComponent.farStart, dofComponent.farDist)
        else:
