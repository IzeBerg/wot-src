...
class _NewYearToy(NewYearCurrentToyInfo):
    __slots__ = ('__totalCount', '__slotId', '__unseenCount', '__unseenInCollection',
                 '__slotsCount')
...
class FriendNewYearRequester(NewYearRequester):
    ...
    def getToys(self):
        return {slotId:{toyId: _NewYearToy(toyId, slotId, 1, 0, 0)} for slotId, toyId in enumerate(self.getSlotsData()) if toyId != INVALID_TOY_ID if toyId != INVALID_TOY_ID}
