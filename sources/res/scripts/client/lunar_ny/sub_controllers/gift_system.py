...
class LunarNYGiftSystemSubController(IBaseLunarSubController):
    ...
    def getEnvelopesEntitlementCountByType(self, envelopeType):
        if self.__giftEventHub is None:
            return 0
        else:
            return self.__giftEventHub.getStamper().getStampCount(ENVELOPE_TYPE_TO_ENTITLEMENT_CODE[envelopeType])

    def getSecretSantaSentPeriodLimit(self):
        if self.__giftEventHub is not None:
            return self.__giftEventHub.getKeeper().getSentPeriodLimit()
        else:
            return 0
    ...
    def envelopeInSecretSantaPoll(self, envelopeType):
        if self.__giftEventHub is None:
            ...
            return self.__giftEventHub.getStamper().getStampCount(ENVELOPE_IN_SECRET_SANTA_ENTITLEMENTS[envelopeType]) > 0
