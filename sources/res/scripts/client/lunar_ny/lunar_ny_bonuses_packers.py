...
class CustomizationLunarNYBonusPacker(SimpleLunarNYBonusPacker):
    ...
    def _getContentId(cls, bonus):
        return [
         BACKPORT_TOOLTIP_CONTENT_ID] * len([ b for b in bonus.getCustomizations() if b is not None ])
    ...
    def __getRewardType(cls, itemTypeName):
        if itemTypeName == 'projection_decal':
            return RewardType.CUSTOMIZATIONDECAL
...
class DossierLunarNYBonusPacker(SimpleLunarNYBonusPacker):
    ...
    def _getContentId(cls, bonus):
        return [
         BACKPORT_TOOLTIP_CONTENT_ID] * (len(bonus.getBadges()) + len(bonus.getAchievements()))
...
class DossierLunarNYProgressionAwardPacker(DossierBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        ...
        for achievement in bonus.getAchievements():
            tooltipData.append(TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.LUNAR_NY_PROGRESSION_ACHIEVS, specialArgs=[
...
class EnvelopesLunarNYBonusPacker(SimpleLunarNYBonusPacker):
    ...
    def _getToolTip(cls, bonus):
        ...
        if value.id in ENVELOPE_ENTITLEMENT_CODE_TO_TYPE:
            return [
             TooltipData(tooltip=None, isSpecial=False, specialAlias=None, specialArgs=[ENVELOPE_ENTITLEMENT_CODE_TO_TYPE[value.id]])]
        else:
            return
...
def _progressionBonusesKeyOrder(bonus):
    if bonus.getName() in _PROGRESSION_BONUSES_ORDER:
        return _PROGRESSION_BONUSES_ORDER.index(bonus.getName())
    return len(_PROGRESSION_BONUSES_ORDER)
