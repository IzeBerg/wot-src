...
class NearByVehicleIndicator(PveBossIndicatorProgressMeta, GameEventGetterMixin):
    ...
    def _isEnabled(self):
        ...
        if self.environmentData.getCurrentEnvironmentID() == envsList[0]['id'] and currentDifficultyLevel == DifficultyLevelItem.EASY:
            return False
        return True
    ...
    def _getIndicatorValue(self):
        if self.nearbyIndicatorData is None:
            return 0
        else:
            return self.nearbyIndicatorData.getIndicatorValue(str(self.__currentVehicleID))
