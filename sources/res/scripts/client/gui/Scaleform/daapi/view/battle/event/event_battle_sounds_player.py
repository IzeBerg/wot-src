...
import WWISE, BigWorld, SoundGroups, TriggersManager
...
class TimerSoundPlayer(_SoundsPlayer):
    ...
    def __onUpdateScenarioTimer(self, waitTime, *args):
        if waitTime == self.EVENT_TIMER_NOTIFICATION_TIME and self.currentSoundEnvID in self.EVENT_TIMER_SOUND_NOTIFICATION_WORLDS and self.currentSoundEnvID != self.__lastTimerNotificationEnvId:
            self.__lastTimerNotificationEnvId = (
             self.currentSoundEnvID, self.currentBossFightPhase)
...
class MatterCollectingSoundPlayer(_SoundsPlayer):
    ...
    FILL_NOTIFICATIONS_BY_ENV_ID = {0: [
         (1.0, 'hw21_vo_notification_collector_fill_100')], 
       1: [
         (0.5, 'hw21_vo_notification_collector_fill_50'), (1.0, 'hw21_vo_notification_collector_fill_100')], 
       2: [
         (0.5, 'hw21_vo_notification_collector_fill_50'),
         (0.75, 'hw21_vo_notification_collector_fill_75'),
         (1.0, 'hw21_vo_notification_collector_fill_100')]}
...
class TeamFightSoundPlayer(_SoundsPlayer):
    ...
    def _onEnvironmentEventIDUpdate(self, eventEnvID):
        ...
        if self.isCorrectSoundEnvironment:
            ...
            self.__alliesRemained = {vInfo.vehicleID for vInfo in arenaDP.getVehiclesInfoIterator() if vInfo.team == EVENT.PLAYERS_TEAM if vInfo.team == EVENT.PLAYERS_TEAM}
