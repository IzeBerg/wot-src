...
class Barracks(BarracksMeta, LobbySubView, IGlobalListener):
    ...
    def __getNoInfoData(self, totalCount=0, filteredCount=0):
        ...
        if hasNoInfoData:
            if self.filter['location'] == BARRACKS_CONSTANTS.LOCATION_FILTER_DISMISSED:
                ...
                noInfoData = {'title': text_styles.highTitle(MENU.BARRACKS_NONOTRECRUITEDTANKMEN_TITLE), 'message': text_styles.main(MENU.BARRACKS_NONOTRECRUITEDTANKMEN_MESSAGE)}
    ...
    def __setCountersData(self, preferenceName, counter):
        counters = [
         _packCounterVO(componentId=_COUNTERS_MAP[preferenceName][0], count=str(counter) if counter else '', selectedIdx=_COUNTERS_MAP[preferenceName][1])]
