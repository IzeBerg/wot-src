...
class EventDestroyTimersPanel(EventDestroyTimersPanelMeta):
    ...
    def __onCurrentTimerChange(self):
        ...
        elif timerId in (BATTLE_NOTIFICATIONS_TIMER_TYPES.HALF_OVERTURNED,
         BATTLE_NOTIFICATIONS_TIMER_TYPES.OVERTURNED):
    ...
    def __updateOverlays(self):
        if self.__currentCollectorState != SoulCollectorDestroyTimerState.NO_TIMER or self.__currentTimer:
            onCollectorProgress({'isRibbonsPanelOverlay': self.__isRibbonsPanelOverlay, 'isBuffsPanelOverlay': self.__isBuffsPanelOverlay})
        else:
