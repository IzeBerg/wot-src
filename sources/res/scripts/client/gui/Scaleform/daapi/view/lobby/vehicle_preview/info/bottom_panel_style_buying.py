...
def _separatePrice(price):
    return ({c:v for c, v in price.iteritems() if c in Currency.ALL if c in Currency.ALL}, {c:v for c, v in price.iteritems() if c in _DYN_CURRENCIES if c in _DYN_CURRENCIES})
