...
class WotAnniversaryQuestInfo(QuestPostBattleInfo):
    ...
    def _getBonuses(self, svrEvents, pCur=None, bonuses=None):
        if self.event.getID().startswith(WOT_ANNIVERSARY_ALL_MASCOT_BATTLE_QUESTS_PREFIX):
            return []
        return super(WotAnniversaryQuestInfo, self)._getBonuses(svrEvents, pCur, bonuses)
