...
import BigWorld, Math, BattleReplay, aih_constants
...
class PersonalEntriesPlugin(common.SimplePlugin):
    __slots__ = ('__isAlive', '__isObserver', '__isCommander', '__viewPointID', '__animationID',
                 '__deadPointID', '__cameraID', '__cameraIDs', '__yawLimits', '__circlesID',
                 '__circlesVisibilityState', '__killerVehicleID', '__defaultViewRangeCircleSize')
...
class ArenaVehiclesPlugin(common.EntriesPlugin, IVehiclesAndPositionsController):
    __slots__ = ('__isObserver', '__isCommander', '__aoiToFarCallbacksIDs', '__destroyCallbacksIDs',
                 '__flags', '__showDestroyEntries', '__isDestroyImmediately', '__destroyDuration',
                 '__isSPG', '__replayRegistrator', '__spottedSupplies', '__selectedVehicles')
    ...
    def __onMinimapFeedbackReceived(self, eventID, entityID, value):
        if eventID == FEEDBACK_EVENT_ID.MINIMAP_SHOW_MARKER and entityID != _getPlayerVehicleID() and entityID in self._entries:
