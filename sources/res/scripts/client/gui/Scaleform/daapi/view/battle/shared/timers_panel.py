...
class _StackTimersCollection(_BaseTimersCollection):
    __slots__ = ('_currentTimer', '_currentSecondaryTimers', '_priorityMap', '_prioritySecondaryMap',
                 '_maxDisplayedSecondaryTimers', 'onCurrentTimerChange')
    ...
    def getCurrentTimerId(self):
        if self._currentTimer is not None:
            return self._currentTimer.typeID
        else:
            return
...
class TimersPanel(TimersPanelMeta, MethodsRules):
    ...
    def _generateSecondaryTimersData(self):
        ...
        data = [
         self._getNotificationTimerData(_TIMER_STATES.STUN, BATTLE_NOTIFICATIONS_TIMER_LINKAGES.STUN_ICON, link, BATTLE_NOTIFICATIONS_TIMER_COLORS.ORANGE, True, False, INGAME_GUI.STUN_INDICATOR),
