...
class BCQuestsView(LobbySubView, MissionDetailsContainerViewMeta):
        ...
        missionData = {'title': text_styles.promoSubTitle(backport.text(R.strings.bootcamp.quest.title())), 
           'battleConditions': [
                              {'rendererLinkage': MISSIONS_ALIASES.BATTLE_CONDITION, 
                                 ...
                                 'isDetailed': True, 
                                 'data': [
                                        {'title': text_styles.promoSubTitle(backport.text(R.strings.bootcamp.quest.name())), 
           ...
           'awards': [
                    ...
                    {'label': text_styles.gold(self._GOLD_LABEL), 
                       'padding': self._AWARD_LABEL_PADDING, 
           ...
           'background': 'default', 
           'prebattleConditions': [
                                 formatters.packMissionPrebattleCondition(text_styles.main(bTypesData[0].iconLabel), ('').join([ iconData.icon for iconData in bTypesData ]), makeTooltip(QUESTS.MISSIONDETAILS_CONDITIONS_BATTLEBONUSTYPE, tooltipBody))], 
    ...
    def _populate(self):
        ...
        self.as_setInitDataS({'pages': [
                   {'buttonsGroup': self._PAGES_LINKAGE, 
                      'pageIndex': 0, 
                      'selected': True}]})