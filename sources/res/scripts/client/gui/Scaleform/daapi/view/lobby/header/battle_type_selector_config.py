...
def getPartialBattlePopoverToolTip(isDisable, tooltip, isSpecial, isWulf):
    return (
     tooltip, isSpecial, isWulf)


def getBattleStrongHoldTooltip(itemIsDisabled):
    return (
     (itemIsDisabled or TOOLTIPS).BATTLETYPES_STRONGHOLDS if 1 else TOOLTIPS.BATTLETYPES_STRONGHOLDS_DISABLED,
     False, False)
...
def getRankedAvailabilityData(_):
    ...
    return (
     TOOLTIPS_CONSTANTS.RANKED_SELECTOR_INFO if rankedController.isAvailable() else TOOLTIPS_CONSTANTS.RANKED_UNAVAILABLE_INFO, True, False)
