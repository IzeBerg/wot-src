...
class StrongholdDataReceiver(object):
    ...
    def getStatsVO(self):
        ...
        return [ {'local': key, 'value': DUMMY_UNAVAILABLE_DATA if elo is None else elo, 'timeExpired': True if elo is None else not actual, 'tooltip': tooltip, 'isHidden': False} for key, elo, actual, tooltip in rows if key not in excludes
               ]
