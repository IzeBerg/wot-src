...
class EpicBattlesBrowserView(LobbySubView, EpicBattlesBrowserViewMeta):
    ...
    def __loadBrowser(self, width, height):
        ...
        if epicBattlesUrl is not None:
            self.__browserId = yield self.browserCtrl.load(url=epicBattlesUrl, useBrowserWindow=False, browserSize=(
             width, height), showBrowserCallback=self.__showBrowser, handlers=webApiCollection(SoundWebApi))
