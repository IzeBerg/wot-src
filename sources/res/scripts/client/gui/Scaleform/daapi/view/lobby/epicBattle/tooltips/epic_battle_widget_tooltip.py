...
class EpicBattleWidgetTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self):
        ...
        if season is None:
            ...
            if self.__epicController.isCurrentCycleActive():
                blocks.extend([
                 self.__getCycleStatusTooltipPack(season),
    ...
    def __getPlayerProgressToLevelBlock(playerFamePts, famePtsToProgress):
        res = formatters.packBlockDataItem(linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_META_LEVEL_PROGRESS_BLOCK_LINKAGE, data={'progressBarData': {'value': playerFamePts, 'maxValue': famePtsToProgress}}, padding=formatters.packPadding(left=20), blockWidth=280)
        return res
