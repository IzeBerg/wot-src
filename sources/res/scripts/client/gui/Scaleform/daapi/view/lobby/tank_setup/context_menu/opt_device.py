...
class HangarOptDeviceSlotContextMenu(BaseHangarEquipmentSlotContextMenu):
    ...
    def _demountProcess(self, isDestroy=False):
        ...
        if result:
            self._sendLastSlotAction(TankSetupConstants.OPT_DEVICES, BaseSetupModel.DESTROY_SLOT_ACTION if isDestroy else BaseSetupModel.DEMOUNT_SLOT_ACTION, {'intCD': item.intCD, 'slotID': self._installedSlotId})
