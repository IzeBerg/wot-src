...
def makeUnitShortVO(unitEntity, unitMgrID=None, maxPlayerCount=MAX_PLAYER_COUNT_ALL):
    ...
    if fullData.unit.isEvent():
        ...
        if commanderInfo:
            squadDifficultyLevel = commanderInfo.player.extraData.get('eventEnqueueData', {}).get('difficultyLevel')
    return {'isFreezed': fullData.flags.isLocked(), 'hasRestrictions': fullData.unit.isRosterSet(ignored=settings.CREATOR_ROSTER_SLOT_INDEXES), 
...
def makeUnitVO(unitEntity, unitMgrID=None, maxPlayerCount=MAX_PLAYER_COUNT_ALL, withPrem=False):
    ...
    if fullData.unit.isEvent():
        ...
        if commanderInfo:
            squadDifficultyLevel = commanderInfo.player.extraData.get('eventEnqueueData', {}).get('difficultyLevel')
    return {'isCommander': isPlayerCreator, 'isFreezed': fullData.flags.isLocked(), 
...
def makeReserveSlotTooltipVO(groupType, header, description, empty, notChosen, havePermition, isInBattle, disabledByRequisition):
    ...
    return (
     makeTooltip(header, description), tooltipType)
