...
class MapboxProgressionTooltip(BlocksTooltipData):
    ...
    def __packMapsProgressionBlock(self, progressionData):
        ...
        for mapName, mapData in sortedProgressionData:
            counter = (mapData.passed or backport.text)(R.strings.mapbox.questFlag.counter(), progress=text_styles.bonusPreviewText(min(mapData.progress, mapData.total)), total=mapData.total) if 1 else ''
    ...
    def __packTotalProgressionBlock(self, progressionData):
        ...
        items.append(formatters.packBlockDataItem(linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_EPIC_PROGRESS_BLOCK_LINKAGE, data={'value': min(progress, total), 'maxValue': total, 'isGreen': progress >= total}, blockWidth=330, padding=formatters.packPadding(top=-11, bottom=8, left=20)))
        if progress < total:
