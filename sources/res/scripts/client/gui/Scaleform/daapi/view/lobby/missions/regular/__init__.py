...
class MissionsPackageBusinessHandler(PackageBusinessHandler):
    ...
    def __loadMissionsPageOrUpdateCurrentTab(self, event):
        ...
        if not subView:
            ...
            if subTab is not None and targetTab == currentTab == QUESTS_ALIASES.MISSIONS_PREMIUM_VIEW_PY_ALIAS:
                subView.currentTab.setDefaultTab(event.ctx['subTab'])
            elif subTab is not None and targetTab == currentTab == QUESTS_ALIASES.BATTLE_PASS_MISSIONS_VIEW_PY_ALIAS:
