...
class LinkedSetDetailsOverlay(LinkedSetDetailsOverlayMeta):
        ...
        if isCompleted:
            ...
            if isAvailable:
                ...
                if curProgress is not None and maxProgress:
                    cardStatus = makeHtmlString('html_templates:lobby/quests/linkedSet', 'questProgressTemplate', {'curValue': backport.getIntegralFormat(curProgress), 'maxValue': backport.getIntegralFormat(maxProgress)})
    ...
    def _getViewData(self, title=None, status=None, info=None, task=None, description=None, isBackOverlay=None, isMovie=None, back=None, btnStartLabel=None, awards=None, progressValue=None):
        ...
        if progressValue is not None:
            res['progressBarData'] = {'value': progressValue, 'minValue': 0, 
               'maxValue': 100}
        return res
...
def _getNewMissionCounter(eventsCache):
    return (
     'missions', len(quest_settings.getNewCommonEvents(eventsCache.getAdvisableQuests().itervalues())))
