...
class VehicleMarkerPlugin(MarkerPlugin, ChatCommunicationComponent, IArenaVehiclesController):
    ...
    __slots__ = ('_markers', '_markersStates', '_clazz', '__playerVehicleID', '_isSquadIndicatorEnabled',
                 '__showDamageIcon', '_markerTimers', '__callbackIDs', '__targetedTankMarkerID',
                 '__targetedMarkerFromCppID', '__equipmentCtrl')
    ...
    def isMarkerActive(self, targetID):
        if targetID in self._markers:
            return self._markers[targetID].isActive()
        return False
