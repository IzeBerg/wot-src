...
class _WGNCPackageBusinessHandler(PackageBusinessHandler):
    ...
    def __showBasicWindow(self, event):
        ...
        if not item:
            ...
            self.loadViewWithDefName(alias, ('{0}_{1}').format(WGNC_ALIASES.MODAL_BASIC_WINDOW, notID), None, {'notID': notID, 'target': target})
            return

    def __showPollWindow(self, event):
        notID = event.getNotID()
        target = event.getTarget()
        self.loadViewWithDefName(WGNC_ALIASES.POLL_WINDOW, ('{0}_{1}').format(WGNC_ALIASES.POLL_WINDOW, notID), None, {'notID': notID, 'target': target})
        return