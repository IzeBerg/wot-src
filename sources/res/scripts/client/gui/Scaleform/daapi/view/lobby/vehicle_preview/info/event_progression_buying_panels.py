...
class _VehiclePreviewEventProgressionBuyingPanel(VehiclePreviewEventProgressionBuyingPanelMeta):
    ...
    def onBuyClick(self):
        item = self._getItem()
        dialog = SFWindow(SFViewLoadParams(EVENTPROGRESSION_ALIASES.EVENT_PROGRESION_BUY_CONFIRM_VIEW_ALIAS), EVENT_BUS_SCOPE.LOBBY, ctx={'item': item, 'price': self._getPrice(item)})
