import math, weakref, typing
from itertools import ifilter, chain
import BigWorld, async as future_async, constants, WWISE
...
class _RankedBattlesWelcomeViewLifecycleHandler(IViewLifecycleHandler):
    ...
    def __init__(self, lobbyHeader):
        super(_RankedBattlesWelcomeViewLifecycleHandler, self).__init__([
...
class LobbyHeader(LobbyHeaderMeta, ClanEmblemsHelper, IGlobalListener):
    ...
    PRB_NAVIGATION_DISABLE_BUTTONS = (
     ...
     BUTTONS.ACCOUNT, BUTTONS.PREMSHOP, BUTTONS.WOT_PLUS)
    ...
    def _updatePrebattleControls(self, *_):
        ...
        if self.__isFightBtnDisabled and not state.hasLockedState:
            ...
            elif isEvent and result.restriction in (
             PREBATTLE_RESTRICTION.EVENT_AFK_BAN, UNIT_RESTRICTION.EVENT_AFK_BAN):
    ...
    def __updateTabCounter(self, alias, counter=None):
        if alias not in self.TABS.ALL():
            ...
            if self.uiSpamController.shouldBeHidden(alias):
                return
