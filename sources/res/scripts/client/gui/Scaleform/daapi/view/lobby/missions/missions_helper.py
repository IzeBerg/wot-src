...
class _EventProgressionDailyMissionInfo(_MissionInfo):
    ...
    def _getCompleteStatusTooltipHeader(self):
        if not self._eventController.isDailyQuestsRefreshAvailable():
            return QUESTS.MISSIONDETAILS_STATUS_NOTAVAILABLE
        return super(_EventProgressionDailyMissionInfo, self)._getCompleteStatusTooltipHeader()

    def _getCompleteKey(self):
        if not self._eventController.isDailyQuestsRefreshAvailable():
            return EPIC_BATTLE.QUESTSTOOLTIP_EPICBATTLE_TIMELEFT
        return super(_EventProgressionDailyMissionInfo, self)._getCompleteKey()
