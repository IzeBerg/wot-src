...
class VehiclePreviewTradeInBuyingPanel(VehiclePreviewTradeInBuyingPanelMeta):
    ...
    def __getStatus(self):
        if not self.__tradeIn.isEnabled():
            ...
            if self.__tradeOffVehicle is not None and self.__tradeInVehicle is not None:
                ...
                return (
                 backport.text(_RScopeTradeIn.tradeOffPriceText() if isValidSelected else _RScopeTradeIn.invalidTradeOffVehicle()),
                 isValidSelected, isValidSelected)
            if self.__tradeIn.getTradeOffVehicles() and self.__tradeInVehicle is not None:
                ...
                return (
                 backport.text(_RScopeTradeIn.availableLevels()).format(levels=romanLevels), True, True)
            return (
             backport.text(_RScopeTradeIn.notAvailableTradeOffVehicles()), False, True)
