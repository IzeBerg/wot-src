            ...
            for idx, (vehCrewRoleIdx, vehCrewRole) in enumerate(veh_crew):
                if vehCrewRoleIdx == roleIdx:
                    ...
                    veh_crew[idx] = (roleIdx,
                     cmp_helpers.createTankman(nationID, vehicleTypeID, role, prevRoleLevel, skills.intersection(selectedSkills)))
                    success = True
                    break
            else:
...
class VehicleCompareConfiguratorView(LobbySubView, VehicleCompareConfiguratorViewMeta):
    ...
    def __updateSkillsData(self):
        skills = [ {'icon': Tankman.getSkillBigIconPath(skillType), 'label': Tankman.getSkillUserName(skillType), 'skillType': skillType, 'isForAll': skillType in tankmen.COMMON_SKILLS, 'selected': skillType in self._container.getCurrentCrewSkills()} for skillType in PARAMS_AFFECTED_TANKMEN_SKILLS
                 ]
    ...
    def __getCrewLevels(isInHangar):
        items = [
         {'label': VEH_COMPARE.VEHICLECOMPAREVIEW_CREW_SKILL100, 'id': CrewTypes.SKILL_100, 'showAlert': False, 
            'tooltip': None}, {'label': VEH_COMPARE.VEHICLECOMPAREVIEW_CREW_SKILL75, 'id': CrewTypes.SKILL_75}, {'label': VEH_COMPARE.VEHICLECOMPAREVIEW_CREW_SKILL50, 'id': CrewTypes.SKILL_50}]
        if isInHangar:
            items.append({'label': VEH_COMPARE.VEHICLECOMPAREVIEW_CREW_CURRENT, 'id': CrewTypes.CURRENT})
        return items
