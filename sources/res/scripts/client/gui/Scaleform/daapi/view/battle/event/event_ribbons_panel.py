...
class EventRibbonsPanel(BattleRibbonsPanel, GameEventGetterMixin):
    ...
    def _checkUserPreferences(self, ribbon):
        if ribbon.getType() in _BET.BUFFS_SET or ribbon.getType() in _BET.EVENT_SET:
            return not self._isRibbonPanelOverlay
        return super(EventRibbonsPanel, self)._checkUserPreferences(ribbon)
