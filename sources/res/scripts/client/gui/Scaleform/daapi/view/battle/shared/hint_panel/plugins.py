...
class SiegeIndicatorHintPlugin(HintPanelPlugin):
    ...
    def start(self):
        ...
        self.__settings = {setting:AccountSettings.getSettings(setting) for setting in (
         SIEGE_HINT_SECTION, WHEELED_MODE_HINT_SECTION, TURBO_SHAFT_ENGINE_MODE_HINT_SECTION)}
...
class PreBattleHintPlugin(HintPanelPlugin):
    __slots__ = ('__isActive', '__hintInQueue', '__callbackDelayer', '__questHintSettings',
                 '__helpHintSettings', '__battleComHintSettings', '__spgHelpHintSettings',
                 '__isInDisplayPeriod', '__haveReqLevel', '__vehicleId', '__hasTempSPGScreens',
                 '__isSPG')
    sessionProvider = dependency.descriptor(IBattleSessionProvider)
