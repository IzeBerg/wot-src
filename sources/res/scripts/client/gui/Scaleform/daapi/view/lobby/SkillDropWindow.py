...
_switchToWindowState = {SwitchState.ENABLED.value: SKILLS_CONSTANTS.RECERTIFICATION_USABLE, SwitchState.DISABLED.value: SKILLS_CONSTANTS.RECERTIFICATION_HIDDEN, 
...
class SkillDropWindow(SkillDropMeta):
    ...
    def __setData(self, *args):
        ...
        if tankman is None:
            ...
            dropSkillsCost.append({'action': None, 'xpReuseFraction': 1.0, 'gold': 0, 'credits': 0})
    ...
    def _getRecertificationStatus(self):
        if not self._recertificationFormGoodie.enabled:
            return SKILLS_CONSTANTS.RECERTIFICATION_HIDDEN
        return (
         _switchToWindowState[self.lobbyContext.getServerSettings().recertificationFormState()],)
