...
class EpicBattlesInfoView(LobbySubView, EpicBattlesInfoViewMeta):
    ...
    def onManageAbilitiesBtnClick(self):
        self.fireEvent(events.LoadViewEvent(SFViewLoadParams(EPICBATTLES_ALIASES.EPIC_BATTLES_SKILL_ALIAS), ctx={'showBackButton': True, 'previousPage': EPICBATTLES_ALIASES.EPIC_BATTLES_INFO_ALIAS}), EVENT_BUS_SCOPE.LOBBY)
        self.destroy()
    ...
    def onGameRewardsBtnClick(self):
        self.fireEvent(events.LoadViewEvent(SFViewLoadParams(EPICBATTLES_ALIASES.EPIC_BATTLES_BROWSER_ALIAS), ctx={'urlID': 'gameRules', 'showBackBtn': True, 'previousPage': EPICBATTLES_ALIASES.EPIC_BATTLES_INFO_ALIAS}), EVENT_BUS_SCOPE.LOBBY)
        self.destroy()
    ...
    def _packMetaProgressVO(self, prestigeAllowed, maxRewardClaimed):
        ...
        return {'titleHtmlText': titleText, 
           ...
           'progressBarData': {'value': pFamePts, 'maxValue': famePtsToProgress}, 'famePointsText': self.__getFormatedProgressHtmlText(pFamePts, famePtsToProgress), 
    ...
    def _packRewardRibbonData(self, pPrestigeLevel, allQuests, maxPrestigeLevel):
        ...
        if not finalRewardReached:
            return {}
        else:
            return {'rewardTitleHtmlText': text_styles.heroTitle(i18n.makeString(EPIC_BATTLE.INFOVIEW_PRESTIGE_CONGRATULATIONS).upper()), 'epicMetaLevelIconData': None, 
