import logging, BigWorld, constants, nations
...
class HangarHeader(HangarHeaderMeta, IGlobalListener, IEventBoardsListener):
    ...
    def _makeHeaderVO(self):
        ...
        if self.__gameEventController.isEventPrbActive():
            return {'isVisible': True, 'quests': self.__getEventQuestsToHeaderVO(self.__prebattleVehicle.item)}
    ...
    def __getEventQuestVO(self, vehicle):
        ...
        questsVO = [
         self._headerQuestFormaterVo(enable=isEnabled, icon=backport.image(icon), label=label, stateIcon=stateIcon, questType=HANGAR_HEADER_QUESTS.QUEST_TYPE_EVENT_BATTLES, flag=backport.image(flag), flagDisabled=flagDisabledIcon, tooltip=TOOLTIPS_CONSTANTS.EVENT_BATTLES_QUESTS_PREVIEW, isTooltipSpecial=True)]
    ...
    def __getMapboxProgressionVO(self):
        ...
        if data is not None and self.__mapboxCtrl.isActive():
            ...
            label = ''
        quests = [
         self._headerQuestFormaterVo(data is not None, progressionIcon, label, HANGAR_HEADER_QUESTS.QUEST_TYPE_MAPBOX, flag=flag, tooltip=TOOLTIPS_CONSTANTS.MAPBOX_PROGRESSION_PREVIEW, isTooltipSpecial=True)]
    ...
    def __getBattleQuestsVO(self, vehicle):
        ...
        quests = [
         self._headerQuestFormaterVo(totalCount > 0, commonQuestsIcon, label, questType, flag=festivityFlagData.flagBackground, tooltip=TOOLTIPS_CONSTANTS.QUESTS_PREVIEW, isTooltipSpecial=True)]
    ...
    def __getRankedBattleQuestsVO(self):
        ...
        questsVo = [
         self._headerQuestFormaterVo(totalCount > 0, backport.image(commonQuestsIcon), label, HANGAR_HEADER_QUESTS.QUEST_GROUP_RANKED_DAILY, flag=backport.image(R.images.gui.maps.icons.library.hangarFlag.flag_ranked()), tooltip=TOOLTIPS_CONSTANTS.RANKED_QUESTS_PREVIEW, isTooltipSpecial=True)]
    ...
    def __getMarathonQuestsVO(self, vehicle, isGroupped=False):
        ...
        if marathons:
            ...
            for index, marathonEvent in enumerate(marathons):
                ...
                if flagVO['visible']:
                    quest = self._headerQuestFormaterVo(flagVO['enable'], flagVO['flagHeaderIcon'], '', ('').join((HANGAR_HEADER_QUESTS.QUEST_TYPE_MARATHON,
    ...
    def __getElenQuestsVO(self, vehicle):
        ...
        quests = [
         self._headerQuestFormaterVo(enable, eventQuestsIcon, eventQuestsLabel, HANGAR_HEADER_QUESTS.QUEST_TYPE_EVENT, questID=eventId, isReward=True, tooltip=eventQuestsTooltip, isTooltipSpecial=eventQuestsTooltipIsSpecial)]
