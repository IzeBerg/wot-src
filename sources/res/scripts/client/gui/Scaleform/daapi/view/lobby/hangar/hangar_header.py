...
class HangarHeader(HangarHeaderMeta, IGlobalListener, IEventBoardsListener):
    ...
    def __getEventQuestVO(self, vehicle):
        ...
        questsVO = [
         self._headerQuestFormaterVo(enable=isEnabled, icon=backport.image(icon), label=label, stateIcon=stateIcon, questType=HANGAR_HEADER_QUESTS.QUEST_TYPE_EVENT_BATTLES, flag=backport.image(flag), tooltip=TOOLTIPS_CONSTANTS.EVENT_BATTLES_QUESTS_PREVIEW, isTooltipSpecial=True)]
