...
class HangarHeader(HangarHeaderMeta, IGlobalListener, IEventBoardsListener):
    ...
    def __getArmoryYardQuestsVO(self, wrap=True):
        ...
        if state == State.BEFOREPROGRESSION:
            ...
            quests = [
             self._headerQuestFormatterVo(enabled, mainIcon, label, questType, tooltip=tooltip, flagDisabled=backport.image(iconsPath.flag_disabled()), stateIcon=stateIcon, isTooltipWulf=isTooltipWulf)]
            if wrap:
