...
class RecruitWindow(RecruitWindowMeta):
    ...
    def updateAllDropdowns(self, nationID, tankType, typeID, roleType):
        nationsDP = [{'id': None, 'label': DIALOGS.RECRUITWINDOW_MENUEMPTYROW}, {'id': nationID, 'label': MENU.nations(nations.NAMES[int(nationID)])}]
        classesDP = [{'id': None, 'label': DIALOGS.RECRUITWINDOW_MENUEMPTYROW}, {'id': tankType, 'label': DIALOGS.recruitwindow_vehicleclassdropdown(tankType)}]
        ...
        for module in modulesAll:
            typesDP.append({'id': module.innationID, 'label': module.shortUserName})
            skillsConfig = getSkillsConfig()
            for role in module.descriptor.type.crewRoles:
                if role[0] == roleType:
                    rolesDP.append({'id': role[0], 'label': convert(skillsConfig.getSkill(role[0]).userString)})
