...
class LinkedSetHintsView(LinkedSetHintsViewMeta):
    ...
    def _getAwards(self, bonuses):
        return [ {'icon': award['imgSource'], 'value': award['label'], 'tooltip': award.get('tooltip', None), 'specialAlias': award.get('specialAlias', None), 'specialArgs': award.get('specialArgs', None)} for award in getLinkedSetBonuses(mergeBonuses(bonuses)) if award
               ]