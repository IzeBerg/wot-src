...
class EventProgressionDataProvider(SelectorExtraItem):
    ...
    def isAvailable(self):
        if self.__bcc.isInBootcamp():
            return False
        return self.__eventProgression.isAvailable() and self.__eventProgression.getCurrentSeason()
    ...
    def getSpecialBGIcon(self):
        if self.__eventProgression.isActive():
            return backport.image(_R_ICONS.buttons.selectorRendererExtraBGEvent())
        return ''
