...
class RTSCommanderMarkerPlugin(VehicleMarkerPlugin):
    ...
    def _getMarkerSymbol(self, vehicleID):
        vehicle = self.__sessionProvider.dynamic.rtsCommander.vehicles.get(vehicleID)
        if vehicle and vehicle.isAllyBot:
            return MARKER_SYMBOL_NAME.RTS_CONTROLLABLE_VEHICLE_MARKER
        return MARKER_SYMBOL_NAME.RTS_VEHICLE_MARKER
...
class RTSTankmanMarkerPlugin(VehicleMarkerPlugin):
    ...
    def _getMarkerSymbol(self, vehicleID):
        player = BigWorld.player()
        arenaVehicle = player.arena.vehicles.get(vehicleID)
        if arenaVehicle.get('accountDBID') == 0:
            return MARKER_SYMBOL_NAME.RTS_TANKMAN_VEHICLE_MARKER
        return super(RTSTankmanMarkerPlugin, self)._getMarkerSymbol(vehicleID)
