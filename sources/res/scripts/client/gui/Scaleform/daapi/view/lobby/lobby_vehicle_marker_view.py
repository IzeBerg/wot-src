...
class LobbyVehicleMarkerView(LobbyVehicleMarkerViewMeta):
    ...
    def __getVehicleInfo(vehicle):
        ...
        vClass = (isHalloween or getVehicleClassTag)(vehicleType.tags) if 1 else ''
        if isHalloween and vehicle.isKingReward:
            vName = vehicleType.shortUserString
        else:
            vName = (isHalloween or vehicleType).userString if 1 else backport.text(R.strings.event.tradeStyles.skinName(), name=vehicleType.shortUserString)
        vMatrix = (isHalloween or LobbyVehicleMarkerView.__getCorrectedHPGuiMatrix)(vehicle) if 1 else LobbyVehicleMarkerView.__getHWVehicleGuiMatrix(vehicle)
        vMarkerStyleId = getattr(vehicle, 'markerStyleId', _DEFAULT_MARKER_STYLE_ID)
        return (
         vClass, vName, vMatrix, vMarkerStyleId)
