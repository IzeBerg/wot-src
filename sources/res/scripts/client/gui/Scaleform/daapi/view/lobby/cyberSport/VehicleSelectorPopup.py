...
class VehicleSelectorPopup(VehicleSelectorPopupMeta, VehicleSelectorBase):
    ...
    def updateData(self):
        if not self.getFilters().get('compatibleOnly', True) or self._vehicles is None:
            vehicleVOs = self._updateData(self.itemsCache.items.getVehicles(~REQ_CRITERIA.VEHICLE.BATTLE_ROYALE | ~REQ_CRITERIA.VEHICLE.HIDDEN_IN_HANGAR | REQ_CRITERIA.INVENTORY))
        else:
