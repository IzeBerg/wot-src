...
def _convertGuiSupplyTypeToTag(guiType):
    if guiType in _SUPPLY_GUI_TYPE_TO_ID:
        return RTSSupply.SUPPLY_ID_TO_TAG[_SUPPLY_GUI_TYPE_TO_ID[guiType]]
    return guiType


def convertTagToGuiSupplyType(tagName):
    if tagName in RTSSupply.TAG_TO_SUPPLY:
        return _SUPPLY_ID_TO_GUI_TYPE[RTSSupply.TAG_TO_SUPPLY[tagName]]
    return tagName
...
class SpawnMenu(SpawnMenuMeta, IRTSSpawnListener, IAbstractPeriodView):
    ...
    def __getBadgeData(vehicleInfo):
        ...
        if badge is None:
            ...
            return badge.getBadgeVO(ICONS_SIZES.X24, {'isAtlasSource': True}, shortIconName=True)
    ...
    def __fillVehiclesData(self, initialVO):
        ...
        if arenaDP is None:
            ...
            for vInfo in self._getSpawnCtrl().getSortedVehicleInfos():
                ...
                elif arenaDP.isEnemyTeam(vInfo.team):
    ...
    def _isStrategistEnemyVehicle(self, vInfo):
        ...
        if vehicle is not None:
            return vehicle.get('accountDBID') == 0 and arena.bonusType == constants.ARENA_BONUS_TYPE.RTS_1x1
        else:
            return True
    ...
    def __prepareYawForUI(self, targetYaw, chosenPointDataYaw):
        if targetYaw is None:
            return 0
        else:
            return targetYaw + chosenPointDataYaw - _HALF_PI
    ...
    def __getVehicleVisionData(self, vehicleID):
        ...
        return (
         visionRadius, engagementRadius, yawLeftLimit, yawRightLimit)
