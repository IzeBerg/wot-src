...
class CurrenciesFormatter(SimpleBonusFormatter):
    ...
    def accumulateBonuses(self, bonus, event=None):
        if bonus.getCode() == Currency.STPCOIN:
            ...
            self._result.append({'formattedValue': self._formatCurrencyBonus(bonus), 'tooltip': bonus.getTooltip()})
        else:
    ...
    def extractFormattedBonuses(self, addLineSeparator=False):
        if self.__hasSTPCoinBonus:
            ...
            for formattedBonus in self._result:
                result.append(formatters.packSimpleBonusesBlock([
                 formattedBonus['formattedValue']], endlineSymbol=_END_LINE_SEPARATOR if addLineSeparator else _EMPTY_STRING, complexTooltip=formattedBonus['tooltip']))
