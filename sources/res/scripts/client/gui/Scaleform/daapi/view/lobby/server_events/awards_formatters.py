...
class LunarEnvelopesFormatter(OldStyleBonusFormatter):
    ...
    def accumulateBonuses(self, bonus, event=None):
        ...
        tooltipData = bonus.getTooltipData()
        block = formatters.packSingleLineBonusesBlock([
         text_styles.main(label)], complexTooltip=tooltipData.tooltip, specialTooltip=tooltipData.specialAlias, specialTooltipArgs=tooltipData.specialArgs)
        self._result.append(block)
...
class OldStyleAwardsPacker(AwardsPacker):
    ...
    def format(self, bonuses, event=None):
        ...
        fmts = [
         self.__lootBoxFormatter, self.__giftStampsFormatter, self.__lunarEnvelopesFormatter,
