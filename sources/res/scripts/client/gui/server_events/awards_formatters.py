...
class TokenBonusFormatter(SimpleBonusFormatter):
    ...
    def _formatLootBoxToken(self, tokenID, token, bonus):
        ...
        return PreformattedBonus(label=self._formatBonusLabel(token.count), userName=self._getLootboxUserName(lootBox), labelFormatter=self._getLabelFormatter(bonus), images=images, tooltip=makeTooltip(header=self._getLootboxUserName(lootBox), body=''), specialAlias=TOOLTIPS_CONSTANTS.EVENT_LOOTBOX, specialArgs=[
         lootBox.getType()], isWulfTooltip=True, align=LABEL_ALIGN.RIGHT, isCompensation=self._isCompensation(bonus))
