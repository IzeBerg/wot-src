...
class RewardBox(namedtuple('RewardBox', [
 'boxIndex',
 'decodePrice',
 'skipPrice',
 'bonusRewards',
...
class EventRewardController(object):
    ...
    def __getRewardBoxQuestSkipPrice(self, rewardBoxID):
        return self.__getRewardBoxes().get(rewardBoxID, {}).get('questSkipCost', (None,
                                                                                  0))
    ...
    def __getRewardBoxes(self):
        if self.isEnabled():
            return self.__getConfig().get('boxes', {})
        return {}
    ...
    def __handleTokensUpdate(self, diff):
        ...
        for token in diff.iterkeys():
            ...
            elif token.startswith(EVENT.REWARD_BOX.KEY_TOKEN):
                self._gameEventController.onRewardBoxKeyUpdated()
            elif token.startswith(EVENT.HW21_DAILY_TOKEN_PREFIX):
...
class OpenRewardBoxProcessor(Processor):
    ...
    def __init__(self, controller, rewardBoxID, isSkipQuest):
        super(OpenRewardBoxProcessor, self).__init__(plugins=(
         plugins.CheckRewardBox(controller, rewardBoxID, isSkipQuest),))
    ...
    def _successHandler(self, code, ctx=None):
        ...
        money = (self._isSkipQuest or rewardBox.decodePrice).amount if 1 else rewardBox.skipPrice.amount
