...
class GameEventController(IGameEventController):
    ...
    def _getContainers(self):
        return (
         self._shop, self._vehiclesController, self._awardWindowShowController)
    ...
    def needEventCrew(self, vehicle):
        if vehicle.intCD in self._vehiclesController.getVehiclesForRent() and not vehicle.hasCrew:
            return True
        return False
