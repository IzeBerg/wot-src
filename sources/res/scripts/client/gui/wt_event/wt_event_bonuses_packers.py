...
class WTTokenBonusUIPacker(SelectableBonusPacker):
    ...
    def _getToolTip(cls, bonus):
        ...
        for tokenID in bonus.getValue().iterkeys():
            ...
            tooltipData.append(TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.EVENT_GIFT_TOKEN, specialArgs=[
             tokenID]))
...
class WtLootboxTokenBonusPacker(WtTokenBonusPacker):
    ...
    def _packTokenTooltip(cls, token):
        lootBox = cls._itemsCache.items.tokens.getLootBoxByTokenID(token.id)
        return createTooltipData(tooltip=TOOLTIPS_CONSTANTS.EVENT_LOOTBOX, isWulfTooltip=True, isSpecial=False, specialAlias=None, specialArgs=[
         lootBox.getType()])
