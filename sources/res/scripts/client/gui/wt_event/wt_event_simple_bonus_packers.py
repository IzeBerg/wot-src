...
BOSS_BONUSES_ORDER = [Currency.GOLD, Currency.CREDITS, Currency.FREE_XP, _RANDOM_CREWBOOK, 'crewBooks',
...
def _getIsStyleCollected(key):
    ...
    if status and callable(status):
        return status() == _Status.COLLECTED
    return False
...
def _composeCustomization(bonuses):
    ...
    for bonus in bonuses:
        ...
        for item in bonus.getCustomizations():
            ...
            elif itemType in (_DECAL, _PROJECTION_DECAL):
                decals.append(bonus)
            elif itemType == _INSCRIPTION:
                inscriptions.append(bonus)
            elif itemType == _EMBLEM:
                emblems.append(bonus)
            elif itemType == _CAMOUFLAGE:
...
class VehicleBlueprintsPacker(object):
    ...
    def getTooltip(bonuses):
        ...
        return TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.EVENT_LOOT_BOXES_VEHICLE_BLUEPRINT_FRAGMENT, specialArgs=[
         fragmentCDs])
