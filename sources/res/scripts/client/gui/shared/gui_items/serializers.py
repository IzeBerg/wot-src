import cPickle, math, collections
...
def packTankman(tankman, isCountPermanentSkills=True):
    ...
    if tankman.isInTank:
        currentVehicleData = {'inventoryID': tankman.vehicleInvID, 'typeCompDescr': tankman.vehicleDescr.type.compactDescr, 
...
def packVehicle(vehicle):
    ...
    result.update({'buyPrice': vehicle.buyPrices.itemPrice.price, 
       ...
       'optDevices': [ packFittingItem(dev) if dev else None for dev in vehicle.optDevices.installed ], 'shells': [ packShell(shell) if shell else None for shell in vehicle.shells.installed ], 'eqs': [ packFittingItem(eq) if eq else None for eq in vehicle.consumables.installed ], 'eqsLayout': [ packFittingItem(eq) if eq else None for eq in vehicle.consumables.layout ], 'type': vehicle.type, 
...
def packDropSkill(tankman, itemsCache=None):
    ...
    result = [ {'level': ('{}%').format(int(lvl)), 'enabled': state, 'price': [cost.getCurrency(), cost.getSignValue(cost.getCurrency())], 'isMoneyEnough': cost <= currentMoney, 'isNativeVehicle': True, 'nation': vehicle.nationName, 'showAction': cost != defCost} for lvl, state, cost, defCost in zip(tankmanLevels, states, trainingCosts, defaultCosts)
             ]
    return result
