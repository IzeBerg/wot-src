...
class Tankman(GUIItem):
    ...
    __slots__ = ('__descriptor', '_invID', '_nationID', '_itemTypeID', '_itemTypeName',
                 ...
                 '_skills', '_skillsMap', '_skinID', '_comparator', '__brotherhoodMarkedAsActive')
...
def getSpecialIconPath(nationID, iconID):
    ...
    if dynAccessor.isValid():
        return backport.image(dynAccessor())
    return backport.image(R.images.gui.maps.icons.tankmen.icons.special.tankman())
...
def _boostCommonSkill(crew, skillName, tankmenSkillLevels, booster):
    if booster is None:
        ...
        if sum(tankmenSkillLevels) > tankmen.MAX_SKILL_LEVEL * len(crew):
            return tankmenSkillLevels
        return [tankmen.MAX_SKILL_LEVEL] * len(crew)
...
def _getSkillLevelWithIncrease(booster, skillLevel, tankman):
    ...
    if booster is None:
        ...
        if tankman.currentVehicleSkillsEfficiency < tankmen.MAX_SKILLS_EFFICIENCY or skillLevel < tankmen.MAX_SKILL_LEVEL:
