...
class Tankman(GUIItem):
    ...
    __slots__ = ('__descriptor', '_invID', '_nationID', '_itemTypeID', '_itemTypeName',
                 '_combinedRoles', '_dismissedAt', '_isDismissed', '_areClassesCompatible',
                 '_vehicleNativeDescr', '_vehicleInvID', '_vehicleDescr', '_vehicleBonuses',
                 '_vehicleSlotIdx', '_skills', '_skillsMap', '_skinID', '_comparator',
    ...
    def __init__(self, strCompactDescr, inventoryID=-1, vehicle=None, dismissedAt=None, proxy=None, vehicleSlotIdx=-1):
        ...
        self._vehicleNativeDescr = vehicles.VehicleDescr(typeID=(
         self.nationID, self.descriptor.vehicleTypeID))
    ...
    def realRoleLevel(self):
        ...
        return (
         realRoleLevel, (commBonus, brothersBonus, eqsBonus, optDevsBonus, penalty))
    ...
    def getFreeSkillsToLearn(self):
        ...
        result.append({'id': self.role, 'skills': skills})
        return result
...
class TankmanSkill(GUIItem):
    __slots__ = ('_name', '_isPerk', '_level', '_type', '_roleType', '_isActive', '_isEnable',
                 '_isFemale', '_isPermanent')
...
class WitchesTankmanSkill(TankmanSkill):
    ...
    def userName(self):
        if self._name == BROTHERHOOD_SKILL_NAME:
            return backport.text(R.strings.item_types.tankman.skills.brotherhood_witches())
        return super(WitchesTankmanSkill, self).userName
