...
def getVehicleConsumablesLayoutPrice(vehicle):
    return sum([ item.getBuyPrice() for item in vehicle.consumables.layout.getItems() if not item.isInInventory and item not in vehicle.consumables.installed
               ], ITEM_PRICE_ZERO)


def getVehicleBattleBoostersLayoutPrice(vehicle):
    return sum([ item.getBuyPrice() for item in vehicle.battleBoosters.layout.getItems() if not item.isInInventory and item not in vehicle.battleBoosters.installed
               ], ITEM_PRICE_ZERO)


def getVehicleOptionalDevicesLayoutPrice(vehicle):
    return sum([ item.getBuyPrice() for item in vehicle.optDevices.layout.getItems() if not item.isInInventory and item not in vehicle.optDevices.installed
               ], ITEM_PRICE_ZERO)
