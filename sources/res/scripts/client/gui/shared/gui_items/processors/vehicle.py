...
class VehiclePersonalTradeInProcessor(VehicleTradeInProcessorBase):
    ...
    def _getPluginsList(self):
        return super(VehiclePersonalTradeInProcessor, self)._getPluginsList() + (
         proc_plugs.VehiclePersonalTradeInValidator(self.item, self.itemToTradeOff),)
...
class VehicleSeller(ItemProcessor):
    ...
    __slots__ = ('vehicle', 'nationGroupVehs', 'shells', 'eqs', 'optDevs', 'gainMoney',
                 ...
                 'isCrewDismiss', 'isDismantlingForMoney', 'isRemovedAfterRent',
                 'usedDemountKitsCount', '__hasPairModification')
    ...
    def __init__(self, vehicle, shells=None, eqs=None, optDevs=None, inventory=None, customizationItems=None, boosters=None, isCrewDismiss=False, itemsForDemountKit=None):
        ...
        if self.__lobbyContext.getServerSettings().isCrewSkinsEnabled():
            ctx = {'price': self.__compensationAmount, 'action': None, 'items': crewSkinsNeedDeletion}
...
class VehicleTmenXPAccelerator(VehicleSettingsProcessor):
    ...
    def __init__(self, vehicle, value):
        super(VehicleTmenXPAccelerator, self).__init__(vehicle, VEHICLE_SETTINGS_FLAG.XP_TO_TMAN, value, (
         proc_plugs.MessageConfirmator('xpToTmenCheckbox', isEnabled=value),))
