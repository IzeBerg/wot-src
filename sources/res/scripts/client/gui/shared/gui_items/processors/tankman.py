...
class TankmanDismiss(ItemProcessor):
    ...
    def __init__(self, tankman):
        ...
        if deletedTankmen and tankman.isRestorable():
            self.addPlugin(plugins.BufferOverflowConfirmator({'dismissed': tankman, 'deleted': deletedTankmen[0]}))
        ...
        if equippedSkinID != NO_CREW_SKIN_ID and lobbyContext.getServerSettings().isCrewSkinsEnabled():
            ...
            self.addPlugin(plugins.CrewSkinsCompensationDialogConfirmator('crewSkins/skinWillBeDeleted', plugins.CrewSkinsRemovalCompensationDialogMeta.OUT_OF_STORAGE_SUFFIX, ctx={'price': self.__compensationPriceObject, 'action': None, 'items': [crewSkinItem]}, isEnabled=self.__compensationRequired))
        return
...
class CrewSkinsProcessorBase(Processor):
    ...
    def __init__(self, tmanInvID):
        ...
        if equippedSkinID != NO_CREW_SKIN_ID and lobbyContext.getServerSettings().isCrewSkinsEnabled():
            ...
            self.__compensationPriceObject = crewSkinItem.getBuyPrice()
            self.__compensationRequired = not crewSkinItem.isStorageAvailable()
            self.addPlugin(plugins.CrewSkinsCompensationDialogConfirmator('crewSkins/skinWillBeDeleted', plugins.CrewSkinsRemovalCompensationDialogMeta.OUT_OF_STORAGE_SUFFIX, ctx={'price': self.__compensationPriceObject, 'action': None, 'items': [crewSkinItem]}, isEnabled=not crewSkinItem.isStorageAvailable()))
        return
...
class TankmanRetraining(ItemProcessor):
    ...
    def __init__(self, tankman, vehicle, tmanCostTypeIdx):
        ...
        super(TankmanRetraining, self).__init__(tankman, (
...
class TankmanChangeRole(ItemProcessor):
    ...
    def __init__(self, tankman, role, vehTypeCompDescr):
        ...
        super(TankmanChangeRole, self).__init__(tankman, (
         plugins.TankmanLockedValidator(tankman),
         plugins.VehicleCrewLockedValidator(vehicle),
        ...
        if equippedSkinID != NO_CREW_SKIN_ID and lobbyContext.getServerSettings().isCrewSkinsEnabled():
            ...
            self.addPlugins((
             plugins.CrewSkinsRoleChangeRemovalConfirmator('crewSkins/skinWillBeRemoved', ctx={'items': [crewSkinItem]}, isEnabled=roleMismatch and crewSkinItem.isStorageAvailable()),
             plugins.CrewSkinsCompensationDialogConfirmator('crewSkins/skinWillBeDeleted', plugins.CrewSkinsRemovalCompensationDialogMeta.ROLE_MISMATCH_SUFFIX, ctx={'price': self.__compensationPriceObject, 'items': [crewSkinItem]}, isEnabled=self.__compensationRequired)))
        return
...
class TankmanDropSkills(ItemProcessor):
    ...
    def __init__(self, tankman, dropSkillCostIdx):
        super(TankmanDropSkills, self).__init__(tankman, (
         plugins.MessageConfirmator('dropSkill'),
...
class TankmanRestore(ItemProcessor):
    ...
    def __init__(self, tankman):
        ...
        super(TankmanRestore, self).__init__(tankman, (
         plugins.TankmanLockedValidator(tankman),
