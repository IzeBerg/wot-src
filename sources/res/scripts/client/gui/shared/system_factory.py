...
def collectEquipmentItem(equipmentName, isReplay, args):
    return __collectEM.handleEvent((EQUIPMENT_ITEMS, equipmentName), {'args': args, 'isReplay': isReplay}).get('item')
...
def collectHangarPresetsGetters(config):
    return __collectEM.handleEvent(HANGAR_PRESETS_PROCESSORS, {'presetsGetters': {}, 'config': config})['presetsGetters']
