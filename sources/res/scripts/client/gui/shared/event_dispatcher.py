...
def showAnimatedDogTags(initBackgroundId=0, initEngravingId=0, closeCallback=None):
    lobbyContext = dependency.instance(ILobbyContext)
    if not lobbyContext.getServerSettings().isDogTagCustomizationScreenEnabled():
        return
    else:
        ...
        dtView = uiLoader.windowsManager.getViewByLayoutID(contentResId)
        if dtView is None:
            g_eventBus.handleEvent(events.LoadGuiImplViewEvent(GuiImplViewLoadParams(R.views.lobby.dog_tags.AnimatedDogTagsView(), AnimatedDogTagsView, ScopeTemplates.LOBBY_SUB_SCOPE), initBackgroundId=initBackgroundId, initEngravingId=initEngravingId, closeCallback=closeCallback), scope=EVENT_BUS_SCOPE.LOBBY)
        return
