...
def showProfileWindow(databaseID, userName):
    ...
    g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(alias, getViewName(alias, databaseID)), ctx={'userName': userName, 'databaseID': databaseID}), EVENT_BUS_SCOPE.LOBBY)
...
def showFrontlineExchangePrestigePoints(ctx):
    ...
    builder.setMessageArgs(fmtArgs=[
     FmtArgs(str(prestigePoints), 'points', R.styles.NeutralTextStyle()),
     FmtArgs(formatGoldPrice(gold), 'gold', R.styles.GoldTextStyle()),
     FmtArgs(formatCrystalPrice(crystals), 'bons', R.styles.CrystalTextStyle())])
    result = yield await(dialogs.showSimple(builder.build()))
    if result:
