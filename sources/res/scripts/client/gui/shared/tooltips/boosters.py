...
class BoosterTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, boosterID, *args, **kwargs):
        ...
        items.append(formatters.packBuildUpBlockData([
         formatters.packTitleDescBlock(title=_ts.highTitle(_ms(MENU.boosterDescriptionBVTLocale(booster.boosterGuiType), effectValue=booster.getFormattedValue(_ts.highTitleAccented), effectTime=booster.getEffectTimeStr(hoursOnly=True)))),
         formatters.packImageBlockData(img=booster.bigTooltipIcon, align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, width=180, height=180, padding=formatters.packPadding(top=-14, bottom=-14))]))
    ...
    def __packAccessCondition(self, questsResult):
        ...
        return formatters.packBuildUpBlockData([
         formatters.packTitleDescBlock(_ts.middleTitle(TOOLTIPS.BOOSTERSWINDOW_BOOSTER_GETCONDITION_TITLE), _ts.standard(_ms(TOOLTIPS.BOOSTERSWINDOW_BOOSTER_GETCONDITION_VALUE, questName=_ts.neutral(qNames))))])
    ...
    def __packActiveState(self, time):
        ...
        return formatters.packBuildUpBlockData([
         formatters.packTitleDescParameterWithIconBlockData(title=state, icon=RES_ICONS.MAPS_ICONS_TOOLTIP_ACTIVEICON, padding=formatters.packPadding(left=93), titlePadding=formatters.packPadding(), iconPadding=formatters.packPadding(top=2, left=-2)),
         formatters.packAlignedTextBlockData(text=timeLeft, align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)])
