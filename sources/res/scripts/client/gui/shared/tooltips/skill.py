    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if infoBlock:
            items.append(formatters.packBuildUpBlockData([
             infoBlock], padding=formatters.packPadding(left=0, right=58, top=-5, bottom=0), linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE))
...
class TankmanSkillTooltipData(BlocksTooltipData):
    ...
    def __packTitleBlock(self):
        return formatters.packImageTextBlockData(title=text_styles.highTitle(TOOLTIPS.skillTooltipHeader(self.__skillType)), desc=text_styles.standard(TOOLTIPS.skillTooltipDescr(self.__skillType)), img=getSkillBigIconPath(self.__skillType), imgPadding={'left': 0, 'top': 3}, txtGap=-4, txtOffset=60, padding={'top': -1, 'left': 7, 'bottom': 10})
    ...
    def __packStatusBlock(self):
        ...
        blocks = [
         ...
         formatters.packImageTextBlockData(title=text_styles.main(_ms(TOOLTIPS.SKILLS_STATUS_ISFORROLE, role=_ms(TOOLTIPS.roleForSkill(role)), special=text_styles.neutral(specialStatus))), img=RES_ICONS.MAPS_ICONS_TANKMEN_CREW_CREWOPERATIONS if role == 'common' else getRoleMediumIconPath(role), imgPadding={'left': 20 if role == 'common' else 22, 'top': -3 if role == 'common' else -5}, txtGap=-4, txtOffset=60, padding={'top': 30, 'left': 7})]
