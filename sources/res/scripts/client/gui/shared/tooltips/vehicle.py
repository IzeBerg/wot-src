...
_SHORTEN_TOOLTIP_CASES = (
 'shopVehicle', 'eventCarouselVehicle')
...
class VehicleInfoTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        headerItems = [
         formatters.packBuildUpBlockData(headerBlockConstructorDescr(vehicle, statsConfig, leftPadding, rightPadding).construct(), padding=leftRightPadding, blockWidth=410),
...
class VehicleAvgParameterTooltipData(BaseVehicleAdvancedParametersTooltipData):
    ...
    def _packBlocks(self, paramName):
        ...
        blocks.append(formatters.packBuildUpBlockData([
         formatters.packTextParameterBlockData(text_styles.main(_ms(TOOLTIPS.getAvgParameterCommentKey(rangeParamName), units=_ms(param_formatter.MEASURE_UNITS.get(rangeParamName)))), text_styles.stats(fmtValue), valueWidth=80)]))
        return blocks
...
class VehicleAdvancedParametersTooltipData(BaseVehicleAdvancedParametersTooltipData):
    ...
    def _packListBlock(self, blocks, listBlock, title):
        if listBlock:
            ...
            blocks.append(formatters.packBuildUpBlockData([
             formatters.packTextBlockData(title, padding=titlePadding),
             formatters.packBuildUpBlockData(listBlock, padding=listPadding)], padding=blockPadding))
    ...
    def _getFootNoteBlock(self, noteType):
        ...
        return [
         formatters.packImageTextBlockData(title='', desc=desc, img=img, imgPadding=formatters.packPadding(left=4, top=3), txtGap=-4, txtOffset=20, padding=formatters.packPadding(left=59, right=20))]
    ...
    def _getBonuses(self):
        ...
        if item is None:
            ...
            vehPostProgressionBonusLevels = {step.action.getTechName():step.getLevel() for step in item.postProgression.iterUnorderedSteps() if step.action.actionType == ACTION_TYPES.MODIFICATION if step.action.actionType == ACTION_TYPES.MODIFICATION}
...
class CrystalBlockConstructor(VehicleTooltipBlockConstructor):
    ...
    def construct(self):
        ...
        if 0 < current < limit:
            block.append(formatters.packBlockDataItem(linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_EPIC_PROGRESS_BLOCK_LINKAGE, data={'value': current, 'maxValue': limit}, padding=formatters.packPadding(top=-5, left=82, bottom=8), blockWidth=304))
        return (
         block, linkage)
...
class StatusBlockConstructor(VehicleTooltipBlockConstructor):
    ...
    def construct(self):
        ...
        if isClanLock or isDisabledInRoaming:
            ...
            return (
             block, result and result.get('operationError') is not None)
