...
class EpicSkillBaseTooltipData(BlocksTooltipData):
    ...
    def __constructInactiveStateBlock(self):
        return [
         formatters.packTitleDescBlock(title=text_styles.critical(backport.text(R.strings.epic_battle.metaAbilityScreen.Ability_not_unlocked())), desc=text_styles.standard(backport.text(R.strings.epic_battle.metaAbilityScreen.how_to_activate())), gap=2)]
...
class EpicSkillExtendedTooltip(EpicSkillBaseTooltipData):
    ...
    def _packBlocks(self, skillID, specLevel=None):
        ...
        bodyBlocks = [
         formatters.packTextBlockData(text=text_styles.middleTitle(('{}{}').format(backport.text(R.strings.epic_battle.abilityInfo.properties()), backport.text(R.strings.common.common.colon()))))]
