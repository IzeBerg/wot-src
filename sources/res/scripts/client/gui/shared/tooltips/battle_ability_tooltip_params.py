...
COLOR_SCHEME = (
 text_styles.critical, text_styles.warning, text_styles.statInfo)
...
class MultipliedValuesMixin(DisplayValuesMixin):
    ...
    def _getDisplayParams(cls, curEq, compareEq, eqsRange, param):
        ...
        return (curValue * multiplier,
         (compValue - curValue) * multiplier,
         max(getattr(eq, param) * multiplier for eq in eqsRange))
...
class ReciprocalValuesMixin(DisplayValuesMixin):
    ...
    def _getDisplayParams(cls, curEq, compareEq, eqsRange, param):
        ...
        return (curValue, compValue - curValue,
         max((1 / val if val != 0 else float('inf')) for val in (getattr(eq, param) for eq in eqsRange)))
...
class ShellStunValuesMixin(DisplayValuesMixin):
    ...
    def _getDisplayParams(cls, curEq, compareEq, eqsRange, param):
        ...
        return (
         curValue,
         compValue - curValue,
         max((shell.stun.stunDuration if shell.hasStun else 0) for shell in shellsRange))
...
class NumericLabelMixin(DisplayLabelMixin):
    ...
    def _formatDeltaStringInternal(cls, delta, value, higherIsBetter=True, unitLocalization=None, signForValue=_getSignForValue, signForDelta=_getSignForDelta):
        ...
        if deltaStr is None:
            ...
            return ('({}) {}').format(deltaStr, valueStr)
    ...
    def createBattleAbilityTooltipRenderers(self, skillInfo, currentLevel, specLevel, bodyBlocks):
        ...
        for tooltipIdentifier in curLvlEq.tooltipIdentifiers:
            ...
            if tooltipInfo is None:
                logger = logging.getLogger(__name__)
                logger.error('[ERROR] createBattleAbilityTooltipRenderers: Failed to find tooltipInfo %(ttid)s for %(us)s (%(name)s).', {'ttid': tooltipIdentifier, 'us': specLvlEq.userString, 
                   'name': specLvlEq.name})
                continue
