...
def getEndingInfoBlockBonusToken(finishDate, blockWidth, padding=None):
    ...
    return formatters.packBuildUpBlockData([
     infoBlock], blockWidth=blockWidth, padding=padding)
...
def getEventCoinsBlock(coinsCount, title, width, left=30):
    ...
    return packBuildUpBlockData([
     valueBlock], blockWidth=width, padding=formatters.packPadding(left=-4))
...
def getHowToGetInfoBlock(title, description, blockWidth=None, linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_LINKAGE):
    return formatters.packBuildUpBlockData([
     ...
     formatters.packTextBlockData(text=text_styles.main(text_styles.concatStylesToMultiLine(backport.text(description))))], blockWidth=blockWidth, linkage=linkage)
...
class EventCostTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self):
        ...
        blocks = [
         formatters.packTextBlockData(text_styles.highTitle(backport.text(R.strings.tooltips.event.cost.title()))),
         formatters.packTextBlockData(text_styles.main(backport.text(R.strings.tooltips.event.cost.description())))]
...
class EventInterrogationTooltipData(BlocksTooltipData):
    ...
    def getStatus(self, ctx):
        status = ctx.status
        if status == self.LAST and ctx.numberMemories >= ctx.totalMemories:
    ...
    def _getEventMemoriesBlock(self, numberMemories, totalMemories):
        ...
        return packBuildUpBlockData([
         block], linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE, blockWidth=self._BLOCK_WIDTH)
    ...
    def _getNotEnoughCoinsBlock(self):
        items = [
         formatters.packAlignedTextBlockData(text=text_styles.critical(backport.text(R.strings.tooltips.event.interrogation.not_enough.title())), align=BLOCKS_TOOLTIP_TYPES.ALIGN_LEFT),
         formatters.packAlignedTextBlockData(text=text_styles.main(backport.text(R.strings.tooltips.event.interrogation.not_enough.description())), align=BLOCKS_TOOLTIP_TYPES.ALIGN_LEFT)]
...
class EventKeyInfoTooltipData(BlocksTooltipData):
    ...
    def _getTitleBlock(self):
        return formatters.packBuildUpBlockData([
         formatters.packTextBlockData(text=text_styles.highTitle(backport.text(R.strings.tooltips.event.key.title())))], blockWidth=self._BLOCK_WIDTH)

    def _getIconBlock(self, icon):
        return formatters.packBuildUpBlockData([
         formatters.packImageBlockData(img=icon, align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)], blockWidth=self._BLOCK_WIDTH, align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILD_BLOCK_BLUE_BIG_LINKAGE)

    def _getDescriptionBlock(self):
        return formatters.packBuildUpBlockData([
         formatters.packTextBlockData(text=text_styles.main(backport.text(R.strings.tooltips.event.key.description())))], blockWidth=self._BLOCK_WIDTH)
    ...
    def _getCoinsBlock(self):
        ...
        return packBuildUpBlockData([
         eventCoinsBlock], linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE, padding=formatters.packPadding(left=30), blockWidth=self._BLOCK_WIDTH)
    ...
    def _getVariant(self, title, value):
        if value > 0:
            return (backport.text(title, value=text_styles.middleTitle(formatAccrualValue(value))),)
        return ()
    ...
    def _getAccrualInfoBlock(self, efficiency, boss, daily):
        ...
        return formatters.packBuildUpBlockData([
         formatters.packTextBlockData(text=text_styles.middleTitle(backport.text(R.strings.tooltips.event.key.what_accrual()))),
         formatters.packTextBlockData(text=text_styles.main(text_styles.concatStylesToMultiLine(*variants)))], blockWidth=self._BLOCK_WIDTH)
...
class EventEfficientInfoTooltipData(BlocksTooltipData):
    ...
    def _getTitleBlock(self):
        return formatters.packBuildUpBlockData([
         formatters.packTextBlockData(text=text_styles.highTitle(backport.text(R.strings.tooltips.event.efficient.title())))], blockWidth=self._BLOCK_WIDTH)

    def _getIconBlock(self, icon):
        return formatters.packBuildUpBlockData([
         formatters.packImageBlockData(img=icon, align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)], blockWidth=self._BLOCK_WIDTH, align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)

    def _getDescriptionBlock(self):
        return formatters.packBuildUpBlockData([
         formatters.packTextBlockData(text=text_styles.main(backport.text(R.strings.tooltips.event.efficient.description())))], blockWidth=self._BLOCK_WIDTH)
...
class EventSelectorWarningTooltip(BlocksTooltipData):
    ...
    def _getHeaderBlock(self):
        return formatters.packBuildUpBlockData([
         formatters.packTextBlockData(text=text_styles.middleTitle(backport.text(R.strings.tooltips.battleTypes.event.header()))),
         formatters.packTextBlockData(text=text_styles.main(backport.text(R.strings.tooltips.battleTypes.event.body())))])
...
class HalloweenQuestsPreviewTooltipData(QuestsPreviewTooltipData):
    ...
    def _getQuestList(self, vehicle):
        return sorted([ q for q in self._questController.getQuestForVehicle(vehicle) if (isHalloween(q.getGroupID()) or isHalloweenAFK(q.getGroupID())) and q.isAvailable()[0] and not q.isCompleted()
    ...
    def _getAllQuestsCompleteBlock(self):
        ...
        return formatters.packBuildUpBlockData([
         formatters.packTextBlockData(text=text_styles.main(backport.text(R.strings.tooltips.event.header.quests.empty(), hour=time.strftime('%H', localtime), min=time.strftime('%M', localtime))), padding=formatters.packPadding(left=20, top=-10, bottom=10))], linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE)
...
class EventShellBlockToolTipData(ShellBlockToolTipData):
    ...
    def _addBottomHint(self, items, showBasicData, leftPadding, rightPadding):
        ...
        items.append(formatters.packBuildUpBlockData([
         formatters.packTextBlockData(text_styles.standard(_ms(backport.msgid(textID), bold=boldText)), padding=lrPaddings)], padding=rPadding, linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE))