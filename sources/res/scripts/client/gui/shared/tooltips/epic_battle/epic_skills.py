...
class EpicSkillBaseTooltipData(BlocksTooltipData):
    ...
    def _constructHeader(self, skillLevel):
        block = [
         formatters.packTitleDescBlock(title=text_styles.highTitle(skillLevel.name), gap=-5),
         formatters.packImageBlockData(img=backport.image(R.images.gui.maps.icons.epicBattles.skills.c_176x176.dyn(skillLevel.icon)()), width=128, height=128, padding=formatters.packPadding(left=123, top=-10))]
        return block
