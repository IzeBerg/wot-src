...
class HeaderItemPacker(object):
    ...
    def pack(self, data):
        ...
        return [
         formatters.packImageTextBlockData(titleStr, img=self.__icon, txtPadding={'left': 18, 'top': 9, 'bottom': 1}, padding={'left': 2, 'top': 7}, titleAtMiddle=True)]
...
class TotalDetectionItemPacker(DetectionItemPacker):
    ...
    def pack(self, data):
        ...
        if value:
            packer = formatters.packTextParameterBlockData
            blocks = [
             packer(value=backport.getIntegralFormat(value), name=backport.text(R.strings.postbattle_screen.tooltip.spotted.totalTanks()))]
            blockToInsert = formatters.packBuildUpBlockData(blocks)
            items.append(blockToInsert)
        return items
