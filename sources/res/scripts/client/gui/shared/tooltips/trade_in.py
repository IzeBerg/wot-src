...
class TradeInInfoTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        return [
         ...
         formatters.packBuildUpBlockData(blocks=self.__getInfo(levels=args[0]), linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE),
    ...
    def __getInfo(levels):
        ...
        return [
         formatters.packImageTextBlockData(img=backport.image(R.images.gui.maps.icons.library.prem_small_icon()), imgPadding=formatters.packPadding(top=4), title=text_styles.main(levelsText), txtPadding=formatters.packPadding(left=3), padding=formatters.packPadding(left=62, bottom=18)),
    ...
    def __getFooter():
        return [
         ...
         formatters.packAlignedTextBlockData(text=text_styles.neutral(_withTradeInFromTo()), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(left=34))]
...
class TradeInStateNotAvailableData(ToolTipBaseData):
    ...
    def getDisplayableData(self, *args, **kwargs):
        ...
        return {'header': backport.text(R.strings.tooltips.tradeInUnavailable.header()), 'body': body}
