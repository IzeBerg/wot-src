...
class ReserveTooltipData(StrongholdTooltipData):
    ...
    def getDisplayableData(self, *args, **kwargs):
        ...
        infoLevel = '%s %s' % (int2roman(reserve.getLevel()),
         i18n.makeString(FORTIFICATIONS.STRONGHOLDRESERVE_TOOLTIP_LEVEL))
...
class SettingsButtonTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        items.append(formatters.packBuildUpBlockData([
         formatters.packTextBlockData(text_styles.highTitle(TOOLTIPS.HEADER_MENU_HEADER)),
         formatters.packTextBlockData(text_styles.standard(TOOLTIPS.HEADER_MENU_DESCRIPTION))]))
        serverBlocks = list()
        ...
        if not constants.IS_CHINA:
            items.append(formatters.packBuildUpBlockData([
             formatters.packTextBlockData(text_styles.middleTitle(TOOLTIPS.HEADER_MENU_PLAYERSONSERVER)),
             formatters.packImageTextBlockData('', serversStats, RES_ICONS.MAPS_ICONS_LIBRARY_CREW_ONLINE, imgPadding=formatters.packPadding(-4, -10), padding=formatters.packPadding(5))]))
        return items
...
class MissionsToken(BlocksTooltipData):
    ...
    def __packTitleBlock(self, token):
        return formatters.packImageTextBlockData(title=text_styles.highTitle(makeString(TOOLTIPS.MISSIONS_TOKEN_HEADER, name=makeString(token.getUserName()))), img=token.getImage(TOKEN_SIZES.MEDIUM), txtPadding={'top': 14, 'left': 11, 'right': 5})
...
class NYGiftStamps(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        blocks = [
         formatters.packTextBlockData(text_styles.highTitle(title), padding=compressPadding),
         formatters.packImageBlockData(image, align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER),
