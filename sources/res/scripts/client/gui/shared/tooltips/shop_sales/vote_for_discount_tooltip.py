...
class VoteForDiscountTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if available:
            ...
            return [
             formatters.packBuildUpBlockData(blocks=[
              titleBlock], blockWidth=345, padding=formatters.packPadding(bottom=-10)),
             formatters.packBuildUpBlockData(blocks=[
              maximumFreeBlock, progressBlock], blockWidth=BLOCK_WIDTH - 20, padding=formatters.packPadding(bottom=-28))]
        ...
        return [
         formatters.packBuildUpBlockData(blocks=[
          availabilityBlock], blockWidth=BLOCK_WIDTH + 20, padding=formatters.packPadding(bottom=-11)),
         formatters.packBuildUpBlockData(blocks=[
          footerBlock], blockWidth=BLOCK_WIDTH, padding=formatters.packPadding(bottom=-35, top=-50))]