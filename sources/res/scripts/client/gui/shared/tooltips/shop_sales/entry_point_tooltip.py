...
def _getPhaseName(isEnabled, eventPhase):
    if isEnabled:
        return _PHASE_NAME.get(eventPhase, 'disabled')
    return 'disabled'
...
class ShopSalesEntryPointTooltipData(BlocksTooltipData):
    ...
    def __packMainBlocks(self, eventPhase, timeLeft):
        return [
         formatters.packBuildUpBlockData(self.__packHeaderBlocks(eventPhase, timeLeft) + [self.__packDescriptionBlock(eventPhase)])]
    ...
    def __packHeaderBlocks(self, eventPhase, timeLeft):
        ...
        headerContentBlock = formatters.packBuildUpBlockData(blocks=(
         titleBlock, self.__packSubTitleBlock(eventPhase, timeLeft)), padding=formatters.packPadding(top=-(50 + _ONE_SIDE_PADDING), left=_ONE_SIDE_PADDING))
        return [
         headerBackgroundBlock, headerContentBlock]
    ...
    def __packFooterBlock(self, eventPhase, savedCount):
        if not self.__shopSales.isEnabled:
            ...
            if eventPhase == EventPhase.FINISHED:
                if savedCount:
    ...
    def __packImgTextBlock(txt, img=None, imgSize=37, imgPadding=-8, txtPadding=-6, textStyle=None, usePadding=True):
        if img is not None:
            imgBlocksList = [
             formatters.packImageBlockData(img=img, padding=formatters.packPadding(top=imgPadding, left=imgPadding), width=imgSize, height=imgSize)]
        else:
