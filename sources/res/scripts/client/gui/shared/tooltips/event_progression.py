...
class EventProgressionQuestsTooltipData(BlocksTooltipData):
    ...
    def __formatDescription(self, quests):
        ...
        if not self._eventProgression.modeIsAvailable() or cycle is None:
            return ''
        if self._eventProgression.isNeedAchieveMaxLevelForDailyQuest():
            ...
            if all(q.isCompleted() for q in quests) and self._eventProgression.isDailyQuestsRefreshAvailable():
