import logging, time, weakref, BigWorld, SoundGroups
...
def onAccountShowGUI(ctx):
    ...
    for func in [
     __runItemsCacheSync, __validateInventoryVehicles, __validateInventoryOutfit, __validateInventoryTankmen,
     __cacheVehicles, __runQuestSync, __runSettingsSync, __processEULA, __notifyOnSyncComplete,
     __requestDossier, __initializeHangarSpace, __initializeHangar, __processWebCtrl, __processElen]:
        try:
            if BigWorld.player():
                ts = time.time()
