...
RELATIVE_POWER_PARAMS = (
 'avgDamage', 'avgPiercingPower', 'stunMinDuration',
 'stunMaxDuration', 'reloadTime', AUTO_RELOAD_PROP_NAME, 'reloadTimeSecs',
 'clipFireRate', DUAL_GUN_CHARGE_TIME, 'turretRotationSpeed', 'turretYawLimits',
...
def getCommonParam(state, name):
    return {'state': state, 'paramID': name}
...
class VehParamsBaseGenerator(object):
    ...
    def _makeExtraParamVO(self, param):
        return (
         getCommonParam(HANGAR_ALIASES.VEH_PARAM_RENDERER_STATE_EXTRA, param.name), 0)
