...
class VehiclesAreaMarkerHandler(object):
    ...
    def __init__(self, parent):
        ...
        self._parent = parent
        self._vehiclesAreaMarker = {'ID': {}, 'matrix': {}, 'lastCheckTime': {}, 'prevData': {}}
    ...
    def __addVehicleMarker(self, markerData, vehicleID, params, createMatrix=True):
        ...
        if createMatrix:
            ...
            if vehicle:
                vehiclesMatrix.update({vehicleID: {'matrix': vehicle.matrix, 'inAoI': True}})
            else:
                vehiclesMatrix.update({vehicleID: {'matrix': math_utils.createRTMatrix(ypr, position), 
                               'inAoI': False}})
            vehiclePrevData.update({vehicleID: {'position': position, 'ypr': Math.Vector3(ypr)}})
        ...
        if vehicleID not in markerIDs:
            ...
            if vehicle:
                ...
                if not vehiclesMatrix[vehicleID]['inAoI']:
                    vehiclesMatrix.update({vehicleID: {'matrix': matrix, 'inAoI': True}})
