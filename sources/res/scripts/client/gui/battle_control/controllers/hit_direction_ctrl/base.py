...
class RTSHitDirection(HitDirection):
    ...
    def show(self, hitData, extend=False):
        if self.__canShowHit(hitData.getDamagedID()):
            return super(RTSHitDirection, self).show(hitData, extend)
        return 0
    ...
    def __canShowHit(self, vID=None):
        if self.__ctrlMode in CTRL_MODE_NAME.COMMANDER_MODES:
            return True
        return self.__sessionProvider.dynamic.vehicleChange.currentVehicleID == vID