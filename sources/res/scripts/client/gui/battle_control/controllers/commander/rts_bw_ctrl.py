...
import aih_constants, BigWorld, GenericComponents, CGF, BattleReplay
...
BaseInfo = namedtuple('BaseInfo', ('baseID', 'team', 'transform', 'radius'))
...
class RTSBWController(CallbackDelayer, IRTSBWController):
        ...
        for baseInfo in self.__controlPoints:
            if baseInfo.baseID == baseID and baseInfo.team == team:
                ...
                for veh in vehicles:
                    ...
                    elif distance < surroundingRadius:
                        surrounding.append(veh.id)

                break

        return (
         invaders, surrounding)
    ...
    def __updateCursor(self):
        ...
        if vehicles.hasSelection and (not self.__isMouseOverUI or minimapInteraction):
        ...
        if cursorId == self.__currentCursor:
            return
        else:
