...
class _VehicleTarget(_IVectorTarget):
    ...
    @property
    def position(self):
        if self._vehicle is None or not self._vehicle.initialized:
            return
...
class OrderVector(object):
    ...
    def update(self, order=None, position=None, target=None, **extra):
        if order in (RTSOrder.GO_TO_POSITION, RTSOrder.FORCE_GO_TO_POSITION, RTSOrder.DEFEND_THE_BASE, RTSOrder.STOP,
         RTSOrder.CAPTURE_THE_BASE, RTSOrder.RETREAT) or extra and extra.get('isPursuit'):
