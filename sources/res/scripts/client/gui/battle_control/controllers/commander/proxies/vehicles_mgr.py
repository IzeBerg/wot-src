...
class ProxyVehiclesManager(_ProxiesContainer, IProxyVehiclesManager, CallbacksSetByID):
    ...
    def startMoving(self):
        ...
        if not self.canQueueCommand(COMMAND_NAME.MOVE, isRequireAll=True, vehicles=selectedVehicles):
            return True
        return self.__commandProducer.startMoving(selectedVehicles, self.__models, controlPoint)
    ...
    def getAliveNearCursor(self, criteria):
        ...
        if focusedVeh is not None and focusedVeh.isAlive and focusedVeh.initialized and criteria(focusedVeh):
            return focusedVeh.id
        else:
            return
    ...
    def __onVehicleReloading(self, vehicleID, timeLeft, baseTime):
        ...
        if gunSettings is None:
            ...
            self[vehicleID].reloadingState = (
             time.time(), timeLeft, baseTime)
    ...
    def canShowVehicleStatus--- This code section failed: ---

 L. 689         0  LOAD_GLOBAL           0  'VehicleConditions'
                3  LOAD_ATTR             1  'DEAD_CONDITION'
                6  LOAD_FAST             0  'self'
                9  LOAD_ATTR             2  '__vehicleConditions'
               12  LOAD_FAST             1  'vehicleID'
               15  BINARY_SUBSCR    
               16  COMPARE_OP            6  in
               19  POP_JUMP_IF_FALSE    26  'to 26'

 L. 690        22  LOAD_GLOBAL           3  'False'
               25  RETURN_END_IF    
             26_0  COME_FROM            19  '19'

 L. 691        26  LOAD_FAST             0  'self'
               29  LOAD_ATTR             4  '__sessionProvider'
               32  LOAD_ATTR             5  'dynamic'
               35  LOAD_ATTR             6  'rtsCommander'
               38  LOAD_ATTR             7  'vehicles'
               41  STORE_FAST            2  'vehProxyMgr'

 L. 692        44  LOAD_FAST             2  'vehProxyMgr'
               47  LOAD_CONST               None
               50  COMPARE_OP            9  is-not
               53  POP_JUMP_IF_FALSE   107  'to 107'

 L. 693        56  LOAD_FAST             2  'vehProxyMgr'
               59  LOAD_ATTR             9  'get'
               62  LOAD_FAST             1  'vehicleID'
               65  CALL_FUNCTION_1       1  None
               68  STORE_FAST            3  'vehProxy'

 L. 694        71  LOAD_FAST             3  'vehProxy'
               74  POP_JUMP_IF_FALSE   103  'to 103'
               77  LOAD_FAST             3  'vehProxy'
               80  LOAD_ATTR            10  'isEnabled'
               83  JUMP_IF_FALSE_OR_POP   106  'to 106'
               86  LOAD_FAST             3  'vehProxy'
               89  LOAD_ATTR            11  'isSupply'
               92  UNARY_NOT        
               93  JUMP_IF_FALSE_OR_POP   106  'to 106'
               96  LOAD_FAST             3  'vehProxy'
               99  LOAD_ATTR            12  'isAllyBot'
              102  RETURN_END_IF    
            103_0  COME_FROM            93  '93'
            103_1  COME_FROM            83  '83'
            103_2  COME_FROM            74  '74'
              103  LOAD_GLOBAL           3  'False'
              106  RETURN_VALUE     
            107_0  COME_FROM            53  '53'

 L. 695       107  LOAD_GLOBAL           3  'False'
              110  RETURN_VALUE     

Parse error at or near `RETURN_END_IF' instruction at offset 102
        ...
        if vehicleId in self.__vehicleConditions:
            if vehicleId in self.__extDestroyTimersStatusInfo:
                ...
                for status in vehicleStatus:
                    ...
                    elif status == VehicleConditions.ON_FIRE_CONDITION:
        ...
        for vehicle in self._proxies.itervalues():
            if vehicle.isControllable:
                ...
                BigWorld.loadResourceListBG((
                 assembler,), makeCallbackWeak(self.__onResourcesLoaded, vehicle.id))
...
def _updateHighlights():
    ...
    if player.userSeesWorld() and player.isCommander():
        player.updateAllHighlights()# Decompile failed :(