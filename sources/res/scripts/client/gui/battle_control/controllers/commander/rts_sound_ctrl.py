...
MANNER_TO_VOICE_EVENT = {RTSManner.DEFENSIVE: R4_SOUND.R4_MANNER_ADAPTIVE, RTSManner.SCOUT: R4_SOUND.R4_MANNER_SCOUT, 
...
TRIGGER_GET_DESTINATION_INSIDE_ENEMY_BASE_AFTER = {
 R4_SOUND.R4_ALLY_CAPTURE_BASE,
 R4_SOUND.R4_ALLY_CAPTURE_NEUTRAL_BASE}
...
class RTSSoundController(IRTSSoundController, CallbackDelayer):
    ...
    def __getVehicleClassTag(self, vID):
        ...
        if vehicleProxy is not None:
            return vehicleProxy.vehicleClassTag
        else:
            return
    ...
    def __vehicleSelectionHasSameManner(self):
        ...
        for v in self.__selectedVehicles:
            if not manner:
                manner = v.manner
            elif v.manner and manner != v.manner:
                return False
