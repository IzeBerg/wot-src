...
import enum, logging, BigWorld, Keys, Math, math_utils
...
class MappedKeys(object):
    ...
    def isKey(cls, event, mappedKey, checkDown=True):
        if checkDown and not event.isKeyDown():
            return False
        return cls.isMappedTo(event.key, mappedKey)
    ...
    def isModifierDown(cls, mappedMod):
        ...
        if mods is None:
            ...
            if mods not in cls.__modifierKeyInfoMap:
                return False
            modKeyInfo = cls.__modifierKeyInfoMap[mods]
            if not (BigWorld.isKeyDown(modKeyInfo.key1) or BigWorld.isKeyDown(modKeyInfo.key2)):
                return False
            return True
