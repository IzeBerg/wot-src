...
class _Command(ICommand):
    __slots__ = ('_order', '_isExecuted', '_commandResult', '_companions', '__entity',
                 '__groupID')
...
class MoveCommand(_Command):
    __slots__ = ('__position', '__heading', '__target', '_order', '__baseID', '__baseTeam',
                 '__marker', '__queuePos', '__navMeshQueryID', '__vehicleModel')
    ...
    def canQueueCommand(self, commandTypeName):
        if self._order in (RTSOrder.DEFEND_THE_BASE, RTSOrder.CAPTURE_THE_BASE):
            return False
        return super(MoveCommand, self).canQueueCommand(commandTypeName)
