...
import math, logging, Keys, Math, BigWorld, ResMgr, math_utils
...
class _VehicleGhostMarker(_Marker, IMarkerEnabled, IMarkerVisible):
    ...
    _ARROW_LENGTH = 2.0
    _VISIBLE_POS_STATES = (
     RTSCommandQueuePosition.PREVIEW, RTSCommandQueuePosition.SINGLE, RTSCommandQueuePosition.LAST)
    _HALF_PI = math.pi / 2
    _TWO_PI = 2.0 * math.pi
    __slots__ = ('__arrow', '__model', '__heading', '__enabled', '__vehID', '__isVisible',
                 '__overwatch')
...
PathMarkerStyle = namedtuple('PathMarkerStyle', [
 'modelPath',
 'modelIntersection',
 'scaleMarker',
 'scalePath',
...
class _LineMarker(_Marker):
    ...
    def _getLine(self):
        return (
         self._position + self.overTerrainOffset, self._targetPosition + self.overTerrainOffset)
...
class AttackMarker(_LineMarker):
    ...
    def _getLine(self):
        ...
        attackerOverTerrainOffset = (self._isInQueue() or self).overTerrainOffset if 1 else pathMarkerStyle.offset
        targetOverTerrainOffset = self.overTerrainOffset if self._targetVehProxy.isVisible else pathMarkerStyle.offset
        return (
         self._position + attackerOverTerrainOffset, self._targetPosition + targetOverTerrainOffset)


OperationCircleConfig = namedtuple('OperationCircleConfig', [
 'visualPath',
 'overTerrainHeight',
 'operationRadius',
