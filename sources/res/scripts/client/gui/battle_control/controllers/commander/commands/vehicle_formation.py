...
import logging, math, BigWorld, Math, math_utils, ResMgr
from Math import Vector3
from gui.shared.gui_items.Vehicle import VEHICLE_CLASS_NAME
_logger = logging.getLogger(__name__)
_HP_TO_WEIGHT_RATIO = 30
RTS_VEHICLE_FORMATION_CFG = 'gui/rts_vehicle_formation.xml'
_Slot = namedtuple('Slot', ('heavyTank', 'atSPG', 'heavyAtSPG', 'mediumAtSPG', 'lightAtSPG',
                            'SPG', 'heavySPG', 'mediumSPG', 'lightSPG', 'mediumTank',
                            'lightTank', 'posOffset'))
...
def getBotPathFinalHeading(bot, path):
    if path is None:
        ...
        return getPathFinalHeading(bot.position, path)
