import logging, typing, math, BigWorld, Math, TriggersManager
...
class RTSGunSettings(object):
    ...
    def shellsInClip(self):
        if self.__currentShellCD not in self.__ammo:
            return 0
        return self.__ammo[self.__currentShellCD][1]
...
class _ProxyVehicle(IProxyVehicle):
    ...
    @property
    def isPlayer(self):
        return not any((self.isCommander, self.isObserver, self.isSupply, self.isBot))
    ...
    @property
    def appearance(self):
        if self._bwVehicle is not None:
            return self._bwVehicle.appearance
        else:
            return
    ...
    @property
    def matrix(self):
        if self._bwVehicle is not None and self._bwVehicle.isStarted:
            return self._bwVehicle.matrix
        else:
            return self._lastKnownMatrix

    @property
    def yaw(self):
        if self._bwVehicle is not None and self._bwVehicle.isStarted:
            return self._bwVehicle.yaw
        else:
            return 0
    ...
    def position(self):
        if self._bwVehicle is not None and self._bwVehicle.isStarted:
            return self._bwVehicle.position
        else:
    ...
    def isPlayerVehicle(self):
        if self._bwVehicle is not None and self._bwVehicle.isStarted:
            return self._bwVehicle.isPlayerVehicle
        else:
            return False
    ...
    def aimParams(self):
        if self._bwVehicle is not None and self._bwVehicle.isStarted:
            return self._bwVehicle.getAimParams()
        else:
            return (0.0, 0.0)
    ...
    def activeGunIndex(self):
        if self._bwVehicle is not None and self._bwVehicle.isStarted:
            return self._bwVehicle.activeGunIndex
        else:
            return 0
    ...
    def serverGunAngles(self):
        if self._bwVehicle is not None and self._bwVehicle.isStarted:
            return self._bwVehicle.getServerGunAngles()
        else:
            return (0.0, 0.0)
    ...
    def unmarkAsTarget(self, instigator):
        if instigator in self._targetedMarks:
            ...
            if activeTargetInstigator in (instigator, None):
                self._activeTargetMask = (
                 None, self.getHighestTargetMark())
            self._container.handleVehicleTargeted(self.id, self.getActiveTargetMark())
            self._updateLastKnownPositionMarker()
        return
    ...
    def getHighestTargetMark(self):
        if self._targetedMarks:
            return max(self._targetedMarks.values(), key=lambda order: order.value)
        else:
            return

    def getActiveTargetMark(self):
        if self._activeTargetMask is not None:
            return self._activeTargetMask[1]
        else:
            return self._activeTargetMask
    ...
    def __getOrderData(self):
        ...
        if commanderData is None:
            ...
            return commanderData.orderData
    ...
    def updateRTSOrder(self, order=None, manner=None, position=None, target=None, heading=None, baseID=None, baseTeam=None, isForce=False, **extra):
        if not self.isEnabled and not isForce:
            ...
            def getValidVector(value, valueFromBase):
                if value is None and valueFromBase is None:
                    return
                else:
                    return Math.Vector3(value or valueFromBase)
...
def makeProxyVehicle(vID, container):
    if isControllableBot(vID):
        return _ControllableProxyVehicle(vID, container)
    return _ProxyVehicle(vID, container)
...
def _getRosterIndex(vID, sessionProvider=None):
    ...
    if vehicleCD in rosterVehicles:
        return rosterVehicles.index(vehicleCD)
    return -1