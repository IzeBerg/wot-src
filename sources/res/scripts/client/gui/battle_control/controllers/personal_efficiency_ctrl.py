...
_AGGREGATED_DAMAGE_EFFICIENCY_TYPES = (
 _ETYPE.DAMAGE, _ETYPE.ASSIST_DAMAGE, _ETYPE.BLOCKED_DAMAGE, _ETYPE.STUN, _ETYPE.SUPPLY_DAMAGE)
_FEEDBACK_EVENT_TYPE_TO_PERSONAL_EFFICIENCY_TYPE = {_FET.PLAYER_DAMAGED_HP_ENEMY: (
   ...
   _FET.PLAYER_DAMAGED_SUPPLY_ENEMY: (
                                    _ETYPE.SUPPLY_DAMAGE, _DamageEfficiencyInfo)}
...
def createEfficiencyCtrl(setup, feedback, vehStateCtrl):
    if avatar_getter.isPlayerCommander():
        return PersonalEfficiencyControllerCommander(setup.arenaDP, feedback, vehStateCtrl)
