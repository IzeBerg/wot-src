import httplib, time, urlparse, typing, logging, Event
...
class WGNPRequestController(IWGNPRequestController):
    ...
    def addEmail(self, email, showWaiting=False):
        ...
        response = yield self._request(url, method='POST', queryParams={'type': 'pow'}, postData={'game': WG_GAMES.TANKS, 'email': email}, auth=True, proofOfWorkURL='./personal/api/v2/account/email/create/challenge/?type=pow', showWaiting=showWaiting)
        if response.isSuccess():
    ...
    def _onEmailAdded(self, email):
        self._updateStatus(STATUS_TYPES.email.name(), {'email': email, 'state': 'email_sent'})
...
def isEmailAddingNeeded(response):
    return response.data and response.data.get('state') in ('no_active_request', 'spa_email_already_taken',
