...
class LootBoxSystemEntryPoint(ViewImpl, ICarouselEventEntry):
    ...
    def getIsActive(state):
        boxType = LootBoxSystemEntryPoint.__lootBoxes.eventName
        if not LootBoxSystemEntryPoint.__lootBoxes.getBoxesIDs(boxType):
            _logger.error('There is no boxes with %s type, check LootBox config.', boxType)
            return False
        return LootBoxSystemEntryPoint.__lootBoxes.isActive and (any(state.isInPreQueue(preQueue) for preQueue in _ENABLED_PRE_QUEUES) or state.isInUnit(PREBATTLE_TYPE.SQUAD))
