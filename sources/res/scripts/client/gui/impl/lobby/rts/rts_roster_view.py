...
class RosterView(ViewImpl, ICarouselEnvironment):
    __slots__ = ('_filter', '_popoverCloseCallback', '_rosterConfig', '__battleType',
                 '_slotCache', '_filteredIntCDs', '__blur', '__isClosed', '__closeConfirmationHelper',
                 '_selectedSupplySlot', '_selectedVehicleSlot', '_isModified', '_vehicleSpecs',
                 '_mannersSettings')
    ...
    def _createBackportTooltip(self, event):
        if not event.hasArgument('specialAlias'):
            ...
            if specialAlias == TOOLTIPS_CONSTANTS.RTS_VEHICLE_PARAMETERS:
                ...
                if specialAlias == TOOLTIPS_CONSTANTS.RTS_VEHICLE_RESTRICTIONS:
                    ...
                    specialArgs = [role]
                elif specialAlias in [TOOLTIPS_CONSTANTS.RTS_ROSTER_SHELL,
                 TOOLTIPS_CONSTANTS.RTS_ROSTER_MODULE,
                 TOOLTIPS_CONSTANTS.RTS_CAROUSEL_VEHICLE]:
    ...
    def _getRosterForSave(self):
        ...
        return {'vehicles': vehicles, 'supplies': supplies, 
           'rtsManners': manners}
    ...
    def _getCurrentVehicleRoster(self):
        return [ (slot.getIsEmpty() or slot.vehicle.getIntCD)() if 1 else 0 for slot in self.viewModel.carousel.getVehicleSlots()
               ]

    def _getCurrentSupplyRoster(self):
        return [ (slot.getIsEmpty() or slot.supply.getIntCD)() if 1 else 0 for slot in self.viewModel.carousel.getSupplySlots()
               ]
    ...
    def _getVehicleIntCDForSlot(self, slotId):
        ...
        slot = slots[slotId]
        if slot is not None and not slot.getIsEmpty():
            return slot.vehicle.getIntCD()
        else:
            return
    ...
    def __onCacheResync(self, reason, diff):
        ...
        if reason in (CACHE_SYNC_REASON.SHOP_RESYNC, CACHE_SYNC_REASON.DOSSIER_RESYNC,
         CACHE_SYNC_REASON.CLIENT_UPDATE) or GUI_ITEM_TYPE.VEHICLE in diff:
            with self.viewModel.transaction() as (model):
                self._invalidateAll(model)
