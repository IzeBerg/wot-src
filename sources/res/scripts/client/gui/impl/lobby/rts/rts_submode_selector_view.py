...
SUB_MODE_ORDER = (
 SubModesEnum.STRATEGIST1X1, SubModesEnum.TANKER, SubModesEnum.STRATEGIST1X7)
...
class RTSSubModeSelectorView(ViewImpl):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.lobby.rts.SubModeTooltip():
            ...
            if contentID == R.views.lobby.rts.StrategistCurrencyTooltip():
    ...
    def _isRosterComplete(self, bonusType):
        ...
        if len(itemCDs) != len(rosterItems):
            return False
        return True
    ...
    def _getCurrentSubMode(self):
        ...
        if not self._rtsController.isCommander():
            return SubModesEnum.TANKER
        return SubModesEnum.STRATEGIST1X7
    ...
    def _getAvailability(self, subMode):
        ...
        if submodeState == SubModeStateEnum.FREE:
            return (SubModeStatus.FREE, R.invalid())
        return (
         SubModeStatus.AVAILABLE, resource.ready_for_battle())
    ...
    def _getSubmodeState(self, subMode):
        ...
        if settings.isWarmupEnabled():
            return (SubModeStateEnum.FREE, R.invalid())
        return (
         SubModeStateEnum.NORMAL, R.invalid())
    ...
    def _getPrbData(self, subMode):
        if subMode is SubModesEnum.STRATEGIST1X1:
            return PREBATTLE_ACTION_NAME.RTS_1x1
        return PREBATTLE_ACTION_NAME.RTS
