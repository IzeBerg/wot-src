...
_SMART_ROUND_PARAMS = ('damage', 'piercingPower', 'bombDamage', 'shellsCount', 'shellReloadingTime',
 'reloadMagazineTime', 'reloadTime', 'dispertionRadius', 'aimingTime', 'weight',
...
def colorize(paramStr, state):
    ...
    if stateType == PARAM_STATE.BETTER:
        return ('%(green_open)s{}%(green_close)s').format(paramStr)
    return paramStr
