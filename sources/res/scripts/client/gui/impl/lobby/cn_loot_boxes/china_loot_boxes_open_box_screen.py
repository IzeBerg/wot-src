...
class ChinaLootBoxesOpenBoxScreen(ViewImpl):
    __slots__ = ('__compensationTooltipData', '__rewards', '__tooltipData', '__boxType',
                 '__vehicle', '__bonuses', '__notifier', '__isClosed', '__osBit',
                 '__isOpenInProgress')
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.cn_loot_boxes.tooltips.ChinaLootBoxesCompensationTooltip():
            return ChinaLootBoxesCompensationTooltip(self.__compensationTooltipData)
        return super(ChinaLootBoxesOpenBoxScreen, self).createToolTipContent(event=event, contentID=contentID)
    ...
    def __getTimeLeft(self):
        ...
        if not boxesLeft:
            return self.__cnLootBoxes.getTimeLeftToResetPurchase() + time_utils.ONE_SECOND
        return 0
