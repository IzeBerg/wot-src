...
class NewYearChallengeTournament(NyHistoryPresenter):
    ...
    def __makeAndFillChallengeCardModel(self, model, token, questInfo):
        ...
        if quest is None:
            ...
            cardModel.setState((quest.isCompleted() or CardState).ACTIVE if 1 else (_getQuestCompleted(*questInfo) or CardState).JUSTCOMPLETED if 1 else CardState.COMPLETED)
