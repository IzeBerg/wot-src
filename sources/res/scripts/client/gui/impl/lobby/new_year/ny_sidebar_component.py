...
class NYSidebar(NyHistoryPresenter):
    __slots__ = ('__tabsController', '__controllers', '__currentTab', '__currentViewName',
                 '__soundsManager', '__regionName', '__notifier')
    ...
    def _getEvents(self):
        ...
        return events + (
         (
          self._nyController.sacksHelper.onUpdated, self.__onSacksUpdated),
         (
    ...
    def __getSoundStateValue(self):
        if self.__currentViewName is NyWidgetTopMenu.GLADE:
            ...
            if self.__currentViewName is NyWidgetTopMenu.CHALLENGE:
                camObj = CHALLENGE_TAB_TO_CAMERA_OBJ.get(self.__currentTab)
