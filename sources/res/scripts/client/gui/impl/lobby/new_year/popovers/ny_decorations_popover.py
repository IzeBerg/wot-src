...
class NyDecorationsPopover(PopOverViewImpl):
    ...
    def __hangToy(self, toy, slotID):
        ...
        if result.success:
            g_eventBus.handleEvent(events.NewYearEvent(events.NewYearEvent.ON_TOY_INSTALLED, ctx={'toyID': toyID, 'slotID': slotID, 'atmoshereBonus': newToyAtmosphere - oldToyAtmosphere}), scope=EVENT_BUS_SCOPE.LOBBY)
            if toy is not None:
