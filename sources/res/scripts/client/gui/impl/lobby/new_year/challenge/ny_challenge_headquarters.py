...
class NewYearChallengeHeadquarters(NyHistoryPresenter):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        return super(NewYearChallengeHeadquarters, self).createToolTipContent(event, contentID)

    def _getEvents(self):
        evnts = super(NewYearChallengeHeadquarters, self)._getEvents()
        return evnts + (
         (
          self.viewModel.onStartAction, self.__onActionClick),
         (
          self.viewModel.onShowStory, self.__onStoryClick),
         (
          self.__celebrityController.onCelebActionTokensUpdated, self.__onCelebActionTokensUpdated))

    def _getListeners(self):
        return (
         (
          events.NyDogEvent.HOVER_IN, self.__onHoverInDog, EVENT_BUS_SCOPE.DEFAULT),
         (
          events.NyDogEvent.HOVER_OUT, self.__onHoverOutDog, EVENT_BUS_SCOPE.DEFAULT))
    ...
    def __fillModel(self):
        with self.viewModel.transaction() as (tx):
            ...
            for guestName in GuestsQuestsTokens.GUESTS_ALL:
                ...
                if storiesTokens:
                    completedStoriesTokens = self.__celebrityController.getAllReceivedTokens([
                     guestName], [GuestQuestTokenActionType.STORY])
