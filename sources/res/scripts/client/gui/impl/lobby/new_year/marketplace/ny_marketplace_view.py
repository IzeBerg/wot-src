...
class NyMarketplaceView(SceneRotatableView, SelectableLogicPresenter):
    __slots__ = ('_tooltips', '__settings', '__needToResetAppearance', '__needResetBloor',
                 '__cameraCallbackId', '__blur', '__tabName')
    ...
    def initialize(self, *args, **kwargs):
        ...
        self.__settings.setResource(max([ (r, self._nyController.currencies.getResouceBalance(r.value)) for r in Resource
                                        ], key=itemgetter(1))[0])
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.lobby.new_year.tooltips.NyMarketCardTooltip():
            return NyMarketCardTooltip(str(event.getArgument('kitState')), str(event.getArgument('kitName')), str(event.getArgument('currentTabName')), int(event.getArgument('kitIndex')), str(event.getArgument('currentResource')))
        return super(NyMarketplaceView, self).createToolTipContent(event, contentID)
