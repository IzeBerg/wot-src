    ...
    def __onSelectedDecoration(self):
        if not self.viewModel.getHasDecoration():
            ...
            if result.success and self.viewStatus == ViewStatus.LOADED:
                g_eventBus.handleEvent(events.NewYearEvent(events.NewYearEvent.ON_TOY_INSTALLED, ctx={'toyID': toyID, 'slotID': self.__slotID, 'atmoshereBonus': points}), scope=EVENT_BUS_SCOPE.LOBBY)
                self.__isSelected = toyID != INVALID_TOY_ID
