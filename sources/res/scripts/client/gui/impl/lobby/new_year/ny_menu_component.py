...
_NAVIGATION_ALIAS_VIEWS = {NyWidgetTopMenu.GLADE: ViewAliases.GLADE_VIEW, NyWidgetTopMenu.MARKETPLACE: ViewAliases.MARKETPLACE_VIEW, 
...
class NYMainMenu(NyHistoryPresenter):
    __slots__ = ('__tabsController', '__soundsManager', '__currentView', '__widgetHelper',
                 '__widgetFriendHelper', '__lockForCustomAnimation', '__notifier')
    ...
    def createPopOverContent(self, event):
        if event.contentID == R.views.lobby.new_year.popovers.NyResourcesConvertPopover():
            return NyResourcesConvertPopover()
        return super(NYMainMenu, self).createPopOverContent(event)
    ...
    def __getSoundStateValue(self):
        if self.__friendsService.friendHangarSpaId is not None:
            return
        else:
