...
class NyGladeView(SceneRotatableView, SelectableLogicPresenter):
    __slots__ = ('__currentObject', '__toysList', '__notifier', '__blur', '__hoveredObjectName',
                 '__levelUpInProgress', '__resourceCollectingLock', '__cameraManager',
                 '__totalAtmPoints')
    ...
    def _getEvents(self):
        return super(NyGladeView, self)._getEvents() + (
         (
    ...
    def __getCurrentState(self):
        ...
        if tutorialState is not None:
            return tutorialState
        else:
            return TutorialStates.INTRO
