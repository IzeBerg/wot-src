...
class MainView(ViewImpl):
    ...
    def _getListeners(self):
        return super(MainView, self)._getListeners() + (
         (
          CameraRelatedEvents.CAMERA_ENTITY_UPDATED, self.__handleSelectedEntityUpdated, EVENT_BUS_SCOPE.DEFAULT),
         (
    ...
    def __registerSubModels(self):
        ...
        self.__componentPresenters.extend([
         NYMainMenu(self.viewModel.mainMenu, self),
         NYSidebar(self.viewModel.sidebar, self)])
    ...
    def __onBackButtonUpdated(self, ctx):
        ...
        if self.viewModel is None:
            ...
            if not ctx.get('isVisible') and self.viewModel.backButton.getIsVisible() or self.__backCallback is None:
