...
class NyFriendsView(NyHistoryPresenter):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        return super(NyFriendsView, self).createToolTipContent(event, contentID)
    ...
    def _getEvents(self):
        return super(NyFriendsView, self)._getEvents() + (
         (
    ...
    def __updateFriendList(self, model):
        friendList = sorted(self._friendsService.friendList.itervalues(), key=lambda data: (
         -data.get(FriendsDataKeys.HANGAR_VISITS, 0),
         -data.get(FriendsDataKeys.ATM_POINTS, 0),
         data.get(FriendsDataKeys.NAME, 0)))
