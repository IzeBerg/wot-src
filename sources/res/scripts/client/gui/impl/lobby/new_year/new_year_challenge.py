...
class NewYearChallenge(HistorySubModelPresenter):
    ...
    def _getCallbacks(self):
        return (
         (
          'cache.vehsLock', self.__onLocksUpdate),)

    def _getListeners(self):
        return (
         (
          events.NewYearEvent.ON_PRE_SWITCH_VIEW, self.__onPreSwitchViewEvent, EVENT_BUS_SCOPE.LOBBY),)

    def _getEvents(self):
        return (
         (
          self.viewModel.onLeaveIntroScreen, self.__onLeaveIntroScreen),
         (
    ...
    def __startLifeCycleWatcher(self):
        self.__viewLifecycleWatcher.start(dependency.instance(IAppLoader).getApp().containerManager, [
         DiscountPopoverHandler(self.__onDiscountPopoverOpened, self.__onDiscountPopoverClosed)])
    ...
    def __iterQuests(self):
        ...
        return sorted((CelebrityQuestTokenParts.getDayNum(questId), group.getActiveQuest(tokens), group.regularQuests, group.bonusQuest.getBonuses()) for questId, group in self.__celebrityController.questGroups.items() if group.isValid)
