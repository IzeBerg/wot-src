...
_YEARLY_REWARDS_BONUSES_ORDER = (
 BonusTypes.BADGE_SUFFIX, BonusTypes.BADGE, BonusTypes.ACHIEVEMENT,
 BonusTypes.STYLE_PROGRESS, BonusTypes.OFFER, BonusTypes.CRYSTAL, BonusTypes.STYLE)
_YEARLY_REWARD_META_BONUSES_ORDER = (
 BonusTypes.RENT_VEHICLE, BonusTypes.STYLE_3D_PROGRESS, BonusTypes.TOKEN,
 BonusTypes.BADGE_SUFFIX, BonusTypes.BADGE, BonusTypes.CREW, BonusTypes.ACHIEVEMENT,
 BonusTypes.OFFER, BonusTypes.CRYSTAL, BonusTypes.STYLE)
...
class Comp7YearlyTokensUIPacker(TokenBonusUIPacker):
    ...
    def _getTokenBonusType(cls, tokenID, complexToken):
        if tokenID.startswith(cls._COMP7_STYLE_TOKEN):
            return cls._COMP7_STYLE_TOKEN
        return super(Comp7YearlyTokensUIPacker, cls)._getTokenBonusType(tokenID, complexToken)
    ...
    def __packStyleTooltip(cls, _, token):
        return TooltipData(tooltip=None, isSpecial=True, specialAlias=None, specialArgs=[
         cls.__getStyleId(token), backport.text(R.strings.usa_vehicles.dyn(cls._VEHICLE)())], isWulfTooltip=False)
...
class Comp7OfferBonusUIPacker(BaseBonusUIPacker):
    ...
    @classmethod
    def _getToolTip(cls, bonus):
        if cls._getGiftCount(bonus) > 0:
            return [
             createTooltipData(isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.COMP7_SELECTABLE_REWARD)]
        return []
    ...
    def __getOfferGiftCount(cls, offer, count):
        if offer is not None:
            return offer.giftTokenCount * count
        else:
            return count
...
def packYearlyRewardCrew(bonus):
    ...
    return packQuestBonuses([
     crewReward], bonusPacker=getComp7YearlyBonusPacker(), order=_YEARLY_REWARDS_BONUSES_ORDER)
...
class Comp7YearlyCustomizationsBonus(CustomizationsBonus):
    ...
    def isShowInGUI(self):
        ...
        if not item:
            return False
        return not self.getC11nItem(item).is3D