...
class CrewBooksView(ViewImpl):
    ...
    MIN_ROLE_LEVEL = 100
    __slots__ = ('__vehicle', '__bookGuiItemList', '__selectedBookGuiItem', '__selectedTankmanVM',
                 '__selectedBookIndex', '__selectedBookIndex', '__invalidTypes')
    ...
    def __getBackportTooltipData(self, event):
        ...
        if tooltipId == CrewBooksTooltips.TOOLTIP_TANKMAN:
            args = [
             int(event.getArgument('invID'))]
        elif tooltipId == CrewBooksTooltips.TOOLTIP_TANKMAN_SKILL:
            args = [
             event.getArgument('skillName'), int(event.getArgument('tankmanInvId'))]
        elif tooltipId == CrewBooksTooltips.TOOLTIP_TANKMAN_NEW_SKILL:
            args = [
             int(event.getArgument('tankmanInvId'))]
        else:
            args = [
             self.__vehicle.invID]
