...
class EventModeSelectorItem(ModeSelectorLegacyItem):
    ...
    def _getDisabledTooltipText(self):
        if not self.__eventController.isModeActive():
            return backport.text(R.strings.event.popover.harrierEvent.tooltip.header())
        return super(EventModeSelectorItem, self)._getDisabledTooltipText()
