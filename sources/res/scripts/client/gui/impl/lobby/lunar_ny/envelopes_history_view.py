...
_ENVELOPE_TYPES = (
 EnvelopeTypes.PREMIUM_PAID, EnvelopeTypes.PAID, EnvelopeTypes.FREE)
_COLUMN_TYPES = (
 ColumnType.PLAYERNAME, ColumnType.RECEIVEDENVELOPES, ColumnType.RECEIVEDGOLD, ColumnType.SENTINRESPONSE)
_COLUMN_SORTING_ORDER = (
 ColumnSortingOrder.ASC, ColumnSortingOrder.DESC)
_DEFAULT_COLUMN_TYPES = ColumnType.RECEIVEDENVELOPES
PlayerGiftHistoryData = namedtuple('PlayerGiftHistoryData', (
 'playerID',
 'nickName',
 'isOnline',
 'clanTag',
...
class EnvelopesHistoryView(ViewImpl):
    __slots__ = ('__window', '__blur', '__userInfoHelper', '__currentEnvelopeType',
                 '__currentColumnType', '__currentSortingOrder', '__currentPageNumber',
                 '__currentUsersInfo', '__countRequester', '__dataRequester', 'isBlurActive')
    ...
    def __sortCurrentUsersByUserName(self):
        ...
        if not sortingOrder:
            self.__currentUsersInfo = sorted(self.__currentUsersInfo, key=lambda user: (
             user['nickName'] is None or user['nickName'] == '', _nickNameSortKeyFun(user['nickName'])), reverse=sortingOrder)
        else:
    ...
    def __getPlayerStatus(self, contact):
        if contact is None:
            ...
            if contact.isFriend() and USER_TAG.SUB_PENDING_OUT in contact.getTags():
                return PlayerStatus.FRIENDREQUESTINPROGRESS
            return PlayerStatus.NOTFRIEND
