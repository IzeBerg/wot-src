import logging, time, typing, json
...
class AdventCalendarMainView(AdventCalendarParentView):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        elif contentID in (R.views.lobby.new_year.tooltips.NyGiftMachineTokenTooltip(),
         R.views.lobby.advent_calendar.tooltips.AdventCalendarBigLootBoxTooltip()):
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if progressionReward and progressionReward.getRewardType() == RewardType.CREW_MEMBER:
                ...
                toolTipData = TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.ADVENT_CALENDAR_TANKMAN_NOT_RECRUITED, specialArgs=[
                 crewMemberToken, state, doorsToOpenAmount, True])
            if not toolTipData:
    ...
    def _getListeners(self):
        return (
         (
          events.AdventCalendarV2Event.PROGRESSION_REWARD_VIEWED,
          self.__updateProgressionRewards, EVENT_BUS_SCOPE.LOBBY),
         (
          events.AdventCalendarV2Event.CHANGE_BLUR_STATUS,
          self.__changeBlurState, EVENT_BUS_SCOPE.LOBBY),
         (
          LobbySimpleEvent.SHOW_LOOT_BOX_VIEW, self.__onClose, EVENT_BUS_SCOPE.LOBBY))
    ...
    def _registerSubModels(self):
        return [
         AdventCalendarNYResourceBalance(self.viewModel.balance, self, self.__onClose, self.__updateModel, self._isConvertPopoverAvailable)]
    ...
    def __getOneDoorInfo(self, doorId):
        default = (
         0, DoorState.CLOSED, 0, 0)
        if self.__adventCalendarV2Ctrl.getMaxDayNumbers >= doorId > 0:
            return (doorId, self.__adventCalendarV2Ctrl.getDoorState(doorId),
