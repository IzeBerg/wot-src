...
class AdventCalendarBonusPacker(BonusUIPacker):
    ...
    def pack(self, bonus):
        if bonus.getName() == 'battleToken' and isinstance(bonus, LootBoxTokensBonus):
            return AdventCalendarLootBoxPacker().pack(bonus)
        return super(AdventCalendarBonusPacker, self).pack(bonus)
        ...
        for tokenID in bonus.getTokens().iterkeys():
            if tokenID.startswith(RECRUIT_TMAN_TOKEN_PREFIX):
                tooltipData.append(TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.ADVENT_CALENDAR_TANKMAN_NOT_RECRUITED, specialArgs=[
                 crewMemberToken, ProgressionState.REWARD_RECEIVED, 0, True]))
