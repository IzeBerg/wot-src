...
class HangarAmmunitionSetupView(BaseHangarAmmunitionSetupView, IGlobalListener):
    ...
    def _createOptionalDevicesAssistantPanel(self):
        if self._lobbyContext.getServerSettings().isOptionalDevicesAssistantEnabled() and self._wotPlusController.isEnabled() and self.prbEntity and self.prbEntity.getQueueType() == QUEUE_TYPE.RANDOMS and self._canDisplayForVehicle():
            return OptionalDevicesAssistantView(self.viewModel.optionalDevicesAssistant, self._vehItem.getItem())
        else:
            return
