...
def getVehicleHWConsumablesLayoutPrice(vehicle):
    result = sum([ item.getBuyPrice() for item in vehicle.hwConsumables.layout.getItems() if not item.isInInventory and item not in vehicle.hwConsumables.installed
                 ], ITEM_PRICE_ZERO)
    return result
...
class HWBuyAndInstallConsumablesProcessor(Processor):
    ...
    def _setupPlugins(self):
        self.addPlugins((
         ...
         proc_plugs.MoneyValidator(getVehicleHWConsumablesLayoutPrice(self._vehicle).price, byCurrencyError=False),
         HWConsumablesInstallValidator(self._vehicle)))
...
class HalloweenInteractor(BaseInteractor):
    ...
    def showExitConfirmDialog(self):
        ...
        result = yield wg_await(showHWTankSetupExitConfirmDialog(items=self.getChangedList(), vehicle=self.getItem(), fromSection=self.getName(), startState=(self.getChangedList() or BuyAndExchangeStateEnum).BUY_NOT_REQUIRED if 1 else None))
