...
class QuestsTooltipBuilder(TooltipBuilder):
    ...
    def __formRequiredVehiclesText(self):
        ...
        if minLvl == maxLvl:
            return backport.text(self._RES_ROOT.questsView.chapter.postprogression.disabled.doFuture.samelvl.body(), lvl=int2roman(minLvl))
        else:
            return backport.text(self._RES_ROOT.questsView.chapter.postprogression.disabled.doFuture.body(), minLvl=int2roman(minLvl), maxLvl=int2roman(maxLvl))
