...
class CrewMemberTankChangeDialog(BaseCrewDialogTemplateView):
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        self.setSubView(Placeholder.TITLE, SimpleTextTitle(str(backport.text(R.strings.dialogs.crewMemberTankChange.title(), tankName=self._vehicleCurrent.descriptor.type.shortUserString)), [
         ImageSubstitution(self._vehicleCurrent.typeBigIconResource(), 'icon', -12, titleIconMargin, -22, titleIconMargin)]))
        contentIconMargin = -12 if self._vehicleNew.isElite else -16
        self.setSubView(Placeholder.CONTENT, SimpleTextContent(str(backport.text(R.strings.dialogs.crewMemberTankChange.desc(), tankLevel=int2roman(self._vehicleNew.level), icon='%(icon)s', tankName=self._vehicleNew.descriptor.type.shortUserString)), [
         ImageSubstitution(getType44x44IconResource(self._vehicleNew.type, self._vehicleNew.isElite)(), 'icon', -11, contentIconMargin, -13, contentIconMargin)]))
