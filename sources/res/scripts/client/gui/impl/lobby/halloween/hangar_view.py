...
class HangarView(HangarSelectableView, HangarEventView):
    ...
    _MAX_REWARDS_NUM = 8
    __slots__ = ('__layoutID', '__isVehicleCameraReadyForC11n', '__vehiclesController',
                 '__ammoPanel', '__isUnitJoiningInProgress', '__timer', '__difficultyPanel',
                 '__keysCounterPanel')
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if tooltipId == TOOLTIPS_CONSTANTS.EVENT_BAN_INFO:
                specialArgs = [
                 self._afkController.banExpiryTime]
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.halloween.tooltips.DailyTooltip():
            return DailyWidgetTooltip()
