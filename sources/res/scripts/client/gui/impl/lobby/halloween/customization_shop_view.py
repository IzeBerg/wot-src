...
ShopItemData = namedtuple('ShopItemData', ['index', 'c11Item', 'bundleID', 'entityID'])
ShopBundlePriceInfo = namedtuple('ShopBundlePriceInfo', ['isEnough', 'priceType', 'amount', 'playerAmount'])
...
class CustomizationShopView(HangarSelectableView, HangarEventView, EventSystemEntity):
    ...
    def __getC11ItemVehicles(self, c11Item):
        if not c11Item.descriptor.filter:
            return []
        return list(itertools.chain.from_iterable(node.vehicles for node in c11Item.descriptor.filter.include if node.vehicles))
    ...
    def __getVehicle(self, shopItem):
        ...
        if vehicleCD is None:
            ...
            return self.itemsCache.items.getItemByCD(vehicleCD)
