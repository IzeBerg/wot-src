...
class MetaView(HangarEventView):
    ...
    def createToolTip(self, event):
        if event.contentID == _R_BACKPORT_TOOLTIP:
            ...
            if tooltipId == MetaViewModel.INTERROGATION_TOOLTIP_ID:
                ...
                window = BackportTooltipWindow(createTooltipData(tooltip='', isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.EVENT_INTERROGATION, specialArgs=[
                 tokenID,
                 InterrogationCtx(status=status, phase=int(rewardBox.questConditions.phaseNum), difficulty=int(rewardBox.questConditions.difficultyLevel))]), self.getParentWindow())
                window.load()
                return window
            if tooltipId == MetaViewModel.FINAL_INTERROGATION_TOOLTIP_ID:
                ...
                window = BackportTooltipWindow(createTooltipData(tooltip='', isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.EVENT_INTERROGATION, specialArgs=[
                 tokenID,
                 InterrogationCtx(status=EventInterrogationTooltipData.LAST, numberMemories=min(currMemories, maxMemories), totalMemories=maxMemories)]), self.getParentWindow())
                window.load()
                return window
    ...
    def __fillModel(self):
        ...
        if self.__selectedBox in rewardBoxesConfig:
            ...
            ctx = {'boxIndex': MetaViewModel.INTRO_VIDEO_ITEM_ID, 'isRewardOpened': True}
    ...
    def __processOpenBox(self, rewardBoxID, isQuest):
        ...
        if success:
            ...
            self.__playVideo(rewardBoxID, callbackOnUnload=lambda boxID=rewardBoxID: (self.soundEventChecker.unlockPlayingSounds(restore=False),
             self.__showReward(boxID)))
    ...
    def __playVideo(self, box, callbackOnUnload=None):
        showBrowserOverlayView(GUI_SETTINGS.lookup(box), alias=VIEW_ALIAS.BROWSER_OVERLAY, webHandlers=createHangarWebHandlers(), forcedSkipEscape=True, callbackOnLoad=lambda : (
         self.uiLogger.videoStarted(),
         WWISE.WW_eventGlobal(EventHangarSound.PLAY_VIDEO if callbackOnUnload is None else EventHangarSound.DECODED_REWARD)), callbackOnUnload=lambda : (
         self.uiLogger.videoStopped(box),
         callbackOnUnload and callbackOnUnload()))
