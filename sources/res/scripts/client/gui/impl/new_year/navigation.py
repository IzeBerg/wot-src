...
class _NavigationState(object):
    ...
    def resetObject(self, aliasName, force):
        if force and self.__currentObject is None:
            ...
            if not self.__isFriendChange(aliasName):
                return
...
OBJECTS_BY_VIEW_MAIN = {ViewAliases.GLADE_VIEW: CustomizationObjects.ALL + (
                          AdditionalCameraObject.RESOURCES, AdditionalCameraObject.UNDER_SPACE), 
   ViewAliases.CELEBRITY_VIEW: AdditionalCameraObject.CELEBRITY_GROUP, 
   ViewAliases.MARKETPLACE_VIEW: (
                                AdditionalCameraObject.MARKETPLACE,), 
   ViewAliases.GIFT_MACHINE: AdditionalCameraObject.GIFT_MACHINE_GROUP}
OBJECTS_BY_VIEW_FRIEND = {ViewAliases.FRIEND_GLADE_VIEW: CustomizationObjects.ALL + (AdditionalCameraObject.RESOURCES,
                                 AdditionalCameraObject.UNDER_SPACE), 
   ViewAliases.FRIEND_CELEBRITY_VIEW: (
                                     AdditionalCameraObject.CELEBRITY,)}
OBJECTS_BY_VIEW = OBJECTS_BY_VIEW_MAIN.copy()
OBJECTS_BY_VIEW.update(OBJECTS_BY_VIEW_FRIEND)
VIEWS_WITH_AUTO_ROUTING = (
...
FRIENDS_VIEWS = (
 ViewAliases.FRIEND_GLADE_VIEW, ViewAliases.FRIEND_INFO_VIEW, ViewAliases.FRIEND_CELEBRITY_VIEW)
...
class NewYearNavigation(object):
    ...
    def createBackButtonText(cls):
        ...
        if viewName:
            return R.strings.ny.backButton.dyn(viewName)()
        return R.invalid()
    ...
    def switchTo(cls, objectName, instantly=False, viewAlias=None, withFade=False, doAutoRouting=False, *args, **kwargs):
        if cls._navigationState.isSceneInFade:
            ...
            if not cls.__friendsService.friendHangarSpaId:
                hangarType = HangarTypes.MAIN_HANGAR if 1 else HangarTypes.FRIEND_HANGAR
                viewAlias = viewAlias or cls.getViewAliasByObjectName(objectName=objectName, hangarType=hangarType)
            cls.__switchUI(viewAlias=viewAlias, isObjectSwitched=True, *args, **kwargs)
            cls.onObjectStateChanged()
    ...
    def getViewAliasByObjectName(cls, objectName, hangarType=HangarTypes.MAIN_HANGAR):
        ...
        if views:
            return first(views)
        if hangarType == HangarTypes.MAIN_HANGAR:
            return ViewAliases.GLADE_VIEW
        return ViewAliases.FRIENDS_VIEW
    ...
    def __switchUI(cls, viewAlias=ViewAliases.GLADE_VIEW, tabName=None, *args, **kwargs):
        ...
        if currentObject is None:
            ...
            if cls._navigationState.isCurrentView(viewAlias):
