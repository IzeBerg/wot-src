...
class LobbyOverlaysManager(IOverlaysManager):
    __slots__ = ('__weakref__', '__gui', '__suspended', '__opened', '__isSuspended',
                 '__callbackID', '__observer', '__condition')
    __gui = dependency.descriptor(IGuiLoader)
    ...
    def __onWindowStatusChanged(self, uniqueID, newState):
        ...
        if window is None:
            ...
            if window.windowFlags != WindowFlags.WINDOW | WindowFlags.WINDOW_FULLSCREEN:
                return
