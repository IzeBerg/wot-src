...
class RTS1x7CurrencyValidator(BaseActionsValidator):
    ...
    def _validate(self):
        if self.__rtsController.isCommander() and not self.__rtsController.hasEnoughCurrency(ARENA_BONUS_TYPE.RTS):
            return ValidationResult(False, PREBATTLE_RESTRICTION.RTS_NOT_ENOUGH_CURRENCY, None)
        else:
            return super(RTS1x7CurrencyValidator, self)._validate()
...
class RTSSubmodeBreakerValidator(BaseActionsValidator):
    ...
    def _validate(self):
        ...
        if not settings.isSubmodeEnabled(bonusType):
            return ValidationResult(False, PRE_QUEUE_RESTRICTION.RTS_SUBMODE_NOT_AVAILABLE, None)
        else:
            return super(RTSSubmodeBreakerValidator, self)._validate()
...
class RTSActionsValidator(PreQueueActionsValidator):
    ...
    def _createStateValidator(self, entity):
        ...
        validators = [baseValidator, RTSSubmodeBreakerValidator(entity), RTSPrimeTimeValidator(entity),
         RTS1x7CurrencyValidator(entity), RTS1x7RosterValidator(entity), RTSBattleValidator(entity)]
