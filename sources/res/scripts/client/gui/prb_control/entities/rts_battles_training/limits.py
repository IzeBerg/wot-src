...
class CommanderInTeamIsValid(AbstractTeamIsValid):
    ...
    def check(self, rosters, team, teamLimits):
        ...
        if self.__isAllCommanders(accountsInfo):
            return (False, 'commanders')
        return (
         True, '')
...
class RtsTrainingLimits(LimitsCollection):
    ...
    def __init__(self, entity):
        super(RtsTrainingLimits, self).__init__(entity, (
         TrainingVehicleIsValid(),), (
         TeamNoPlayersInBattle(PREBATTLE_TYPE.RTS_TRAINING),
         TeamIsValid(),
         CommanderInTeamIsValid()))