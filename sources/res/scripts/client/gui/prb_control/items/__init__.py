...
class FunctionalState(object):
    ...
    def doLeaveToAcceptInvite(self, prbType=0):
        ...
        if self.isInLegacy() or self.isInUnit():
            return True
        return self.entityTypeID not in self.__getQueueTypeByPrbType(prbType)
    ...
    def __getQueueTypeByPrbType(self, prbType):
        prbToQueue = {PREBATTLE_TYPE.SQUAD: [
                                QUEUE_TYPE.RANDOMS], 
           PREBATTLE_TYPE.UNIT: [
                               QUEUE_TYPE.UNITS], 
           PREBATTLE_TYPE.EVENT: [
                                QUEUE_TYPE.EVENT_BATTLES, QUEUE_TYPE.EVENT_BATTLES_2], 
