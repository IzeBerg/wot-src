...
from gui.wot_anniversary.wot_anniversary_sound import WOT_ANNIVERSARY_WELCOM_WINDOW_SOUND
...
class WotAnniversaryWelcomeView(ViewImpl):
    ...
    _COMMON_SOUND_SPACE = WOT_ANNIVERSARY_WELCOM_WINDOW_SOUND
    ...
    def __showWelcomeOverlayVideo(self):
        showBrowserOverlayView(self.__wotAnniversaryCtrl.getUrl(WotAnniversaryUrls.WELCOME_VIDEO), VIEW_ALIAS.BROWSER_OVERLAY, callbackOnClose=self.destroyWindow)
    ...
    def __markVisited(self):
        if not self.__settingsCore.serverSettings.getSection(SETTINGS_SECTIONS.WOT_ANNIVERSARY_STORAGE).get(WotAnniversaryStorageKeys.WOT_ANNIVERSARY_WELCOME_SHOWED):
            self.__settingsCore.serverSettings.setSections([
             SETTINGS_SECTIONS.WOT_ANNIVERSARY_STORAGE], {WotAnniversaryStorageKeys.WOT_ANNIVERSARY_WELCOME_SHOWED: True})
