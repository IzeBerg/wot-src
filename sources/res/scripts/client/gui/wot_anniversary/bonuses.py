...
class WotAnniversaryGroupsBonusPacker(GroupsBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        return [
         createTooltipData(isSpecial=True, specialArgs=[
          bonus.getValue()])]
...
class CustomizationProgressBonusUIPacker(BaseBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        tooltipData = [
         TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.TECH_CUSTOMIZATION_ITEM_AWARD, specialArgs=CustomizationTooltipContext(itemCD=bonus.getC11nItem().intCD, showInventoryBlock=True))]
        return tooltipData
...
def packBonusModelAndTooltipDataFromQuests(quests, bonusModelsList, tooltipData, packer=None):
    ...
    for quest in quests:
        ...
        if quest.isCompleted():
            ...
            if isTokenQuestUnlocked(quest, WOT_ANNIVERSARY_LOGIN_UNLOCK_TOKEN):
