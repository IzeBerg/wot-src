...
import BigWorld, Math, Keys, math_utils
...
class HangarCameraYawFilter(object):
    ...
    def getNextYaw(self, currentYaw, targetYaw, delta):
        ...
        if not self.__cycled:
            ...
            elif delta > 0.0 and self.__end < nextYaw <= self.__start:
                nextYaw = self.__end
            elif delta < 0.0 and self.__start > nextYaw >= self.__end:
                nextYaw = self.__start
...
class HangarCameraManager(object):
    ...
    def __updateCameraByMouseMove(self, dx, dy, dz):
        if self.__cam is None:
            ...
            if self.__cam != BigWorld.camera() and not self.__handleInactiveCamera:
                return
            from gui.ClientHangarSpace import hangarCFG
            cfg = hangarCFG()
