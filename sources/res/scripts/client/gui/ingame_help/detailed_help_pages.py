...
def buildPagesData(ctx):
    ...
    spgHelpScreenGuiTypes = (
     ARENA_GUI_TYPE.TRAINING,) + ARENA_GUI_TYPE.RANDOM_RANGE
...
def buildDualGunPages(headerTitle):
    ...
    _addPage(pages, headerTitle, backport.text(R.strings.ingame_help.detailsHelp.dualGun.volley_fire.title()), text_styles.mainBig(backport.text(R.strings.ingame_help.detailsHelp.dualGun.volley_fire())), [
     chargeKey], backport.image(R.images.gui.maps.icons.battleHelp.dualGunHelp.volley_fire()))
    _addPage(pages, headerTitle, backport.text(R.strings.ingame_help.detailsHelp.dualGun.quick_fire.title()), text_styles.mainBig(backport.text(R.strings.ingame_help.detailsHelp.dualGun.quick_fire())), [
     shootKey], backport.image(R.images.gui.maps.icons.battleHelp.dualGunHelp.quick_fire()))
    return pages
...
def buildSPGPages(headerTitle, tempPagesSettings):
    pages = []
    spgTempPages = [
     'differentTrajectories', 'miniMap', 'tracers', 'intuition']
...
def buildSPGPagesForOtherVehicle(headerTitle, tempPagesSettings):
    ...
    tempPagesLeftToShow = tempPagesSettings.get(SPG_HELP_PAGES_LEFT_TO_SHOW, 0)
    pageNames = [
     'miniMap', 'tracers', 'lamp', 'intuition']
