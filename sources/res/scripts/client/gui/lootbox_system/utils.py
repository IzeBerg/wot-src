...
def getPreferredBox(category='', lootBoxes=None):
    if category:
        return first(lootBoxes.getBoxes(lambda b: b.getCategory() == category))
    return first(lootBoxes.getActiveBoxes(lambda b: b.getInventoryCount())) or first(lootBoxes.getActiveBoxes())
...
def getInfoPageSettings(setting, lootBoxes=None):
    ...
    if eventSetting is not None:
        ...
        return GUI_SETTINGS.lootboxSystem.get('infoPage').get(setting).get('default')
...
def getLootboxStatisticsKey(boxID=None, itemsCache=None):
    ...
    if box is not None:
        ...
        return getPreferredBox().getStatsName()
