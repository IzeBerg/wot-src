...
def _createCreditsModel(reusable, getter):
    ...
    replayRecords = (
     (
      creditsRecords.additional,),
     (
      creditsRecords.main,))
    creditFields = (
     getter.credits.getCreditExpenses(),
     getter.credits.getCreditEarnings(reusable))
    modelTypes = (
     DetailsStatsModel.EXPENSES_SUBGROUP_TYPE,
     DetailsStatsModel.EARNINGS_SUBGROUP_TYPE)
    models = _addCurrencySubgroups(creditFields, replayRecords, modelTypes, CurrencyGroupBlockIndexes.CURRENCY_DETAILS)
    records = [
     common.createRecord(getter.credits.getEarnedCredits(), (creditsRecords.main,))]
    records += models
...
def _createCrystalsModel(reusable, getter):
    ...
    if crystalsRecords is None:
        ...
        modelTypes = (
         ...
         DetailsStatsModel.EARNINGS_SUBGROUP_TYPE)
        crystalFields = (
         getter.crystals.getCrystalsExpenses(),
         getter.crystals.getCrystalsEarnings())
        records = [
         common.createRecord(getter.crystals.getEarnedCrystals(), (crystalsRecords.main,))]
