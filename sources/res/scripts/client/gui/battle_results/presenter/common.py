...
def getCrystalsRecords(reusable):
    ...
    if not crystalsRecords or not reusable.personal.haveCrystalsChanged():
        return
...
def getTotalXPToShow(reusable, records, hasPremium):
    if hasPremium:
        return reusable.personal.getXPToShow()[0]
...
def getTotalCreditsToShow(reusable, records, hasPremium):
    if hasPremium:
        return reusable.personal.getCreditsToShow()[0]
...
def _createPremiumBonusModel(reusable):
    if reusable.personal.isPremiumPlus or reusable.personal.isPremiumPlus:
        return None
    return getCurrencies((
     reusable.personal.getCreditsDiff(), reusable.personal.getXPDiff()), (
