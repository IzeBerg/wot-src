...
def _makeStrWithThousandSlash(left, right):
    return (
     _makeStrWithThousand(left, digitLimit=3), _makeStrWithThousand(right, digitLimit=3))
...
class AllyTeamTitle(_BaseTeamTitle):
    ...
    def _convert(self, result, reusable):
        if reusable.personal.isCommander:
            return backport.text(_RTS_STR_PATH.team.stats.battleResources())
        return backport.text(self._BASE_STR_PATH.ownTeam())
...
class EnemyTeamTitle(_BaseTeamTitle):
    ...
    def _convert(self, result, reusable):
        ...
        if reusable.personal.isCommander:
            return backport.text(self._BASE_STR_PATH.enemyTeam())
        else:
            return backport.text(_RTS_STR_PATH.team.stats.enemyStrategist(), playerName=commanderInfo.getFullName())
...
class PersonalVehiclesRTSStatsBlock(PersonalVehiclesBaseStatsBlock):
    ...
    def _createStatsBlock(self, reusable=None):
        ...
        if reusable.personal.isCommander:
            return RTS1x7VehicleStatValuesBlock()
        return RTSVehicleStatValuesBlock()


class RTSVehicleStatValuesBlock(RegularVehicleStatValuesBlock):
    __slots__ = ('tankmanDamageToSupplies', 'supplyDamageToTankman', 'spottedStrategistItems',
                 'spottedSupplies', 'damagedKilledSupplies')
...
class RTS1x7VehicleStatValuesBlock(RegularVehicleStatValuesBlock):
    __slots__ = ('shotsByTanksSupplies', 'damageDealtByTanksSupplies', 'damageBlockedByTanksSupplies',
                 'hitsByTanks', 'hitsBySupplies')
...
class RTS1x1VehicleStatValuesBlock(RegularVehicleStatValuesBlock):
    __slots__ = ('tankmanDamageToSupplies', 'supplyDamageToTankman', 'spottedStrategistItems',
                 'spottedSupplies', 'damagedKilledSupplies', 'shotsByTanksSupplies',
                 'damageDealtByTanksSupplies', 'damageBlockedByTanksSupplies', 'hitsByTanks',
                 'hitsBySupplies')
...
class RTSTeamStatsBlock(TeamStatsBlock):
    ...
    def _createCommanderBlock(vehicle, reusable):
        ...
        if _isRTS1x1(reusable):
            return RTSVehicleStatsBlock()
        return RTSCommanderVehicleStatsBlock()
...
class RTSEnemyTeamStatsBlock(RTSTeamStatsBlock):
    ...
    def _getTeamPlayerInfo(self, reusable):
        if _isRTS1x1(reusable):
            return _get1x1OpponentPlayerInfo(reusable)
        else:
            return
...
class RTSCommanderUserVehicleStatsBlock(RTSCommanderVehicleStatsBlock):
    ...
    def getVO(self):
        if not self.__isInitialized:
            ...
            return super(RTSCommanderUserVehicleStatsBlock, self).getVO()


class RTSSupplyStatValuesBlock(base.StatsBlock):
    __slots__ = ('shotsBySupplies', 'hits', 'damageDealtBySupplies', 'sniperDamageDealtBySupplies',
                 'directHitsReceivedBySupplies', 'piercingsReceivedBySupplies', 'noDamageDirectHitsReceivedBySupplies',
                 'spottedTanksBySupplies', 'damagedKilledTanksBySupplies')
