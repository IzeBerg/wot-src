...
class HW21RewardsItem(base.StatsItem):
    ...
    def __getDailyKeysReward(self, info, questsProgress):
        ...
        if progress is None or not isQuestCompleted(progress):
            return 0
        return self.gameEventController.getVehiclesController().getDailyKeysReward(vehIntCD)
    ...
    def __makeMedalVO(self, teamFightPlace, keys):
        return {'icon': backport.image(R.images.gui.maps.icons.event.battleResult.medals.personal.num(teamFightPlace)()), 
           ...
           'isSpecial': True, 
           'specialArgs': [
                         backport.image(R.images.gui.maps.icons.event.battleResult.medals.personal.num(teamFightPlace)()),
                         keys['efficiency']], 
...
class EventVehicleStatsBlock(vehicles.RegularVehicleStatsBlock):
    __slots__ = ('matter', 'matterOnTank', 'tankType', 'banStatus', 'banStatusTooltip',
                 'teamFightPlace', 'rewardBoxKeys', 'damageBlockedByArmor', 'medals')
    _NO_BAN = 'ok'
    _BANNED = 'ban'
    _WARNED = 'warning'
    ...
    def __buildMedals(self, teamFightPlace):
        if teamFightPlace == EVENT.INVALID_BATTLE_PLACE:
            return []
        return [
         self.__makeMedalVO(teamFightPlace)]
