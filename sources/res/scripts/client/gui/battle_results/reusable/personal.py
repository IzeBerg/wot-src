...
class PersonalInfo(shared.UnpackedInfo):
    __slots__ = ('__avatar', '__vehicles', '__lifeTimeInfo', '__isObserver', '_economicsRecords',
                 '__questsProgress', '__PM2Progress', '__rankInfo', '__battleRoyale',
                 '__isTeamKiller', '__progressiveReward', '__premiumMask', '__isAddXPBonusApplied',
    ...
    def getVehicleCDsIterator(self, result, reusable=None):
        ...
        if self.__isCommander and reusable is not None:
            for supplyID, intCD in reusable.common.getTeamSupplies(self.avatar.team):
                yield (
                 intCD, first(result[_RECORD.VEHICLES][supplyID]))
