...
class CommonInfo(shared.UnpackedInfo):
    __slots__ = ('__arenaTypeID', '__winnerTeam', '__finishReason', '__arenaVisitor',
                 '__bots', '__numDefended', '__supplies', '__suppliesIDtoCD')
...
class RTSCommonInfo(CommonInfo):
    ...
    def _processBot(self, info, additionalInfo=None):
        ...
        if botTeam == self.__commanderTeam:
            return PlayerInfo(dbID=self.__commanderID, **additionalInfo)
        return PlayerInfo(team=botTeam, realName=preprocessBotName(botName))
