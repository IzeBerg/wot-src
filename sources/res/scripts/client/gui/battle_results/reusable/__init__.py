...
_Reusables = namedtuple('_Reusables', ('common', 'personal', 'players', 'vehicles',
                                       'avatars'))
...
class _ReusableInfo(object):
    ...
    def getPersonalDetailsIterator(self, result, onlySummary=False):
        ...
        for _, vData in self.__personal.getVehicleCDsIterator(result):
            ...
            if not onlySummary:
                yield (
                 (
                  bases,), sorted(vehicles, key=sortVehicleKey), sorted(supplies))

        yield (
         (
          totalBases,), sorted(totalSortableVehicles.itervalues(), key=sortVehicleKey),
         sorted(totalSortableSupplies.itervalues(), key=sortVehicleKey))
        return
    ...
    def getBiDirectionTeamsIterator(self, result, sortKey=sort_keys.TeamItemSortKey):
        ...
        return (
         _allies(allies, sortKey), _enemies(enemies, sortKey))
