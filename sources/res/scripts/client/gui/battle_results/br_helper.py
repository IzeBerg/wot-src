...
_EnemyInfo = namedtuple('EnemyInfo', ('name', 'clanTag', 'tankName', 'shortVehicleName',
                                      'vehicleType', 'value', 'isTeamKiller'))
...
def getDamageBlockDetails(damageDealt, piercings, labels):
    ...
    descriptions = [
     backport.text(labels[0], vals=style.getTooltipParamsStyle()),
     backport.text(labels[1])]
    return (
     values, descriptions)
...
def getStunBlockDetails(assisted, count, duration, labels):
    ...
    stunNames = [
     backport.text(labels[0], vals=style.getTooltipParamsStyle()),
     backport.text(labels[1]),
     backport.text(labels[2], vals=style.getTooltipParamsStyle(BATTLE_RESULTS.COMMON_TOOLTIP_PARAMS_VAL_SECONDS))]
    return (
     stunValues, stunNames)
