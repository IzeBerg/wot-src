...
def _serverSettingsChangeBrowserHandler(browser, diff):
    eventConfigKeys = (
     Configs.BATTLE_ROYALE_CONFIG.value, Configs.EVENT_PROGRESSION_CONFIG.value)
...
class EventProgressionController(IEventProgressionController):
    ...
    def modeIsEnabled(self):
        if self.__currentController:
            return self.__currentController.getModeSettings().isEnabled
        return False
    ...
    def isActive(self):
        if self.__currentController:
            return self.modeIsEnabled() and self.__currentController.getCurrentSeason() is not None and self.__currentController.getCurrentCycleInfo()[1]
        else:
            return False
    ...
    def getCalendarInfo(cls):
        ...
        for key, selectedSeason in cls.getMostRelevantSeasons().iteritems():
            if selectedSeason is not None:
                calendarData[key] = {}
                calendarData[key]['season'] = {'id': selectedSeason.getSeasonID(), 'start': selectedSeason.getStartDate(), 
                   'end': selectedSeason.getEndDate()}
                calendarData[key]['cycles'] = [ {'id': cycle.ID, 'start': cycle.startDate, 'end': cycle.endDate, 'announce_only': cycle.announceOnly} for cycle in selectedSeason.getAllCycles().values()
                                              ]
    ...
    def getCurrentOrNextActiveCycleNumber(self, season):
        if self.__currentController:
            return self.__currentController.getCurrentOrNextActiveCycleNumber(season)
        return 0
    ...
    def isNeedAchieveMaxLevelForDailyQuest(self):
        ...
        if self.isFrontLine and level < self.getMaxPlayerLevel():
            return True
        return False
    ...
    def getPrimeTimeStatus(self, peripheryID=None):
        if self.__currentController:
            return self.__currentController.getPrimeTimeStatus(peripheryID)
        return (
    ...
    def getHeaderTooltipPack(self):
        items = [
         self.__getTopBackgroundTooltipWithTextData()]
        bottom = -30
    ...
    def getCycleStatusTooltipPack(self):
        ...
        items.append(formatters.packBuildUpBlockData(blocks=[
         formatters.packBlockDataItem(linkage=linkage, data=getProgressionIconVODict(cycleNumber=cycleNumber, playerLevel=levelInfo.currentLevel))], layout=BLOCKS_TOOLTIP_TYPES.LAYOUT_HORIZONTAL, align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER))
    ...
    def __getPlayerProgressToLevelBlock(self, playerLevel, playerFamePts):
        ...
        data = EpicBattlesWidgetTooltipVO(progressBarData={'value': playerFamePts, 'maxValue': famePtsToProgress})._asdict()
