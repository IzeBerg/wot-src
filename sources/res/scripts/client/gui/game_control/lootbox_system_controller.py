...
class LootBoxSystemController(ILootBoxSystemController, EventsHandler):
    ...
    def getBoxesCount(self, category=None):
        if category is None:
            return sum(self.__boxesCount.itervalues())
        else:
            return self.__boxesCount.get(category, 0)
    ...
    def getActiveBoxes(self, criteria=None):
        return list((callable(criteria) or self.getBoxes)(lambda b: b.getType() == self.eventName) if 1 else self.getBoxes(lambda b: b.getType() == self.eventName and criteria(b)))
