...
class DragonBoatController(IDragonBoatController):
    ...
    def getState(self):
        ...
        if teamNumber:
            return DragonBoatState.STARTED
        return DragonBoatState.NOT_STARTED

    def getTeam(self):
        teamTokens = self.getConfig().dragonBoatTokens.get('activeParticipant')
        if teamTokens:
            return self.itemsCache.items.tokens.getTokenCount(teamTokens[0])
        return 0
    ...
    def _getUrl(self, urlName=None):
        ...
        if urlName is None:
            ...
            return baseUrl + GUI_SETTINGS.dragonBoat.get(urlName, '')
