...
class WOController(IWOController):
    ...
    def _pushNotification(self, notificationType, intCD):
        ...
        if not self.__isAccountEligible():
            ...
            self._systemMessages.proto.serviceChannel.pushClientMessage(message=None, msgType=SCH_CLIENT_MSG_TYPE.WO_NOTIFICATION, auxData=[
             {'header': backport.text(offerResId.header(), vehicleName=vehicle.userName), 
