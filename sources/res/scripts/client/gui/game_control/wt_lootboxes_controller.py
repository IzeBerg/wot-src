...
class WTLootBoxesController(IWTLootBoxesController):
    ...
    def getLastViewedCount(self):
        ...
        return (
         hunterLastViewedCount, bossLastViewedCount)
    ...
    def getLootBoxesByType(self, lootBoxType=None):
        if lootBoxType is None:
            lootBoxTypes = (
             EventLootBoxes.WT_HUNTER, EventLootBoxes.WT_BOSS)
        else:
            lootBoxTypes = (
             lootBoxType,)
    ...
    def onPortalOpened(self, boxType, parentWindow, callbackFailure=None):
        ...
        if result is None or not result.success:
            ...
            if callbackFailure:
                callbackFailure()
            return
        if result.success and result.auxData[boxType]['count'] == 0:
