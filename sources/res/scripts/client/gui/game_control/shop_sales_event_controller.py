...
class _EventStatusSystemChanelNotifier(object):
    ...
    def __getStateMessageData(self):
        if self.__ctrl.isEnabled:
            return (self.__msgRes.enabledBySwitch, SM_TYPE.EventWarningWithButton)
        return (
         self.__msgRes.disabledBySwitch, SM_TYPE.EventError)
    ...
    def __getPhaseMessageData(self):
        if self.__ctrl.currentEventPhase == EventPhase.IN_PROGRESS:
            ...
            if self.__ctrl.currentEventPhase == EventPhase.FINISHED:
                return (self.__msgRes.activePhaseFinished, SM_TYPE.EventInfoWithButton)
            if not self.__ctrl.isInEvent:
                return (self.__msgRes.eventFinished, SM_TYPE.EventInfo)
            return (
             R.invalid, None)
...
class ShopSalesEventController(IShopSalesEventController, _Notifiable):
    ...
    def __init__(self):
        ...
        self.__state = {}
        self.__phase = None
        self.__currentBundleInfo = (True, '', 0, None)
