...
class MapboxController(Notifiable, SeasonProvider, IMapboxController, IGlobalListener):
    ...
    def getUnseenItemsCount(self):
        ...
        if not self.isEnabled() or self.getCurrentCycleID() is None or progressionData is None:
            return 0
        return len([ mapName for mapName, mapData in progressionData.surveys.iteritems() if not self.__settingsManager.isMapVisited(mapName) and mapData.available and mapData.progress >= mapData.total
                   ]) + len([ rewardItem for battles, reward in progressionData.rewards.iteritems() if battles <= progressionData.totalBattles for rewardItem in reward.bonusList if rewardItem.getName() == 'selectableCrewbook'
                            ])
