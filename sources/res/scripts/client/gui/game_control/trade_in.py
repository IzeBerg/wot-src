...
class TradeInInfo(namedtuple('TradeInInfo', [
 'minDiscountVehicleCD', 'minDiscountPrice', 'maxDiscountVehicleCD', 'maxDiscountPrice'])):
...
class TradeInController(ITradeInController):
    ...
    def getStartEndTimestamps(self):
        if self.isEnabled():
            ...
            if actions:
                return (
                 min(a.getStartTimeRaw() for a in actions),
                 max(a.getFinishTimeRaw() for a in actions))
        return (0.0, 0.0)
