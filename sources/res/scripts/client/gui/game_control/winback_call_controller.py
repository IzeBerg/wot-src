...
class FriendsDataKeys(object):
    ...
    CALLER_CLAN = 'caller_clan'
    INVITE_HOST = 'lms_url'
    FRIENDS_WINBACK_COUNT = 'friends_winback_count'


_WinbackCallWebResponse = namedtuple('_WinbackCallWebResponse', ('isSuccess', 'status',
                                                                 'data'))
_Record = namedtuple('_Record', ('name', 'value', 'compare'))
_MAX_FRIENDS = 20
_COMMON_SERVICE_RECORDS = ('common_battles_count', 'common_wins_count', 'winback_damage_dealt',
                           'brothers_in_arms_medal_cnt', 'crucial_contribution_medal_cnt',
                           'in_caller_contacts_days', 'common_clan_days_cnt', 'common_event_battles_cnt')
_PERSONAL_SERVICE_RECORDS = ('battles_count', 'wins_count', 'damage_dealt', 'frags_count',
                             'account_age_days', 'medals_count')
_SERVICE_RECORDS_LIMITS = {'common_battles_count': 10.0, 
   'common_wins_count': 0.4, 
   'winback_damage_dealt': 10000.0, 
   'brothers_in_arms_medal_cnt': 20.0, 
   'crucial_contribution_medal_cnt': 20.0, 
   'in_caller_contacts_days': 365.0, 
   'common_clan_days_cnt': 30.0, 
   'common_event_battles_cnt': 3.0, 
   'battles_count': 1000.0, 
   'wins_count': 0.4, 
   'damage_dealt': 50000.0, 
   'frags_count': 1000.0, 
...
class _Friend(object):
    ...
    @property
    def spaID(self):
        return self.__data.get('winback_spa_id', '')

    @property
    def userName(self):
        return self.__data.get('winback_name', '')

    @property
    def userClan(self):
        return self.__data.get('winback_clan', '')

    @property
    def lastLogin(self):
        return self.__data.get('last_login_dt', time_utils.getServerUTCTime())

    @property
    def vehicleName(self):
        return self.__data.get('preferable_vehicle_name', '')
    ...
    @property
    def isAwarded(self):
        return bool(self.__data.get('awarded', False))

    @property
    def hasEmail(self):
        return bool(self.__data.get('is_email', False))

    @property
    def wasNotifiedByEmail(self):
        return bool(self.__data.get('email_status', False))

    @property
    def inviteCode(self):
        return str(self.__data.get('invite_code', ''))
    ...
    def updateStatus(self, status):
        if status in WinBackCallFriendState:
            self.__data.update({'invite_status': status.value})
            return True
        return False
    ...
    def __parse(self, data):
        if not data:
            ...
            for friendData in friends:
                vehName = friendData.get('preferable_vehicle_name', '')
...
class WinBackCallController(IWinBackCallController, IGlobalListener):
    ...
    def inviteTokenQuestID(self):
        if self.__winBackCallConfig:
            return self.__winBackCallConfig.inviteTokenQuest
        return _DEF_QUEST_ID

    def eventPeriod(self):
        if self.__winBackCallConfig:
            return (self.__winBackCallConfig.startTime, self.__winBackCallConfig.endTime)
        return (0, 0)
    ...
    def getFriendsList(self, callback=None):
        ...
        if self.__webCtrl.isAvailable():
            ...
            if isSuccess:
                ...
                errorStr = responseData.get('error', {}).get('code', '')
