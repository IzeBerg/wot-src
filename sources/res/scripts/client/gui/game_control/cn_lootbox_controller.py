...
class CNLootBoxesController(ICNLootBoxesController):
    __slots__ = ('__statusChangeNotifier', '__em', '__boxesCount', '__isLootBoxesAvailable',
                 '__isActive', '__boxInfo')
    ...
    def getExpiresAtLootBoxBuyCounter(self):
        ...
        if entitlement is not None:
            return entitlement.getExpiresAtInTimestamp()
        else:
            return 0
    ...
    def getStoreInfo(self):
        return {lb.getType():lb for lb in self.__itemsCache.items.tokens.getLootBoxes().values() if lb.getCategory() == CHINA_LOOT_BOXES_CATEGORIES if lb.getCategory() == CHINA_LOOT_BOXES_CATEGORIES}
