...
def _readVideoSection(section):
    messagesFields = ('video-path', 'event-start', 'event-stop', 'event-pause', 'event-resume',
                      'event-loop', 'icon', 'video-fit-to-screen')
    subtitlesFields = ('subtitle', 'voiceover', 'keypoint')
    messageSec = section['message']
    content = {'messages': _readSequenceItem(messageSec, messagesFields), 'voiceovers': _readSubItemSectionSequence(messageSec, 'subtitles', subtitlesFields)}
    return content
