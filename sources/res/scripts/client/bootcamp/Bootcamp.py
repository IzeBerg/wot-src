...
class Bootcamp(EventSystemEntity):
    ...
    def __getLessonStatus(self, lesson):
        return [
         BootcampStatuses.RECEIVED if g_bootcamp.getLessonNum() >= lesson or not self.bootcampController.needAwarding() and self.isLastLesson(lesson - 1) else BootcampStatuses.WAIT, lesson]
