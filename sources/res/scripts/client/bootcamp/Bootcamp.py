...
_BattleResults = namedtuple('_BattleResults', (
 'type',
 'typeStr'))
...
class Bootcamp(EventSystemEntity):
    ...
    def __getLessonStatus(self, lesson):
        return [
         BootcampStatuses.RECEIVED if g_bootcamp.getLessonNum() >= lesson or not self.bootcampController.needAwarding() and self.isLastLesson(lesson) else BootcampStatuses.WAIT, lesson]
    ...
    def _getProgressBarIcon(self, icon, tooltipArgs, lesson):
        return {'icon': icon, 'tooltipArgs': tooltipArgs + self.__getLessonStatus(lesson)}

    def _getProgressBarItem(self, rewards, tooltipArgs):
        return {'rewards': rewards, 'tooltipArgs': tooltipArgs}
    ...
    def __getProgressBarItems(self, iconSize):
        ...
        firstVehicleIcon = self._getProgressBarIcon(bootcampIcons.bcTank2_48() if iconSize == ICON_SIZE.SMALL else bootcampIcons.bcTank2_80(), [
         BOOTCAMP.TOOLTIP_PROGRESSION_LABEL_VEHICLE_SECOND_LEVEL,
         BOOTCAMP.TOOLTIP_PROGRESSION_DESCRIPTION_VEHICLE,
         RES_ICONS.MAPS_ICONS_BOOTCAMP_REWARDS_TOOLTIPS_BCAWARDOPTIONS], 1)
        secondVehicleIcon = self._getProgressBarIcon(bootcampIcons.bcTank3_48() if iconSize == ICON_SIZE.SMALL else bootcampIcons.bcTank3_80(), [
         BOOTCAMP.TOOLTIP_PROGRESSION_LABEL_VEHICLE_THIRD_LEVEL,
         BOOTCAMP.TOOLTIP_PROGRESSION_DESCRIPTION_VEHICLE,
         RES_ICONS.MAPS_ICONS_BOOTCAMP_REWARDS_TOOLTIPS_BCAWARDOPTIONS], 3)
        finalLesson = self.getContextIntParameter('lastLessonNum')
        finalBonusIcons = [
         self._getProgressBarIcon(bootcampIcons.bcGold_48() if iconSize == ICON_SIZE.SMALL else bootcampIcons.bcGold_80(), [
          BOOTCAMP.TOOLTIP_PROGRESSION_LABEL_GOLD,
          BOOTCAMP.TOOLTIP_PROGRESSION_DESCRIPTION_GOLD,
          RES_ICONS.MAPS_ICONS_BOOTCAMP_REWARDS_TOOLTIPS_BCGOLD], finalLesson),
         self._getProgressBarIcon(bootcampIcons.bcPremium_universal_48() if iconSize == ICON_SIZE.SMALL else bootcampIcons.bcPremium_universal_80(), [
          ...
          RES_ICONS.MAPS_ICONS_BOOTCAMP_REWARDS_TOOLTIPS_BCPREMIUMPLUS], finalLesson),
