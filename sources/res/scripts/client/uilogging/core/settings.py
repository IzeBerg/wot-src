...
class LocalSettings(object):
    __slots__ = ('_settings', 'clientLog', 'logsQueueLimit', 'logUrl', 'settingsUrl',
                 'settingsTtl', 'enabled')
...
class Settings(object):
    ...
    def toDict(self):
        return {'status': self.status, 'disabled': self.disabled, 'loglevel': self.loglevel}
    ...
    def _request(self, feature, group):
        ...
        if spaID is None or not feature or not group:
            ...
            if self._requestsCount.get(key, 0) >= self.REQUESTS_LIMIT:
                ...
                return settings
            postData = {'feature': feature, 'spa_id': spaID}
