...
class NyLogger(BaseLogger):
    ...
    def log(self, action, additionalInfo=None, parentScreen=None, **params):
        ...
        data = {k:v for k, v in data.iteritems() if v is not None if v is not None}
        data.update(params)
...
class NyMainViewFlowLogger(NyTopMenuFlowLogger):
    ...
    def logCloseClick(self, view, tab, isEscPressed):
        if self.__wasLogged:
            return
        transitionMethod = (isEscPressed or TransitionMethods).CLOSE_BUTTON if 1 else TransitionMethods.ESC_BUTTON
