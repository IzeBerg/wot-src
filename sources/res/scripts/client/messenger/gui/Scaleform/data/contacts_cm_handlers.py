...
class SimpleContactsCMHandler(AbstractContextMenuHandler, EventSystemEntity):
    ...
    def _generateOptions(self, ctx=None):
        return [
         self._makeItem(CONTACTS_ACTION_CONSTS.EDIT_GROUP, MESSENGER.MESSENGER_CONTACTS_CONTEXTMENU_EDITGROUP),
         self._makeItem(CONTACTS_ACTION_CONSTS.REMOVE_GROUP, MESSENGER.MESSENGER_CONTACTS_CONTEXTMENU_REMOVEGROUP)]
...
class PlayerContactsCMHandler(BaseUserCMHandler):
    ...
    def createContactNote(self):
        self.fireEvent(events.ContactsEvent(events.ContactsEvent.CREATE_CONTACT_NOTE, ctx={'databaseID': self.databaseID, 'userName': self.userName}), scope=EVENT_BUS_SCOPE.LOBBY)

    def editContactNote(self):
        self.fireEvent(events.ContactsEvent(events.ContactsEvent.EDIT_CONTACT_NOTE, ctx={'databaseID': self.databaseID, 'userName': self.userName}), scope=EVENT_BUS_SCOPE.LOBBY)
    ...
    def _addContactsNoteInfo(self, options, userCMInfo):
        if self.showUserNotes and self.proto.contacts.isNoteSupported():
            ...
            if userNote:
                options.extend([
                 self._makeItem(CONTACTS_ACTION_CONSTS.EDIT_CONTACT_NOTE, MENU.contextmenu(CONTACTS_ACTION_CONSTS.EDIT_CONTACT_NOTE)),
                 self._makeItem(CONTACTS_ACTION_CONSTS.REMOVE_CONTACT_NOTE, MENU.contextmenu(CONTACTS_ACTION_CONSTS.REMOVE_CONTACT_NOTE))])
            else:
