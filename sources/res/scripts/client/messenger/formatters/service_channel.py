    ...
    def getCelebrityQuestsMessages(self, message):
        ...
        if completedQuestIDs is not None:
            ...
            for quest in quests.itervalues():
                ...
                if isBattleQuest:
                    for bonus in bonuses:
                        if bonus.getName() == b'battleToken' and getCoinToken() in bonus.getValue():
            ...
            if coinsRewards:
                ...
                formatted = g_settings.msgTemplates.format(templateName, ctx={b'title': title, b'rewards': coinsRewards})
                messages.append(MessageData(formatted, settings))
        return messages
    ...
    def format(self, message, callback):
        ...
        if message.data and isSynced:
            ...
            if arenaCreateTime and arenaType:
                ...
                for quest in battleResults.get(b'detailedRewards', {}):
...
class AdventCalendarV2ProgressionAchievesFormatter(LootBoxAchievesFormatter):
    ...
    def _processTokens(cls, data):
        ...
        boxId = LOOTBOX_TOKEN_PREFIX + str(box.id) if box is not None else b''
        for tokenID, info in tokens.iteritems():
            if tokenID == boxId:
                count = info.get(b'count', 0)
                tokenId = tokenID
                if count > 0:
                    result.append(g_settings.htmlTemplates.format(b'adventLootbox', {b'count': count}))
