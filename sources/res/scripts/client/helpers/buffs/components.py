import logging, AnimationSequence, BigWorld, Math, configs
...
class SequenceBuffComponent(ClientBuffComponent):
    ...
    def _getAvatarCtrlMode(self):
        ...
        if not avatar:
            return CTRL_MODE_NAME.DEFAULT
        return avatar.inputHandler.ctrlModeName
    ...
    def _isVisibleInSniperMode(self):
        ...
        if self._hideInSniperModeFor == self.VisibilityModes.SELF and self._shouldShowForSelf():
            return False
        if self._hideInSniperModeFor == self.VisibilityModes.OTHERS and not self._shouldShowForSelf():
            return False
        return True
...
class RibbonBuffComponent(ClientBuffComponent):
    ...
    def _apply(self):
        data = {'eventType': _BET.BUFF_APPLIED, 'targetID': 0, 
           'details': self._config.buffName, 
           'count': 1}
        data = (
         feedback_events.PlayerFeedbackEvent.fromDict(data),)
