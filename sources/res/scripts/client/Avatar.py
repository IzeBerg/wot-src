...
import typing, logging, RTSShared, BigWorld, BWReplay, Keys, Math, ResMgr, WWISE, WoT, CGF, Account, AccountCommands, AreaDestructibles, ArenaType, AvatarInputHandler, AvatarPositionControl, BattleReplay, ClientArena, CommandMapping, Event, FlockManager, MusicControllerWWISE, ProjectileMover, SoundGroups, TriggersManager, Vehicle, Weather, constants
...
class PlayerAvatar(BigWorld.Entity, ClientChat, CombatEquipmentManager, AvatarObserver, TeamHealthbarMechanic, AvatarEpicData, AvatarRecoveryMechanic, AvatarRespawnMechanic, VehiclesSpawnListStorage, AvatarChatKeyHandling, VehicleChangeComponent, VehicleRemovalController, VehicleHealthBroadcastListenerComponent, TriggersController):
    ...
    def __init__(self):
        ...
        self.__damageInfoNoNotification = ('DEVICE_CRITICAL', 'DEVICE_DESTROYED', 'TANKMAN_HIT',
                                           'FIRE', 'DEVICE_CRITICAL_AT_DROWNING',
                                           'DEVICE_DESTROYED_AT_DROWNING', 'TANKMAN_HIT_AT_DROWNING')
    ...
    def onBecomePlayer(self):
        ...
        if self.isCommander():
            self.__damageInfoNoNotification += ('DEATH_FROM_SHOT', 'DEATH_FROM_DROWNING',
                                                'DEATH_FROM_OVERTURN')
    ...
    def startServerSideReplay(self):
        ...
        LOG_DEBUG_DEV('Avatar.startServerSideReplay: playerVehicleID=%s, vehicles=%s' % (
         self.playerVehicleID, repr(otherVehicles)))
            ...
            if cmdMap.isFired(CommandMapping.CMD_SHOW_HELP, key) and isDown and mods == 0 and not self.isCommander():
                ...
                if vehicle is not None:
                    ...
                    if self.arenaBonusType in constants.ARENA_BONUS_TYPE.BATTLE_ROYALE_RANGE:
                        ctx = {'battleRoyale': True, 'mapGeometryName': self.arena.arenaType.geometryName, 
                           'hasUniqueVehicleHelpScreen': True}
                    if ctx:
    ...
    def updateDualGunStatus(self, vehicleID, status, times):
        if not self.__updateVehicleStatus(vehicleID):
            ...
            self.guiSessionProvider.invalidateVehicleState(VEHICLE_VIEW_STATE.DUAL_GUN_CHARGER, (
             status, times))
    ...
    def updateDeathZoneWarningNotification(self, enable, strikeTime, waveDuration):
        self.guiSessionProvider.invalidateVehicleState(VEHICLE_VIEW_STATE.DEATHZONE, (
         enable, strikeTime, waveDuration))
        ...
        if constants.HAS_DEV_RESOURCES:
            ...
            if code == DEVELOPMENT_INFO.BONUSES:
                ...
                if board is not None:
                    ...
                    board.setUpdater(lambda x: (' / ').join([
                     str(params['finalAttrs'].get(x)),
                     str(params['factors'].get(x)),
                     str(params['miscAttrs'].get(x))]))
                    board.update()
    ...
    def __getBattleEventsAdditionalData(self):
        ...
        if numOfControlledVehicles > 0:
            return {'role': controlledVehicle[0].typeDescriptor.role}
        return {'role': constants.ROLE_TYPE.NOT_DEFINED}
    ...
    def __showDamageIconAndPlaySound(self, damageCode, extra, vehicleID, isCurrentVehPossessed, isAttaching):
        ...
        if deviceState is not None:
            ...
            if rtsCommander and not isAttaching and self.isCommander() and damageCode not in self.__damageInfoDrowning:
                rtsCommander.invalidateControlledVehicleState(vehicleID, VEHICLE_VIEW_STATE.DEVICES, (
                 deviceName, deviceState, actualState))
            if self.isCommander():
    ...
    def __isPlayerInSquad(self):
        return self.arena is not None and self.arena.guiType in (constants.ARENA_GUI_TYPE.RANDOM,
         ...
         constants.ARENA_GUI_TYPE.EPIC_BATTLE) and self.guiSessionProvider.getArenaDP().isSquadMan(vID=self.playerVehicleID)
