...
import typing, BigWorld, Math, Vehicular, AnimationSequence
...
ProjectionDecalGenericParams.__new__.__defaults__ = (
 Math.Vector4(0.0), Math.Vector3(0.0), Math.Vector3(0.0, 1.0, 0.0), 1.0, '', '', 0.0, 0.0, False, False, False,
 True)
...
def processTiling(appearance, vDesc, descId, camouflage, component):
    ...
    if camouflage.tiling.get(vDesc.type.compactDescr) is not None or tilingType == CamouflageTilingType.LEGACY:
        ...
        textureSize = (BigWorld.PyTextureProvider(camouflage.texture).width,
         BigWorld.PyTextureProvider(camouflage.texture).height)
