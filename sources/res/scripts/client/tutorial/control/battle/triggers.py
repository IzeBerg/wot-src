...
__all__ = ('VehicleOnArenaTrigger', 'PlayerVehicleNoAmmoTrigger', 'ObjectAIMTrigger',
           'AreaTrigger', 'AimAtVehicleTrigger', 'AutoAimAtVehicleTrigger', 'VehicleDestroyedTrigger',
           'VehicleOnSoftTerrainTrigger', 'ShotMissedTrigger', 'ShotNoDamageTrigger',
           'ShotDamageTrigger', 'SniperModeTrigger', 'TriggersDispatcher')
...
class PlayerVehicleNoAmmoTrigger(Trigger):
    ...
    def run(self):
        if not self.isSubscribed:
            self.__addListeners()
            self.__pIdx = g_tutorialWeaver.weave(pointcut=aspects.AmmoQuantityPointcut, aspects=(
             aspects.AmmoQuantityAspect(self),))
...
class ObjectAIMTrigger(_DispatchableTrigger):
    ...
    def getAllowed(self):
        return [
         TriggersManager.TRIGGER_TYPE.AIM]
...
class AreaTrigger(_DispatchableTrigger):
    ...
    def getAllowed(self):
        return [
         TriggersManager.TRIGGER_TYPE.AREA]
...
class AimAtVehicleTrigger(_DispatchableTrigger):
    ...
    def getAllowed(self):
        return (
         TriggersManager.TRIGGER_TYPE.AIM_AT_VEHICLE,)
...
class AutoAimAtVehicleTrigger(_DispatchableTrigger):
    ...
    def getAllowed(self):
        return [
         TriggersManager.TRIGGER_TYPE.AUTO_AIM_AT_VEHICLE]
...
class VehicleOnSoftTerrainTrigger(_DispatchableTrigger):
    ...
    def getAllowed(self):
        return [
         TriggersManager.TRIGGER_TYPE.PLAYER_VEHICLE_ON_SOFT_TERRAIN]
...
class ShotNoDamageTrigger(_DispatchableTrigger):
    ...
    def isAllowed(self, triggerType, event):
        return triggerType in [
         TriggersManager.TRIGGER_TYPE.PLAYER_SHOT_RICOCHET,
         TriggersManager.TRIGGER_TYPE.PLAYER_SHOT_NOT_PIERCED] and self.getVar() == event.get('targetId')
...
class SniperModeTrigger(_DispatchableTrigger):
    ...
    def getAllowed(self):
        return [
         TriggersManager.TRIGGER_TYPE.SNIPER_MODE]
