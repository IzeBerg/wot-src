import time, weakref, BigWorld, Math, TriggersManager, AnimationSequence
    ...
    def triggerEffect(self):
        ...
        if task is not None:
            ...
            if flagID is not None:
                ...
                soundID = SOUND_EVENT.TASK_COMPLETED
            if self._gui.playEffect(GUI_EFFECT_NAME.NEXT_TASK, [
             task.getID(), task.getText(), flag]):
...
class FunctionalDisableCameraZoomEffect(FunctionalEffect):
    ...
    def triggerEffect(self):
        ...
        if weaver.findPointcut(aspects.ArcadeCtrlMouseEventsPointcut) == -1:
            weaver.weave(pointcut=aspects.ArcadeCtrlMouseEventsPointcut, aspects=(
             aspects.MouseScrollIgnoreAspect,))
        if weaver.findPointcut(aspects.CameraUpdatePointcut) == -1:
            ...
            self._cameraUpdatePointIdx = weaver.weave(pointcut=aspects.CameraUpdatePointcut, aspects=(
             aspects.CameraZoomModeIgnoreAspect,))
        return True
...
class FunctionalBattleScene(FunctionalScene):
    ...
    def __requestRequiredData(self):
        ...
        if localCtx.accCompleted < 0:
            ...
            player.requestAccountStats([
             'tutorialsCompleted'], self.__onReceiveAccountStats)
        return
