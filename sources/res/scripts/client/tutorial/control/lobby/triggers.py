...
__all__ = ('BonusTrigger', 'BattleCountRequester', 'ItemUnlockedTrigger', 'ItemInInventoryTrigger',
           'ItemInstalledTrigger', 'EquipmentInstalledTrigger', 'CurrentVehicleChangedTrigger',
           'FreeVehicleSlotChangedTrigger', 'PremiumPeriodChangedTrigger', 'PremiumDiscountUseTrigger',
           'IsCollectibleVehicleTrigger')
...
class FreeVehicleSlotChangedTrigger(Trigger):
    ...
    def run(self):
        if not self.isSubscribed:
            self.__pIdx = g_tutorialWeaver.weave(pointcut=aspects.BuySlotPointcut, aspects=[
             aspects.BuySlotAspect(self)])
            self.isSubscribed = True
        self.isRunning = True
        self.toggle(isOn=self.isOn())
...
class PremiumDiscountUseTrigger(Trigger):
    ...
    def run(self):
        if not self.isSubscribed:
            self.__pIdx = g_tutorialWeaver.weave(pointcut=aspects.BuyPremiumWithDiscountPointcut, aspects=[
             aspects.BuyPremiumWithDiscountAspect(self)])
            self.isSubscribed = True
...
class FreeXPChangedTrigger(Trigger):
    ...
    def run(self):
        if not self.isSubscribed:
            self.__startProcessPointcutId = g_tutorialWeaver.weave(pointcut=aspects.StartXpExchangePointcut, aspects=[
             aspects.StartXpExchangeAspect(self)])
            self.isSubscribed = True
