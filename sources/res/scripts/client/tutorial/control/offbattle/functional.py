import collections, urllib, MusicControllerWWISE, ResMgr
...
class FunctionalOpenInternalBrowser(FunctionalEffect):
    ...
    def triggerEffect(self):
        ...
        if self.browserCtrl.getBrowser(browserID) is None:
            ...
            self.browserCtrl.load(url=('file:///{0}').format(('{pageDir}/index_{lang}.html').format(pageDir=pageDir, lang=_ms('#settings:LANGUAGE_CODE'))), title=_ms('#miniclient:tutorial/video/title'), showCloseBtn=True, showActionBtn=False, browserSize=(780,
                                                                                                                                                                                                                                                                470), browserID=browserID)(lambda success: True)
        return True
