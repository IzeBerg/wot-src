...
_COMPONENT_EFFECTS = (
 GUI_EFFECT_NAME.SHOW_GREETING, GUI_EFFECT_NAME.SHOW_HINT,
 GUI_EFFECT_NAME.NEXT_TASK)
_REQUIRED_BATTLE_ALIASES = {
 BATTLE_VIEW_ALIASES.BATTLE_TUTORIAL,
 BATTLE_VIEW_ALIASES.MINIMAP,
 BATTLE_VIEW_ALIASES.MARKERS_2D}
...
class SfBattleProxy(GUIProxy):
    ...
    def setChapterInfo(self, title, description):
        if self.__tutorial is not None:
            try:
                text = makeHtmlString('html_templates:battle/tutorial', 'chapterDescription', ctx={'title': title, 'description': description})
