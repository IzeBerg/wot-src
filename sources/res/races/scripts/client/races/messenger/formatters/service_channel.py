...
class RacesBattleResultsFormatter(BattleResultsFormatter):
    ...
    def __makeTitleString(self, finishedPlace):
        if _FIRST_PLACE <= finishedPlace <= _THIRD_PLACE:
            return backport.text(R.strings.races_messenger.serviceChannelMessages.racesBattleResults.win())
        return backport.text(R.strings.races_messenger.serviceChannelMessages.racesBattleResults.defeat())

    def __makeFinishedPlaceString(self, finishedPlace):
        if finishedPlace == _NO_FINISHED_PLACE:
            return backport.text(R.strings.races_messenger.serviceChannelMessages.racesBattleResults.noFinishedPlace())
        return backport.getIntegralFormat(finishedPlace)
