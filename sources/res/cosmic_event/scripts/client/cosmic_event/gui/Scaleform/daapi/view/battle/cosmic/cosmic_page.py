...
class DynamicAliases(CONST_CONTAINER):
    COSMIC_VOICE_HINTS_PLAYER = 'cosmicVoiceHintsPlayer'
    COSMIC_TEXT_HINTS_PLAYER = 'cosmicTextHintsPlayer'


_COSMIC_COMPONENTS_CONFIG = ComponentsConfig(config=(
 (
  BATTLE_CTRL_ID.ARENA_PERIOD, (BATTLE_VIEW_ALIASES.COSMIC_HUD,)),
 (
  BATTLE_CTRL_ID.DEBUG, (BATTLE_VIEW_ALIASES.DEBUG_PANEL,)),
 (
  BATTLE_CTRL_ID.BATTLE_HINTS,
  (DynamicAliases.COSMIC_VOICE_HINTS_PLAYER,
   DynamicAliases.COSMIC_TEXT_HINTS_PLAYER))), viewsConfig=(
 (
  DynamicAliases.COSMIC_VOICE_HINTS_PLAYER,
  lambda : cosmic_hud.CosmicVoiceHintsPlayer(DynamicAliases.COSMIC_VOICE_HINTS_PLAYER, BattleHintQueueParams('voice', withFadeOut=False))),
 (
  DynamicAliases.COSMIC_TEXT_HINTS_PLAYER,
  lambda : cosmic_hud.CosmicTextHintsPlayer(DynamicAliases.COSMIC_TEXT_HINTS_PLAYER, BattleHintQueueParams('text', withFadeOut=False)))))
...
class CosmicPage(SharedPage):
    ...
    def as_isComponentVisibleS(self, componentKey):
        if componentKey in self.components:
            return super(CosmicPage, self).as_isComponentVisibleS(componentKey)
        return False