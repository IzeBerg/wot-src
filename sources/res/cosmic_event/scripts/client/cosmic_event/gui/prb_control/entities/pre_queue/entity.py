...
def canSelectPrbEntity(ctrl=None):
    return ctrl.isAvailable() and ctrl.isPrbActive()
...
class CosmicEventBattleEntity(PreQueueEntity):
    ...
    def _loadHangar(self):
        if self.__cosmicEventBattleCtrl.isEnabled:
            yield wg_await(self._fadeManager.show())
    ...
    def _goToQueueUI(self):
        self.__cosmicEventBattleCtrl.showQueueUI()
        return FUNCTIONAL_FLAG.COSMIC_EVENT

    def _exitFromQueueUI(self):
        self.__cosmicEventBattleCtrl.showMainRoute()

    def _makeQueueCtxByAction(self, action=None):
        return CosmicEventBattleQueueCtx(g_currentVehicle.item.invID, waitingID='prebattle/join')
