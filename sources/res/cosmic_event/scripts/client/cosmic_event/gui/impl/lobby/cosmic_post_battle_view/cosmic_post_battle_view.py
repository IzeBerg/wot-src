...
_scoringToKey = [
 (
  ScoringTypeEnum.SHOT, 'cosmicScore/SHOT'),
 (
 ...
 (
  ScoringTypeEnum.FRAGMENTHOLDERKILL, 'cosmicScore/FRAGMENT_HOLDER_KILL'),
 (
  ScoringTypeEnum.KILLASSIST, 'cosmicScore/KILL_ASSIST'),
 (
  ScoringTypeEnum.ABILITYPICKUP, 'cosmicScore/ABILITY_PICKUP'),
 (
  ScoringTypeEnum.FRAGMENTPICKUP, 'cosmicScore/FRAGMENT_PICKUP'),
 (
  ScoringTypeEnum.FRAGMENTCARRYING, 'cosmicScore/FRAGMENT_CARRYING'),
 (
  ScoringTypeEnum.FRAGMENTDELIVERING, 'cosmicScore/FRAGMENT_DELIVERING'),
 (
  ScoringTypeEnum.EVACUATION, 'cosmicScore/EVACUATION')]
_rewardKeys = [
 'index', 'name', 'value', 'isCompensation', 'tooltipId', 'tooltipContentId', 'label']

def _createScoringInfo(scoringType, points):
    score = ScoringModel()
    score.setEventPoints(points)
...
class CosmicPostBattleView(ViewImpl, LobbyHeaderVisibility):
    __slots__ = ('_battleResultsData', '_requestSource')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._requestSource = kwargs.get('ctx', {}).get('requestSource')
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        if self.__battleResults is not None and self._battleResultsData:
            with self.viewModel.transaction() as (model):
                ...
                totalPointsFromQuests = self._fillQuestsList(model)
                self._setMainScores(model, totalPointsFromQuests)
        return

    def _initialize(self, *args, **kwargs):
        super(CosmicPostBattleView, self)._initialize(*args, **kwargs)
        if self._requestSource != REQUEST_SOURCE.NOTIFICATION:
            CosmicHangarSounds.playPostbattleScreenEnter()
        self.suspendLobbyHeader(self.uniqueID)

    def _finalize(self):
        CosmicHangarSounds.playPostbattleStopScoreSound()
        CosmicHangarSounds.playPostbattleScreenExit()
        self.resumeLobbyHeader(self.uniqueID)
        super(CosmicPostBattleView, self)._finalize()

    def _setMainScores(self, model, pointsFromOtherSources):
        personalData = self._getPersonalData()
        model.setFragmentsDelivered(personalData['cosmicBattleEvent/FRAGMENT_DELIVERING'])
        model.setKillAmount(personalData['kills'])
        model.setEvacuationPosition(personalData['cosmicBattleEvent/EVACUATION'])
        model.setTotalPoints(personalData['cosmicTotalScore'] + pointsFromOtherSources)
    ...
    def _setPlayersList(self, model):
        ...
        reusableInfo = self._getReusableData()
        avatars = reusableInfo.avatars
        players = model.getPlayersList()
        players.clear()
        players.reserve(len(vehicles))
        vehicles = sorted(vehicles, key=lambda x: (not avatars.getAvatarInfo(x['accountDBID']).hasPenalties(),
         x['cosmicTotalScore']), reverse=True)
    ...
    def _fillPlayerEntry(self, playerEntry, vehicleData, place, isDeserter):
        ...
        playerEntry.setPlayerName(info.realName)
        playerEntry.setPlayerClan(info.clanAbbrev)
    ...
    def _fillQuestsList(self, model):
        ...
        for quest in quests:
            ...
            if quest.isCompleted():
                originalBonusData = quest.getBonuses()
                for bonus in originalBonusData:
                    if isinstance(bonus, BattleTokensBonus):
                        bonusTokens = bonus.getTokens()
                        for tokenID in bonusTokens.iterkeys():
                            if tokenID == PROGRESSION_TOKEN and not self.__progressionController.isProgressionFinished():
                                totalPointsEarned += bonus.getCount()
    ...
    def _getRelevantDailyQuests(self):
        ...
        for quest in quests.values():
            if quest.getID() in questsProgress:
                _, _, currentProgress = questsProgress[quest.getID()]
