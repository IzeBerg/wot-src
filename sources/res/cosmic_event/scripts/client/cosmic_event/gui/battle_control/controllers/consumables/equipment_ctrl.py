...
from cosmic_event_common.cosmic_constants import COSMIC_EVENT_ROCKET_BOOSTER, COSMIC_EVENT_RAPIDSHELLING, COSMIC_EVENT_BLACKHOLE, COSMIC_EVENT_OVERCHARGE, COSMIC_EVENT_SHIELD
...
class _CosmicEventHookShotItem(_CosmicBaseItem):
    _EFFECT_SWITCH_OFF_STAGES = (
     EQUIPMENT_STAGES.COOLDOWN, EQUIPMENT_STAGES.SHARED_COOLDOWN,
     EQUIPMENT_STAGES.EXHAUSTED)

    def activate(self, entityName=None, avatar=None):
        super(_CosmicEventHookShotItem, self).activate(entityName, avatar)
        CosmicBattleSounds.Abilities.playDpmIncreaseActivated()
        CosmicEffectComponentManager.toggleCurrentVehicleAdvancedShot()

    def onElapsed(self):
        super(_CosmicEventHookShotItem, self).onElapsed()
        CosmicBattleSounds.Abilities.playDpmIncreaseElapsed()
        CosmicEffectComponentManager.toggleCurrentVehicleBasicShot()
