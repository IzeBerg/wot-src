...
from cosmic_event.gui.battle_control.controllers.vehicle_hit_sound_ctrl import CosmicVehicleHitSound
from gui.battle_control.controllers import arena_load_ctrl, consumables, feedback_adaptor, msgs_ctrl, period_ctrl, vehicle_state_ctrl, view_points_ctrl, anonymizer_fakes_ctrl, prebattle_setups_ctrl, debug_ctrl, aiming_sounds_ctrl, kill_cam_ctrl
...
class CosmicDynamicControllersRepository(_ControllersRepositoryByBonuses):
    ...
    def create(cls, setup):
        ...
        repository.addViewController(battle_hints_ctrl.BattleHintsController(closeOnRoundFinished=False), setup)
        repository.addController(CosmicVehicleHitSound())
        return repository