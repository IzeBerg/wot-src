    ...
    def _generateOptions(self, ctx=None):
        ...
        if vehicle is None:
            ...
            if BATTLE_ROYALE_STATS_ENABLED:
                options.extend([
                 self._makeItem(VEHICLE.STATS, MENU.contextmenu(VEHICLE.STATS), {'enabled': True})])
            ...
            if rentState != EquipmentPanelCmpRentStates.STATE_NORMAL:
                ...
                if rentState in testDriveStates:
                    ...
                    options.extend([
                     self._makeItem(VEHICLE.TAKE_TEST_DRIVE, text, {'enabled': rentState == EquipmentPanelCmpRentStates.STATE_TEST_DRIVE_AVAILABLE})])
                    ...
                    options.extend([
                     self._makeItem(VEHICLE.TAKE_RENT, text, {'enabled': isEnabled})])
            return options
