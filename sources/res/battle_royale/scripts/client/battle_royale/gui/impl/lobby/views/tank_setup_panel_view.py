...
class TankSetupPanelView(ViewImpl, IGlobalListener):
    ...
    def __getTooltipData(self, event):
        ...
        if not tooltipType or not intCD:
            return None
        if tooltipType == TankSetupPanelViewModel.TOOLTIP_RESPAWN:
            return self.__getRespawnTooltipData()
        else:
            if tooltipType == TankSetupPanelViewModel.TOOLTIP_ABILITY:
                return self.__getAbilityTooltipData(intCD)
            return self.__getAmmoTooltipData(intCD)
    ...
    def __getArtefactData(self, artefact, vehicle, isEquipment):
        if isEquipment:
            return self.__getAbilityData(artefact, vehicle.name)
