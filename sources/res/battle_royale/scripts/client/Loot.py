...
class _LootHandler(ILootHandler):
    ...
    def onCreated(self, loot):
        event = LootEvent(LootEvent.LOOT_SPAWNED, ctx={'id': loot.id, 'position': loot.position})
        g_eventBus.handleEvent(event, scope=EVENT_BUS_SCOPE.BATTLE)

    def onDestroyed(self, loot):
        event = LootEvent(LootEvent.LOOT_PICKED_UP, ctx={'id': loot.id, 'position': loot.position})
        g_eventBus.handleEvent(event, scope=EVENT_BUS_SCOPE.BATTLE)
