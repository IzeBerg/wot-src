...
class FLRandomReserve(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if self.__isDisabled(index):
            return self.__getDisabledBlocks()
        items = [
         formatters.packBuildUpBlockData(blocks=[self.__getTitle(i18n.makeString(equipment.userString)), self.__getSubTitle()] if isStack and hasRandomBonusCap else [
         ...
         formatters.packBuildUpBlockData(padding=formatters.packPadding(right=20), blocks=[
          self.__getContent(i18n.makeString(equipment.shortDescription))], linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE if isStack else BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_LINKAGE)]
