...
class ArmoryYardEventValidator(SyncValidator):
    ...
    def _validate(self):
        if not self.__armoryYardCtrl.isActive():
            return makeError('event_is_not_active')
        return makeSuccess()
...
class BuyStepTokenCountValidator(SyncValidator):
    ...
    def _validate(self):
        ...
        if currencyTokenCount + self.__buyCount > maximumTokenCount:
            return makeError('result_token_count_more_than_max')
        return makeSuccess()
...
class CurrencyValidator(SyncValidator):
    ...
    def _validate(self):
        if self.__currency not in self.__armoryYardCtrl.getTokenCurrencies():
            return makeError(('invalid_currency: {}').format(self.__currency))
        return makeSuccess()
