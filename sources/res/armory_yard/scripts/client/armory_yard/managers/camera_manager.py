import BigWorld, math, math_utils, Math, CGF, GenericComponents
    ...
    def goToHangar(self, hangarSpace=None):
        if hangarSpace is not None and hangarSpace.space is not None:
            ...
            if hangarCameraManager is not None:
                ...
                hangarCameraManager.setCameraLocation(targetPos=pos, pivotPos=cfg['cam_pivot_pos'], yaw=math.radians(cfg['cam_start_angles'][0]), pitch=math.radians(cfg['cam_start_angles'][1]), dist=cfg['cam_start_dist'], camConstraints=[
                 cfg['cam_pitch_constr'], cfg['cam_yaw_constr'], cfg['cam_dist_constr']], movementMode=IMMEDIATE_CAMERA_MOVEMENT_MODE)
