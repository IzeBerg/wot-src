...
class LootBoxOpenedFormatter(ServiceChannelFormatter):
    ...
    def __formOpenedBoxesSection(self, openedLootBoxes):
        ...
        for boxID, count in openedLootBoxes.iteritems():
            ...
            if lootBox is not None and count > 0:
                openedStr = makeHtmlString('html_templates:lobby/quests/bonuses', 'lootBox', {'name': lootBox.getUserName(), 'count': count})
    ...
    def __formFailBoxesSection(self, openedLootBoxes, failedKeys):
        ...
        for boxID, _ in openedLootBoxes.iteritems():
            ...
            if lootBox.openedWithKey() and lootBox is not None:
                ...
                if countFail:
                    openedStr = makeHtmlString('html_templates:lobby/quests/bonuses', 'lootBox', {'name': lootBox.getUserName(), 'count': countFail})
    ...
    def __formFailKeySection(self, usedKeys):
        ...
        if countLockpick:
            lockpick.append(makeHtmlString('html_templates:lobby/quests/bonuses', 'lootBoxKey', {'name': backport.text(R.strings.quests.bonuses.item.lockpick()), 'count': countLockpick}))
        if countKey:
    ...
    def __splitRewards(self, allRewards):
        ...
        for token in allRewards.get('tokens', {}).keys():
            ...
            elif not token.startswith(LOOTBOX_TOKEN_PREFIX) and not token.startswith(LOOTBOX_KEY_PREFIX) or allRewards['tokens'][token].get('count', 0) > 0:
