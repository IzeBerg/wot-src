...
class EventLootBoxesOpenBoxScreen(ViewImpl):
    __slots__ = ('__compensationTooltipData', '__rewards', '__tooltipData', '__boxType',
                 '__vehicle', '__bonuses', '__notifier', '__osBit')
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.event_lootboxes.lobby.event_lootboxes.tooltips.CompensationTooltip():
            return EventLootBoxesCompensationTooltip(self.__compensationTooltipData)
        return super(EventLootBoxesOpenBoxScreen, self).createToolTipContent(event=event, contentID=contentID)
    ...
    def __getTimeLeft(self):
        ...
        if not boxesLeft:
            return self.__eventLootBoxes.getTimeLeftToResetPurchase() + time_utils.ONE_SECOND
        return 0
