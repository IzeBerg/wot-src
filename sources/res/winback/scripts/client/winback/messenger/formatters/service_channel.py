...
class WinbackProgressionAchievesFormatter(QuestAchievesFormatter):
    ...
    def _preprocessBoosters(cls, goodies):
        ...
        if boostersStrings:
            return g_settings.htmlTemplates.format('winbackBoostersInvoiceReceived', ctx={'boosters': (', ').join(boostersStrings)})
        else:
            return
    ...
    def _processSelectableBonusToken(tokenID):
        ...
        formattedResult = g_settings.htmlTemplates.format(('').join(['winback', selectableType, compensationTemplateSuffix]), ctx={'level': int2roman(level), 'value': compensationValue})
        return (
...
class WinbackProgressionSystemMessageFormatter(ServiceChannelFormatter):
    ...
    def _selectButtonTemplate(self, data):
        ...
        return 'ProgressionButton'

    def _getTemplate(self, data):
        return ('').join([
         'VersusAI', self.__winbackController.progressionName,
