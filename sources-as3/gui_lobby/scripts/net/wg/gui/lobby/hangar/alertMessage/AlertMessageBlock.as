      ...
      override protected function draw() : void
      {
         ...
         if(this._data != null)
         {
            ...
            if(isInvalid(InvalidationType.DATA))
            {
               ...
               _loc2_ = StringUtils.isNotEmpty(this._data.alertIcon);
               if(_loc2_)
               {
                  this.alertIcon.source = this._data.alertIcon;
               }
               this.alertIcon.visible = _loc2_;
               invalidateLayout();
            }
            if(isInvalid(InvalidationType.LAYOUT))
            {
               _loc3_ = this.alertIcon.width + ALERT_TO_MESSAGE_GAP + this.messageTF.width + MESSAGE_ICON_GAP;
               if(_loc1_)
               {
                  _loc3_ += this.popoverIconButton.width;
               }
               _loc4_ = this.bg.width - _loc3_ >> 1;
               this.alertIcon.x = _loc4_;
               _loc4_ += this.alertIcon.width + ALERT_TO_MESSAGE_GAP;
               this.messageTF.x = _loc4_;
               _loc4_ += this.messageTF.width + MESSAGE_ICON_GAP;
               if(_loc1_)
               {
                  this.popoverIconButton.x = _loc4_;
               }
