package net.wg.gui.lobby.hangar
{
   ...
   public class HangarHeader extends HangarHeaderMeta implements IHangarHeaderMeta, IHelpLayoutComponent, IHangarHeader, IStageSizeDependComponent
   {
      ...
      private var _rtsMetaWidget:RtsMetaWidget = null;
      
      private var _rtsSubmodeSelector:RtsSubModeSelector = null;
      ...
      override protected function onDispose() : void
      {
         ...
         this._rtsMetaWidget = null;
         this._rtsSubmodeSelector = null;
      ...
      public function as_createRTSBattles() : void
      {
         if(this._rtsMetaWidget == null && this._rtsSubmodeSelector == null)
         {
            this._scheduler.cancelTask(this.createRTS);
            this._scheduler.scheduleOnNextFrame(this.createRTS);
         }
      }
      ...
      public function as_removeRTSBattles() : void
      {
         var _loc1_:String = null;
         if(this._rtsSubmodeSelector != null)
         {
            if(this.questsFlags.getEntryPoint() is RtsSubModeSelector)
            {
               this.questsFlags.setEntryPoint(null);
            }
            _loc1_ = HANGAR_ALIASES.RTS_SUBMODE_SELECTOR;
            if(isFlashComponentRegisteredS(_loc1_))
            {
               unregisterFlashComponentS(_loc1_);
            }
            this._rtsSubmodeSelector = null;
         }
         if(this._rtsMetaWidget != null)
         {
            if(isFlashComponentRegisteredS(HANGAR_ALIASES.RTS_META_WIDGET))
            {
               unregisterFlashComponentS(HANGAR_ALIASES.RTS_META_WIDGET);
            }
            removeChild(this._rtsMetaWidget);
            this._rtsMetaWidget = null;
         }
         invalidateLayout();
      }
      ...
      private function createRTS() : void
      {
         this._rtsSubmodeSelector = new RtsSubModeSelector();
         this.questsFlags.setEntryPoint(this._rtsSubmodeSelector);
         registerFlashComponentS(this._rtsSubmodeSelector,HANGAR_ALIASES.RTS_SUBMODE_SELECTOR);
         this._rtsMetaWidget = new RtsMetaWidget();
         registerFlashComponentS(this._rtsMetaWidget,HANGAR_ALIASES.RTS_META_WIDGET);
         addChildAt(this._rtsMetaWidget,getChildIndex(this.questsFlags));
         invalidateLayout();
      }
