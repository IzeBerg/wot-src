package net.wg.infrastructure.managers.impl
{
   ...
   public class ElementBlurAdapter implements IElementBlurAdapter
   {
      ...
      public function blurElements(param1:int, param2:int) : void
      {
         ...
         {
            for each(_loc12_ in this._bluredElements)
            {
               if(_loc3_.indexOf(_loc12_) == -1)
               {
                  _loc12_.filters = null;
               }
            }
         }
         if(_loc4_.layer != TOPSUBVIEW_LAYER)
         {
            _loc13_ = _loc4_.numChildren;
            _loc14_ = 0;
            while(_loc14_ < _loc13_)
            {
               _loc3_.push(_loc4_.getChildAt(_loc14_));
               _loc14_++;
            }
         }
